...
\begin{example}
\begin{codeblock}
std::pair<int, @\oldtxt{const char*} \newtxt{std::string_view}@> pairs[] = {{2, "foo"}, {1, "bar"}, {0, "baz"}};
std::ranges::sort(pairs, std::@\newtxt{ranges::}@less<>{}, [](auto const& p) { return p.first; });
\end{codeblock}
...
a \oldtxt{random access iterator}
\newtxt{\oldconcept{RandomAccessIterator}}\iref{random.access.iterators} and
\removed{of a contiguous iterator\iref{iterator.requirements.general}.}
\begin{addedblock}
the additional requirement that\newtxt{, when \tcode{a} and \tcode{(a + n)} are
valid dereferenceable pointer values for some integral value \tcode{n},}
\begin{codeblock}
