...
\tcode{SwappableWith<T, U>} is satisfied. \tcode{Swappable\-With<T, U>} is
...
void value_swap(T&& t, U&& u) {
  ...
  @\added{ranges::}@swap(std::forward<T>(t), std::forward<U>(u)); // OK: uses ``swappable with'' conditions
...
void lv_swap(T& t1, T& t2) {
  @\removed{using std::swap;}@
  @\added{ranges::}@swap(t1, t2);                                 // OK: uses swappable conditions for
...
namespace N {
  ...
  void swap(A& x, Proxy p) {
    @\changed{std}{ranges}@::swap(x.m, p.a->m);                       // OK: uses context equivalent to swappable
