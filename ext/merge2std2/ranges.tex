...
namespace std::ranges {
  ...
  // \ref{range.view}, View
  template<class T>
    inline constexpr bool enable_view = @\seebelownc@;
  ...
  // \ref{range.view_interface}, class template \tcode{view_interface}
  template<class D>
  ...
  template<WeaklyIncrementable W, Semiregular Bound = unreachable_t>
...
\item
  Otherwise, \tcode{DIFF}, where \tcode{DIFF} is
  \tcode{(ranges::end(E) - ranges::begin(E))}
  except that \tcode{E} is only evaluated once,
  if \tcode{DIFF} is a valid expression and
...
Viewable ranges can be converted to views and stored safely.
...
\pnum
Pursuant to \cxxref{namespace.std}, users may specialize \tcode{enable_view}
...
\pnum
\tcode{ContiguousRange} breaks this pattern by additionally requiring that
...
model \libconcept{DerivedFrom<view_interface<D>>} and \libconcept{View}.
...
\pnum
If a range adaptor object accepts more than one arguments,
...
\pnum
Many of the types in this subclause are specified in terms of
an exposition-only class template \tcode{\placeholder{semiregular}<T>}.
...
namespace std::ranges {
  ...
  public:
    ...
    template<@\placeholder{not-same-as}@<@\placeholder{ref-view}@> T>
      requires @\seebelow@
    constexpr @\placeholder{ref-view}@(T&& t) noexcept(is_nothrow_convertible_v<T, R&>);
...
namespace std::ranges {
  ...
  public:
    ...
    {
      if constexpr (is_lvalue_reference(decltype(*i)))
        return std::move(*i);
      else
        return *i;
    }
...
\pnum
\expects
\tcode{Bound} denotes \tcode{unreachable_t} or
...
\pnum
\tcode{take_view} produces a \libconcept{View} of at most $N$ elements
...
\effects Equivalent to:
\begin{codeblock}
auto&& inner_rng = @\placeholdernc{inner-range}@;
...
\begin{codeblock}
namespace std::ranges {
  template<auto> struct @\placeholder{require-constant}@; // \expos
...
namespace std::ranges {
  ...
  public:
    ...
    constexpr decltype(auto) operator*() const { return *i_.@\placeholdernc{current}@; }
...
\item If \tcode{T} models \libconcept{Iterator} and
  ...
  \item Otherwise,
    \tcode{subrange\{counted_iterator\{E, F\},
    default_sentinel\{\}\}}.
\end{itemize}
...
\pnum
\returns
\tcode{make_reverse_iterator(ranges::next(ranges::begin(base_), ranges::end(base_)))}.
