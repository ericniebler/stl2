...
namespace std { @\oldtxt{namespace experimental \{}@
  namespace ranges {
    ...
    template<Range R>
    using safe_iterator_t = @\oldtxt{\seebelow}\newtxt{decltype(ranges::begin(declval<R>()))}@;
    ...
    // \expos
    template<class R>
    concept @\placeholder{tiny-range}@ = @\seebelow@;
...
namespace std { namespace ranges {
    ...
    private:
      ...
      I begin_ {}; // \expos
      S end_ {}; // \expos
      @\newtxt{iter_}@difference@\oldtxt{_type}@_t<I> size_ = 0; // \expos; only present when StoreSize is true
    public:
      using iterator = I;
      using sentinel = S;
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  public:
    ...
    @\newtxt{using iterator_concept  = \seebelow;}@
    using value_type        = @\newtxt{iter_}@value@\oldtxt{_type}@_t<iterator_t<R>>;
...
\tcode{\newtxt{DerivedFrom<C, bidirectional_iterator_tag>}}\newtxt{ is satisfied},
...
\tcode{ForwardRange<R>}}
\tcode{\newtxt{DerivedFrom<C, forward_iterator_tag>}}\newtxt{ is satisfied},
then \tcode{iterator_category} \newtxt{denotes} \oldtxt{is an alias for}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  public:
    using iterator_category =
      @\newtxt{typename iterator_traits<iterator_t<Base>>::iterator_category;}@
...
\indexlibrary{\idxcode{operator!=}!\idxcode{take_view::iterator}}%
\begin{itemdecl}
friend constexpr bool operator!=(const sentinel& x, const CI& y)
  @\oldtxt{requires EqualityComparable<iterator_t<Base>>}@;
\end{itemdecl}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  private:
    using InnerRng = @\newtxt{iter_}@reference_t<iterator_t<R>>; // \expos
    template<bool Const>
      struct iterator; // \expos
    template<bool Const>
      struct sentinel; // \expos
...
\item If \tcode{ref_is_glvalue} is \tcode{true},
  \begin{itemize}
  \item If \tcode{DerivedFrom<\placeholder{OUTERC}, bidirectional_iterator_tag>} and
  \tcode{DerivedFrom<\placeholder{INNERC}, bidirectional_iterator_tag>}
  are each satisfied, \tcode{iterator_category} denotes
  \tcode{bidirectional_iterator_tag}.

  \item Otherwise, if \tcode{DerivedFrom<\placeholder{OUTERC}, forward_iterator_tag>} and
  \tcode{DerivedFrom<\placeholder{INNERC}, forward_iterator_tag>}
  are each satisfied, \tcode{iterator_category} denotes
...
\rSec4[range.adaptors.split_view.ctor]{\tcode{split_view} constructors}
...
\rSec4[range.adaptors.split_view.begin]{\tcode{split_view} range begin}
...
\rSec4[range.adaptors.split_view.end]{\tcode{split_view} range end}
...
\pnum
\begin{note}
\tcode{split_view::outer_iterator} is an exposition-only type.
\end{note}
...
\pnum
\tcode{split_view<Rng, Pattern>::outer_iterator::iterator_category} is defines
as follows:
\begin{itemize}
\item If \tcode{outer_iterator::Base} \oldtxt{satisfies} \newtxt{models} \tcode{ForwardRange}, then
\tcode{iterator_category} is \tcode{ranges::forward_iterator_tag}.
...
do {
  auto [b,p] = mismatch(@\placeholdernc{current}@, end, pbegin, pend);
  if (p != pend) continue; // The pattern didn't match
  @\placeholder{current}@ = @\placeholdernc{bump}@(b, pbegin, pend, end); // skip the pattern
  break;
...
\rSec4[range.adaptors.split_view.outer_sentinel.ctor]{\tcode{split_view::outer_sentinel} constructors}
...
\rSec4[range.adaptors.split_view.outer_sentinel.nonmember]{\tcode{split_view::outer_sentinel} non-member functions}
...
\begin{itemize}
\item If \tcode{Rng} \oldtxt{satisfies} \newtxt{models} \tcode{ForwardRange}, \tcode{x.current_}.
...
\pnum
\begin{note}
\tcode{split_view::outer_iterator::value_type} is an exposition-only type.
\end{note}
...
\rSec4[range.adaptors.split_view.outer_iterator.value_type.ctor]{\tcode{split_view::outer_iterator::value_type} constructors}
...
\rSec4[range.adaptors.split_view.outer_iterator.value_type.begin]{\tcode{split_view::outer_iterator::value_type} range begin}
...
\rSec4[range.adaptors.split_view.outer_iterator.value_type.end]{\tcode{split_view::outer_iterator::value_type} range end}
...
\pnum
\begin{note}
\tcode{split_view::inner_iterator} is an exposition-only type.
\end{note}
...
\begin{itemdescr}
\pnum
\returns \tcode{*\placeholdernc{current}(i_)}.
\end{itemdescr}
...
\begin{itemdecl}
friend constexpr decltype(auto) iter_move(const inner_iterator& i)
  noexcept(@\oldtxt{\seebelow}\newtxt{noexcept(ranges::iter_move(\placeholdernc{current}(i.i_)))}@);
\end{itemdecl}

\begin{itemdescr}
\pnum
\returns \tcode{ranges::iter_move(\placeholdernc{current}(i.i_))}.
...
\begin{itemdecl}
friend constexpr void iter_swap(const inner_iterator& x, const inner_iterator& y)
  noexcept(@\oldtxt{\seebelow}\newtxt{noexcept(ranges::iter_swap(\placeholdernc{current}(x.i_), \placeholdernc{current}(y.i_)))}@)
  requires IndirectlySwappable<iterator_t<Base>>;
\end{itemdecl}
...
\pnum
The name \tcode{view::split} denotes a range adaptor object\iref{range.adaptor.object}.
...
IndirectlyComparable<iterator_t<T>, const @\newtxt{iter_}@value@\oldtxt{_type}@_t<iterator_t<T>>*> &&
CopyConstructible<@\newtxt{iter_}@value@\oldtxt{_type}@_t<iterator_t<T>>> &&
ConvertibleTo<U, @\newtxt{iter_}@value@\oldtxt{_type}@_t<iterator_t<T>>>
\end{codeblock}
\end{itemize}
...
static_cast<\newtxt{iter_}difference\oldtxt{_type}_t<T>{>}(F)), default_sentinel\{\}\}}
if \tcode{It\-erator<T> \&\& ConvertibleTo<U, \newtxt{iter_}difference\oldtxt{_type}_t<T>{>}} is satisfied.
...
\rSec4[range.adaptors.common_view.ctor]{\tcode{common_view} constructors}
...
\rSec4[range.adaptors.common_view.conv]{\tcode{common_view} conversion}
...
\rSec4[range.adaptors.common_view.begin]{\tcode{common_view} begin}
...
\rSec4[range.adaptors.common_view.end]{\tcode{common_view} end}
...
\rSec4[range.adaptors.common_view.size]{\tcode{common_view} size}
...
\rSec4[range.adaptors.reverse_view.ctor]{\tcode{reverse_view} constructors}
...
\rSec4[range.adaptors.reverse_view.conv]{\tcode{reverse_view} conversion}
...
\rSec4[range.adaptors.reverse_view.begin]{\tcode{reverse_view} begin}
...
\rSec4[range.adaptors.reverse_view.end]{\tcode{reverse_view} end}
...
\rSec4[range.adaptors.reverse_view.size]{\tcode{reverse_view} size}
