...
\item Otherwise, if \tcode{T} is a specialization of class template
  ...
  \tcode{unordered_set}\cxxiref{unord.set},
  \tcode{unordered_multiset}\cxxiref{unord.multiset}, or
  \tcode{match_results}\cxxiref{re.results}, \tcode{false}.
...
\pnum
\tcode{ContiguousRange} additional requires that
...
\begin{itemdecl}
constexpr decltype(auto) back() requires BidirectionalRange<D> && constexpr decltype(auto) back() const
  requires BidirectionalRange<const D> && CommonRange<const D>CommonRange<D>;
\end{itemdecl}
