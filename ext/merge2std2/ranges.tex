...
\rSec1[range.syn]{Header \tcode{<ranges>} synopsis}
...
types of \tcode{ranges::end(E)} and \tcode{ranges::begin(E)} model
\libconcept{Sentinel}.
\end{note}
...
types of \tcode{ranges::cend(E)} and \tcode{ranges::cbegin(E)} model
\tcode{Sentinel}.
\end{note}
...
types of \tcode{ranges::\brk{}rend(E)} and \tcode{ranges::\brk{}rbegin(E)}
model \tcode{Sentinel}.
\end{note}
...
types of \tcode{ranges::crend(E)} and \tcode{ranges::\brk{}crbegin(\brk{}E)}
model \tcode{Sentinel}.
\end{note}
...
\item
  Otherwise, \tcode{\placeholdernc{DECAY_COPY}(E.size())}
  if it is a valid expression and its type \tcode{I} models
  \libconcept{Integral} and
  \tcode{disable_sized_range<remove_cvref_t<T>>}\iref{range.sized}
  is \tcode{false}.
...
\item
  ...
  \tcode{disable_sized_range<remove_cvref_t<T>>} is \tcode{false}.

\item
  Otherwise,
  \tcode{\placeholdernc{DECAY_COPY}(ranges::end(E) - ranges::begin(E))},
...
when the return type does not satisfy \libconcept{ForwardIterator}, repeated calls
...
not in fact satisfy \libconcept{SizedRange}. A program that instantiates a library template
...
\begin{itemdescr}
\pnum
\effects Initializes \tcode{current_} with \tcode{i.current_} and
...
\begin{itemdescr}
\pnum
Let \tcode{Rng} be \tcode{R} for the first overload and \tcode{const R} for the
second.

\pnum
\effects
\begin{itemize}
\item If \tcode{Rng} models both \libconcept{SizedRange} and
  \libconcept{RandomAccessRange}, equivalent to:
  \tcode{return ranges::begin(base_);}
\item Otherwise, if \tcode{Rng} models \libconcept{SizedRange}, equivalent to:
...
\item Otherwise, equivalent to:
  \tcode{return sentinel<is_const_v<Rng>>\{ranges::end(base_)\};}
\end{itemize}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Initializes \tcode{outer_} with \tcode{i.outer_},
\tcode{inner_} with \tcode{i.inner_}, and
\tcode{parent_} with \tcode{i.parent_}.
\end{itemdescr}
...
namespace std::ranges {
  ...
  class split_view : public view_interface<split_view<Rng, Pattern>> {
  private:
    Rng base_ {};                               // \expos
...
\begin{codeblock}
return outer_iterator<@\placeholder{simple-view}@<Rng>>{*this, ranges::begin(base_)};
\end{codeblock}
...
\pnum
\effects Equivalent to:
\tcode{return outer_iterator<\placeholder{simple-view}<Rng>>\{*this, ranges::end(base_)\};}
\end{itemdescr}
...
\effects
If \tcode{Rng} models
\tcode{ForwardRange<Rng> \&\& ForwardRange<const Rng> \&\& CommonRange<const Rng>},
equivalent to:
...
namespace std::ranges {
  ...
  template<bool Const>
  struct split_view<Rng, Pattern>::outer_iterator { // \expos
  private:
    using Parent =                // \expos
...
\tcode{current_} with \tcode{i.current_}.
\end{itemdescr}
...
namespace std::ranges {
  template<class Rng, class Pattern>
  template<bool Const>
  struct split_view<Rng, Pattern>::inner_iterator { // \expos
  private:
    using Base =
      conditional_t<Const, const Rng, Rng>; // \expos
...
\begin{itemdescr}
\pnum
Let \tcode{R} be \tcode{Rng} or \tcode{const Rng} for the first and
...
\item Otherwise, \tcode{common_view\{E\}}, if that is a well-formed expression.
