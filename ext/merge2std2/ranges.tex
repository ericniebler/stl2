...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  public:
    ...
    constexpr split_view(O&& o, @\newtxt{iter_}@value@\oldtxt{_type}@_t<iterator_t<O>> e)@\oldtxt{;}@
      @\newtxt{: split_view\{view::all(std::forward<O>(o)), single_view\{std::move(e)\}\}}@
    @\newtxt{\{\}}@
...
\effects
{\color{newclr}
Initializes \tcode{base_} with \tcode{Rng(view::all(std::forward<O>(o)))} and
\tcode{pattern_} with \tcode{Pattern(\brk{}view::all(std::forward<P>(p)))}.
} %% \color{newclr}

\oldtxt{Delegates to:
}\tcode{\oldtxt{split_view\{view::all(std::forward<O>(o)), view::all(std::forward<P>(p))\}}}
\end{itemdescr}
...
\begin{itemdecl}
constexpr auto end()
constexpr auto end() const requires ForwardRange<@\newtxt{const}@ Rng>;
\end{itemdecl}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  private:
    ...
    iterator_t<Base> current_ {}; // \expos, present only if \tcode{\newtxt{Base}\oldtxt{Rng}} models \libconcept{ForwardRange}
    Parent* parent_ = nullptr;    // \expos
  public:
    using iterator_category = @\newtxt{input_iterator_tag}\oldtxt{\seebelow}@;
...
Where {\color{oldclr}
\tcode{\placeholder{current}} is equivalent to:
\begin{itemize}
...
\oldtxt{and} \tcode{\placeholdernc{bump}(b, x, y, e)} is equivalent to:
\begin{itemize}
\item If \tcode{Rng} \oldtxt{satisfies} \newtxt{models} \libconcept{ForwardRange},
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  private:
    ...
    bool zero_ = false;          @\newtxt{// \expos}@
  public:
    using iterator_category = iterator_category_t<outer_iterator<Const>>; // FIXME
    using difference_type = @\newtxt{iter_}@difference@\oldtxt{_type}@_t<iterator_t<Base>>;
...
\begin{itemdescr}
\pnum
\newtxt{\effects Equivalent to: \tcode{return *i_.\placeholdernc{current};}}
