...
namespace std { @\oldtxt{namespace experimental \{}@
  namespace ranges {
    ...
    @\newtxt{// \ref{range.adaptors.take_view}}@
    @\newtxt{template<View R>}@
    @\newtxt{class take_view;}@

    @\newtxt{namespace view \{ inline constexpr \unspec take = \unspecnc; \}}@
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \tcode{\oldtxt{return ranges::begin(derived());}}
\begin{codeblock}
...
\begin{codeblock}
using I = @\placeholder{range-common-iterator}@<@\newtxt{const D} \oldtxt{R}@>;
...
\pnum
Many of the types in this subclause are specified in terms of \oldtxt{an}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  public:
    ...
    template<@\oldtxt{Input}\newtxt{Viewable}@Range O>
      requires @\oldtxt{ViewableRange<O> \&\&}@ Constructible<R, all_view<O>>
    constexpr take_view(O&& o, @\newtxt{iter_}@difference@\oldtxt{_type}@_t<iterator_t<R>> count);

    constexpr R base() const;

    constexpr auto begin() @\newtxt{requires !\placeholder{simple-view}<R>}@;
    constexpr auto begin() const requires Range<const R>;
    @\oldtxt{constexpr auto begin() requires RandomAccessRange<R> \&\& SizedRange<R>;}@
    @\oldtxt{constexpr auto begin() const}@
      @\oldtxt{requires RandomAccessRange<const R> \&\& SizedRange<const R>;}@

    constexpr auto end() @\newtxt{requires !\placeholder{simple-view}<R>}@;
    constexpr auto end() const requires Range<const R>;
    @\oldtxt{constexpr auto end() requires RandomAccessRange<R> \&\& SizedRange<R>;}@
...
\begin{itemdescr}
\pnum
\effects Initializes \tcode{base_} with \tcode{std::move(base)} and \oldtxt{initializes}
...
\pnum
\effects Initializes \tcode{base_} with \tcode{view::all(std::forward<O>(o))}
and \oldtxt{initializes} \tcode{count_} with \tcode{count}.
\end{itemdescr}
...
\begin{itemize}
{\color{newclr}
\item If \tcode{Rng} models both
\libconcept{SizedRange} and \libconcept{RandomAccessRange},
\tcode{return ranges::begin(base_);}

\item Otherwise, if \tcode{Rng} models \libconcept{SizedRange},
\tcode{return counted_iterator\{ranges::begin(base_), size()\};}
} %% \color{newclr}

\item \newtxt{Otherwise,} \tcode{return \oldtxt{make_}counted_iterator\oldtxt{(}\newtxt{\{}ranges::begin(base_), count_\newtxt{\}}\oldtxt{)};}
\end{itemize}
\end{itemdescr}
...
\effects
Equivalent to:
\tcode{\oldtxt{sentinel<\placeholder{simple-view}<R>>\{ranges::end(base_)\}}}
\oldtxt{and } \\ \tcode{\oldtxt{sentinel<true>\{ranges::end(base_)\}}}
\oldtxt{for the first and second overload, respectively.}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  private:
    @\oldtxt{using Parent =                                 // \expos}@
      @\oldtxt{conditional_t<Const, const take_view, take_view>;}@
    using Base = conditional_t<Const, const R, R>; // \expos
  ...
  public:
    ...
    friend constexpr bool operator==(const CI& @\oldtxt{x}\newtxt{y}@, const sentinel& @\oldtxt{y}\newtxt{x}@)
      @\oldtxt{requires EqualityComparable<iterator_t<Base>>}@;
    friend constexpr bool operator!=(const sentinel& x, const CI& y)
      @\oldtxt{requires EqualityComparable<iterator_t<Base>>}@;
    friend constexpr bool operator!=(const CI& @\oldtxt{x}\newtxt{y}@, const sentinel& @\oldtxt{y}\newtxt{x}@)
...
friend constexpr bool operator==(const sentinel& x, const CI& y)
  @\oldtxt{requires EqualityComparable<iterator_t<Base>>}@;
@\newtxt{friend constexpr bool operator==(const CI\& y, const sentinel\& x)}@
\end{itemdecl}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  public:
    ...
    @\newtxt{constexpr T* begin() noexcept;}@
    constexpr const T* begin() const noexcept;
    @\newtxt{constexpr T* end() noexcept;}@
    constexpr const T* end() const noexcept;
    constexpr static ptrdiff_t size() noexcept;
    @\newtxt{constexpr T* data() noexcept;}@
...
\pnum
\oldtxt{\returns \tcode{value_.operator->()}.}

\pnum
\newtxt{\effects Equivalent to: \tcode{return data();}}
\end{itemdescr}
...
\pnum
\oldtxt{\returns \tcode{value_.operator->() + 1}.}

\pnum
\newtxt{\effects Equivalent to: \tcode{return data() + 1;}}
\end{itemdescr}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace ranges \{}@
  ...
  public:
    ...
    constexpr auto begin();
    @\oldtxt{constexpr auto begin() requires ForwardRange<Rng>;}@
    constexpr auto begin() const
    ...
    constexpr auto end() const@\newtxt{;}@
      @\oldtxt{requires ForwardRange<const Rng> \&\& CommonRange<const Rng>;}@
  };
...
\pnum
\effects
\newtxt{If \tcode{Rng} models \libconcept{ForwardRange}, equivalent to:}
\begin{codeblock}
@\newtxt{return outer_iterator<\placeholder{simple-view}<Rng>>\{*this, ranges::begin(base_)\};}@
\end{codeblock}
...
\pnum
\effects Equivalent to:
\tcode{return outer_iterator<\newtxt{true} \oldtxt{\placeholder{simple-view}<R>}>\{*this, ranges::begin(base_)\};} \\
\oldtxt{and }\tcode{\oldtxt{return outer_iterator<true>\{*this, ranges::begin(base_)\};}}\oldtxt{
for the first and second overloads, respectively.}
\end{itemdescr}
...
\effects
Equivalent to:
\tcode{return outer_sentinel<\placeholder{simple-view}<R>>\{*this\};}
...
constexpr auto end()
  requires ForwardRange<Rng> && CommonRange<Rng>;
@\oldtxt{constexpr auto end() const}@
  @\oldtxt{requires ForwardRange<Rng> \&\& CommonRange<Rng>;}@
\end{itemdecl}

\begin{itemdescr}
\pnum
\effects Equivalent to:
\tcode{return outer_iterator<\placeholder{simple-view}<R\newtxt{ng}>>\{*this, ranges::end(base_)\};} \\
...
\pnum
\effects
If \tcode{Rng} models \tcode{ForwardRange<Rng> \&\& ForwardRange<const Rng> \&\& CommonRange<const Rng>}, equivalent to: \tcode{return outer_iterator<true>\{*this, ranges::end(base_)\};}
...
@\newtxt{if constexpr (!ForwardRange<Base>) \{}@
  @\newtxt{if constexpr (Pattern::size() == 0) \{}@
    @\newtxt{return *this;}@
  @\newtxt{\}}@
@\newtxt{\}}@
