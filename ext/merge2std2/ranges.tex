...
customization point object\cxxiref{customization.point.object}.
\newtxt{For some subexpressions \tcode{E} and \tcode{F},}
...
\effects Equivalent to:
\begin{codeblock}
auto update_inner = [this](@\newtxt{iter_}@reference_t<iterator_t<Base>> x) -> decltype(auto) {
...
\begin{codeblock}
namespace std::ranges {
  @\newtxt{template <auto> struct \placeholder{require-constant}; // \expos}@

  template<class R>
  concept @\placeholder{tiny-range}@ = // \expos
    SizedRange<R> &&
    @\newtxt{requires \{ typename \placeholder{require-constant}<remove_reference_t<R>::size()>; \} \&\&}@
...
namespace std::ranges {
  ...
  private:
    ...
    bool @\oldtxt{zero_}\newtxt{incremented_}@ = false;                 // \expos
  public:
...
if (pcur == pend) return x.@\oldtxt{zero_}\newtxt{incremented_}@;
do {
...
\pnum
\effects Equivalent to
\tcode{ranges::iter_swap(\newtxt{x.i_.}\placeholdernc{current}\oldtxt{(x.i_)}, \newtxt{y.i_.}\placeholdernc{current}\oldtxt{(y.i_)})}.
\end{itemdescr}
...
\oldtxt{such that their decayed types are \tcode{T} and \tcode{U} respectively}.
Then the expression \tcode{view::counted(E, F)} is expression-equivalent to:

\begin{itemize}
\item If \tcode{T} models \libconcept{Iterator} and \oldtxt{\tcode{U}}
\newtxt{\tcode{decltype((F))}} models \libconcept{ConvertibleTo<iter_difference_t<T>>},
  \begin{itemize}
  \item \tcode{subrange\{E, E + \newtxt{static_cast<iter_difference_t<T>>(}F\newtxt{)}\}}
    if \tcode{T} models \libconcept{ContiguousIterator}.
  \item Otherwise,
    \tcode{subrange\{counted_iterator\{E, \oldtxt{static_cast<iter_difference_t<T>>(}F\oldtxt{)}\},
    default_sentinel\{\}\}}.
\end{itemize}
...
namespace std::ranges {
  ...
  public:
    ...
    @\oldtxt{constexpr auto begin()}@
      @\oldtxt{requires RandomAccessRange<V> \&\& SizedRange<V>;}@
    @\oldtxt{constexpr auto begin() const}@
      @\oldtxt{requires RandomAccessRange<const V> \&\& SizedRange<const V>;}@
...
\begin{itemize}
\item \newtxt{\tcode{view::all(E)}}, if \tcode{decltype((E))} models \libconcept{CommonRange}
\newtxt{and} \tcode{view::all(E)}\oldtxt{, if that} is a well-formed expression.
...
\pnum
\returns
\tcode{\newtxt{make_}reverse_iterator(ranges::next(ranges::begin(base_), ranges::end(base_)))}.
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \tcode{return \newtxt{make_}reverse_iterator(ranges::end(base_));}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \tcode{return \newtxt{make_}reverse_iterator(ranges::begin(base_));}
\end{itemdescr}
