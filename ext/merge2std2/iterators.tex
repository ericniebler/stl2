...
The generic term \term{iterator} refers to any type that models the
...
\pnum
Destruction of an iterator \added{whose category is weaker than forward}
...
respectively. \added{The type }\tcode{\added{iterator_traits<I>::pointer}}\added{ shall be
...
\end{codeblock}
\added{If the \grammarterm{qualified-id} \tcode{I::pointer} is valid and
denotes a type, then
}\tcode{\added{iterator_traits<I>::pointer}}\added{ names that type; otherwise,
it names \tcode{void}.}
...
\tcode{pointer} names that type. Otherwise, \tcode{pointer} is \tcode{void}.
