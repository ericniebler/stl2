...
\end{codeblock}\begin{addedblock}\begin{codeblock}
  template<class T> using @\placeholder{with-reference}@ // \expos
    = T&;
...
\end{codeblock}\begin{addedblock}\begin{codeblock}
  ...
  namespace ranges {
    ...
    inline namespace @\unspec@ {
      // \ref{iterator.cust.move}, iter_move
      inline constexpr @\unspec@ iter_move = @\unspecnc@;

      // \ref{iterator.cust.swap}, iter_swap
  ...
  // \ref{common.alg.req.indirectlymovable} IndirectlyMovable
  ...
  // \ref{common.alg.req.indirectlycopyable} IndirectlyCopyable
  template<class In, class Out>
  ...
  // \ref{common.alg.req.indirectlyswappable} IndirectlySwappable
  template<class I1, class I2 = I1>
    concept IndirectlySwappable = @\seebelownc@;

  // \ref{common.alg.req.indirectlycomparable} IndirectlyComparable
  template<class I1, class I2, class R, class P1 = identity, class P2 = identity>
    concept IndirectlyComparable = @\seebelownc@;

  // \ref{common.alg.req.permutable} Permutable
  template<class I>
    concept Permutable = @\seebelownc@;

  // \ref{common.alg.req.mergeable} Mergeable
...
\pnum
Destruction of a\removed{n} \added{non-forward} iterator may invalidate
...
\tcode{iterator_traits<I>::\brk{}difference_type}.
...
If \tcode{iterator_traits<I>} names a specialization
generated from the primary template,
then \tcode{iter_value_t<I>} denotes \tcode{readable_traits<I>::value_type};
otherwise, it denotes \tcode{iterator_traits<I>::value_type}.
...
\begin{codeblock}
@\removed{iterator_traits<Iterator>::reference}@
iterator_traits<@\changed{Iterator}{I}@>::pointer
@\added{iterator_traits<I>::reference}@
\end{codeblock}

shall be defined as the iterator's \added{pointer and} reference \removed{and pointer} types\changed{,}{;} that is, for an
iterator object \tcode{a} \added{of class type}, the same type as
\removed{the type of \tcode{*a} and \tcode{a->}}
\added{\tcode{decltype(a.operator->())} and \tcode{decltype(*a)}},
respectively. \added{The type
}\tcode{\added{iterator_traits<I>::pointer}}\added{ shall be \tcode{void} for
an iterator of class type \tcode{I} that does not support
\tcode{operator->}. Additionally, i}\removed{I}n the case of an output iterator,
the types

\begin{codeblock}
@\added{iterator_traits<I>::value_type}@
...
\tcode{reference}, and \tcode{iterator_category}, \added{then}
\tcode{iterator_traits<\changed{Iterator}{I}>}
\changed{shall have}{has} the following \removed{as}
publicly accessible members:
\begin{codeblock}
  @\added{using iterator_category = typename I::iterator_category;}@
  @\removed{using difference_type{ }{ }{ }= typename Iterator::difference_type;}@
  using value_type        = typename @\changed{Iterator}{I}@::value_type;
  @\added{using difference_type{ }{ }{ }= typename I::difference_type;}@
  using pointer           = @\changed{typename Iterator::pointer}{\seebelownc}@;
  using reference         = typename @\changed{Iterator}{I}@::reference;
  @\removed{using iterator_category = typename Iterator::iterator_category;}@
\end{codeblock}
\begin{addedblock}
...
then \tcode{iterator_traits<I>} has the following
publicly accessible members:
\begin{codeblock}
...
\changed{shall have}{has} no members by any of the above names.
\end{itemize}
...
namespace std {
  ...
  struct iterator_traits<T*> {
    @\added{using iterator_concept}@  @\added{= contiguous_iterator_tag;}@
    @\added{using iterator_category = random_access_iterator_tag;}@
    @\added{using value_type}@        @\added{= remove_cv_t<T>;}@
    using difference_type   = ptrdiff_t;
    @\removed{using value_type}@        @\removed{= remove_cv_t<T>;}@
    using pointer           = T*;
    using reference         = T&;
    @\removed{using iterator_category = random_access_iterator_tag;}@
  };
}
\end{codeblock}
...
\begin{codeblock}
template<class B@\changed{idirectionalIterator}{I}@>
void reverse(B@\changed{idirectionalIterator}{I}@ first, B@\changed{idirectionalIterator}{I}@ last) {
  typename iterator_traits<B@\changed{idirectionalIterator}{I}@>::difference_type n =
    distance(first, last);
  --n;
  while(n > 0) {
    typename iterator_traits<B@\changed{idirectionalIterator}{I}@>::value_type
     tmp = *first;
    *first++ = *--last;
    *last = tmp;
    n -= 2;
...
a customization point object\cxxiref{customization.point.object}.
...
\tcode{I} models \libconcept{ContiguousIterator} only if
...
iterator's behavior. \added{Additionally,
...
if \added{the definitions of} any of the members
\begin{itemize}
\item
  \tcode{operator+},
  \tcode{operator-},
...
\end{itemize}
are \removed{referenced in a way that requires instantiation}
\added{instantiated}\cxxiref{temp.inst}.
...
namespace std {
  ...
  public:
    ...
    constexpr @\changed{move_iterator}{auto}@ operator++(int);
    constexpr move_iterator& operator--();
...
namespace std {
  struct unreachable_t {
    template<WeaklyIncrementable I>
      friend constexpr bool operator==(unreachable_t, const I&) noexcept;
    template<WeaklyIncrementable I>
      friend constexpr bool operator==(const I&, unreachable_t) noexcept;
    template<WeaklyIncrementable I>
      friend constexpr bool operator!=(unreachable_t, const I&) noexcept;
    template<WeaklyIncrementable I>
      friend constexpr bool operator!=(const I&, unreachable_t) noexcept;
...
\indexlibrary{\idxcode{operator==}!\idxcode{unreachable_t}}%
\indexlibrary{\idxcode{unreachable_t}!\idxcode{operator==}}%
\begin{itemdecl}
template<WeaklyIncrementable I>
  friend constexpr bool operator==(unreachable_t, const I&) noexcept;
template<WeaklyIncrementable I>
...
namespace std {
  ...
  public:
    ...
    @\added{constexpr istream_iterator(default_sentinel_t);}@
    ...
    @\added{friend bool operator==(const istream_iterator\& i, default_sentinel_t);}@
    @\added{friend bool operator==(default_sentinel_t, const istream_iterator\& i);}@
    @\added{friend bool operator!=(const istream_iterator\& x, default_sentinel_t y);}@
    @\added{friend bool operator!=(default_sentinel_t x, const istream_iterator\& y);}@
...
constructor\added{s}.
\\ \ednote{If P0738 "I Stream, You Stream, We All Stream for \tcode{istream_iterator}"
lands at the same time as this proposal, the recommended merge for this sentence
is "If \tcode{is_trivially_default_constructible_v<T>} is \tcode{true},
...
namespace std {
  ...
  public:
    ...
    @\added{constexpr istreambuf_iterator(default_sentinel_t) noexcept;}@
    ...
    @\added{friend bool operator==(default_sentinel_t s, const istreambuf_iterator\& i);}@
    @\added{friend bool operator==(const istreambuf_iterator\& i, default_sentinel_t s);}@
    @\added{friend bool operator!=(default_sentinel_t a, const istreambuf_iterator\& b);}@
    @\added{friend bool operator!=(const istreambuf_iterator\& a, default_sentinel_t b);}@
