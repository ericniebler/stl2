...
\end{codeblock}\begin{addedblock}\begin{codeblock}
  ...
  template<class F, class... Is>
    requires @\newtxt{(Readable<Is> \&\& ...) \&\&}@ Invocable<F, @\newtxt{iter_}@reference_t<Is>...>
...
template<class X, class Y>
  constexpr @\newtxt{iter_}@value@\oldtxt{_type}@_t<remove_reference_t<X>> iter_exchange_move(X&& x, Y&& y)
    noexcept(@\seebelow@);
\end{itemdecl}
...
{\color{newclr}
\pnum
Let \tcode{\placeholdernc{ITER_TRAITS}(I)} be \tcode{I} if \tcode{iterator_traits<I>}
names an instantiation of the primary template; otherwise, \tcode{iterator_traits<I>}.

\pnum
Let \tcode{\placeholdernc{ITER_CONCEPT}(I)} be defined as follows:
\begin{itemize}
\item If \tcode{\placeholdernc{ITER_TRAITS}(I)::iterator_concept} is valid
  and names a type, then \tcode{\placeholdernc{ITER_TRAITS}(I)::iterator_concept}.
\item Otherwise, if \tcode{\placeholdernc{ITER_TRAITS}(I)\brk{}::iterator_category} is valid
  and names a type, then \tcode{\placeholdernc{ITER_TRAITS}(I)::iterator_category}.
...
\item Otherwise, \tcode{\placeholdernc{ITER_CONCEPT}(I)} does not name a type.
\end{itemize}
} %% \color{newclr}
...
@\newtxt{namespace std \{}@
  ...
  template<class F, class... Is>
    requires @\newtxt{(Readable<Is> \&\& ...) \&\&}@ Invocable<F, @\newtxt{iter_}@reference_t<Is>...>
  struct indirect_result@\oldtxt{_of}@<F@\oldtxt{(}\newtxt{, }@Is...@\oldtxt{)}@> :
    @\oldtxt{result_of<F(reference_t<Is>\&\&...)>}\newtxt{invoke_result<F, iter_reference_t<Is>...>}@ { };
@\newtxt{\}}@
\end{codeblock}
...
\pnum
\begin{note}
The \tcode{\newtxt{ranges::}equal_to<>} and \tcode{\newtxt{ranges::}less<>}\iref{range.comparisons} function \newtxt{object} types used in the
concepts below impose \oldtxt{additional} constraints on their arguments \oldtxt{beyond}
\newtxt{in addition to} those that appear explicitly in the
concepts' bodies. \tcode{\newtxt{ranges::}equal_to<>} requires its arguments satisfy \libconcept{EqualityComparableWith}~(\cxxref{concept.equalitycomparable}),
and \tcode{\newtxt{ranges::}less<>} requires its arguments satisfy \libconcept{StrictTotallyOrderedWith}~(\cxxref{concept.stricttotallyordered}).
\end{note}
...
namespace std {
  ...
  public:
    ...
    @\newtxt{using iterator_category = \seebelow;}@
...
namespace std { @\oldtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  public:
    ...
    template<class I2, @\oldtxt{SizedSentinel<I2> I1,}@ @\oldtxt{SizedSentinel<I2> S1,}@ SizedSentinel<I1> S2>
      @\newtxt{requires SizedSentinel<I1, I2> \&\& SizedSentinel<S1, I2>}@
    friend difference_type_t<I2> operator-(
      const common_iterator@\oldtxt{<I1, S1>}@& x, const common_iterator<I2, S2>& y);

    friend @\newtxt{iter_}@rvalue_reference_t<I> iter_move(const common_iterator& i)
      noexcept(@\oldtxt{\seebelow}\newtxt{noexcept(ranges::iter_move(i.iter))}@)
        requires InputIterator<I>;
    template<IndirectlySwappable<I> I2, class S2>
      friend void iter_swap(const common_iterator& x, const common_iterator<I2, S2>& y)
        noexcept(@\oldtxt{\seebelow}\newtxt{noexcept(ranges::iter_swap(x.iter, y.iter))}@);
  ...
  struct iterator_@\oldtxt{category}\newtxt{traits}@<common_iterator<I, S>> {
    ...
    @\newtxt{using pointer = \seebelow;}@
    using @\oldtxt{type}\newtxt{iterator_category}@ = input_iterator_tag;
    @\newtxt{using iterator_concept = input_iterator_tag;}@
  };
  ...
  struct iterator_@\oldtxt{category}\newtxt{traits}@<common_iterator<I, S>> {
    ...
    @\newtxt{using reference = iter_reference_t<I>;}@
    @\newtxt{using pointer = \seebelow;}@
    using @\oldtxt{type}\newtxt{iterator_category}@ = forward_iterator_tag;
    @\newtxt{using iterator_concept = forward_iterator_tag;}@
  };
}@\oldtxt{\}\}\}}@
\end{codeblock}
...
\tcode{common_iterator<I, S>}, the nested \grammarterm{typedef-name}
\tcode{pointer} is defined as follows:
\begin{itemize}
...
\tcode{pointer} denotes the type of that expression.
\item Otherwise, \tcode{pointer} denotes \tcode{void}.
\end{itemize}
} %% \color{newclr}
...
with \tcode{\oldtxt{u}\newtxt{that}.is_sentinel}, \tcode{iter} with \tcode{\oldtxt{u}\newtxt{that}.iter}, and \tcode{sentinel}
with \tcode{\oldtxt{u}\newtxt{that}.sentinel}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Assigns \tcode{\oldtxt{u}\newtxt{that}.is_sentinel} to \tcode{is_sentinel}, \tcode{\oldtxt{u}\newtxt{that}.iter} to
\tcode{iter}, and \tcode{\oldtxt{u}\newtxt{that}.sentinel} to \tcode{sentinel}.
