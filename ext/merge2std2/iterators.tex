...
ranges::iter_swap(@\newtxt{std::move(--xtmp), std::move(--ytmp)}@ @\oldtxt{ranges::prev(x.current), ranges::prev(y.current)}@)
\end{codeblock}
...
\begin{codeblock}
  is_nothrow_copy_constructible_v<Iterator> &&
    @\oldtxt{noexcept(--declval<Iterator\&>()) \&\&}@
    noexcept(ranges::iter_swap(@\newtxt{std::move(--x.current), std::move(--y.current)}@ @\oldtxt{declval<Iterator>(), declval<Iterator>()}@))
\end{codeblock}
\end{itemdescr}
\end{addedblock}
