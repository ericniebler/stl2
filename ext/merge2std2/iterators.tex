...
\rSec1[range.iterators]{Iterators library}

\rSec2[range.iterators.general]{General}
...
as summarized in Table~\ref{tab:range.iterators.lib.summary}.

\begin{libsumtab}{Iterators library summary}{tab:range.iterators.lib.summary}
\ref{range.iterator.requirements} & Iterator requirements          & \tcode{<\oldtxt{experimental/ranges/iterator}\newtxt{range}>} \\
\ref{range.indirectcallable}      & Indirect callable requirements &                           \\
\ref{range.commonalgoreq}         & Common algorithm requirements  &                           \\
\ref{range.iterator.primitives}   & Iterator primitives            &                           \\
\ref{range.iterators.predef}      & Predefined iterators           &                           \\
\ref{range.iterators.stream}      & Stream iterators               &                           \\
\end{libsumtab}

\rSec2[range.iterator.requirements]{Iterator requirements}

\rSec3[range.iterator.requirements.general]{General}
...
as shown in Table~\ref{tab:range.iterators.relations}.

\begin{floattable}{Relations among iterator categories}{tab:range.iterators.relations}
{llll}
\topline
...
\tcode{Sentinel}~(\ref{range.iterators.sentinel}).
A range \range{i}{s}
...
\rSec3[range.iterator.custpoints]{Customization points}

\rSec4[range.iterator.custpoints.iter_move]{\tcode{iter_move}}
...
\rSec4[range.iterator.custpoints.iter_swap]{\tcode{iter_swap}}
...
\begin{codeblock}
is_nothrow_constructible@\newtxt{_v}@<value_type_t<T1>, rvalue_reference_t<T1>>@\oldtxt{::value}@ &&
is_nothrow_assignable@\newtxt{_v}@<value_type_t<T1>&, rvalue_reference_t<T1>>@\oldtxt{::value}@ &&
is_nothrow_assignable@\newtxt{_v}@<reference_t<T1>, rvalue_reference_t<T2>>@\oldtxt{::value}@ &&
is_nothrow_assignable@\newtxt{_v}@<reference_t<T1>, value_type_t<T2>>@\oldtxt{::value}@> &&
is_nothrow_move_constructible@\newtxt{_v}@<value_type_t<T1>>@\oldtxt{::value}@ &&
noexcept(ranges::iter_move(declval<T1&>()))
\end{codeblock}
\end{itemdescr}

\rSec3[range.iterator.assoc.types]{Iterator associated types}
...
satisfies the \tcode{WeaklyIncrementable} concept~(\ref{range.iterators.weaklyincrementable}),
\tcode{R} is the name of a type that
satisfies the \tcode{Readable} concept~(\ref{range.iterators.readable}), and
\tcode{II} is the name of a type that satisfies the
\tcode{InputIterator} concept~(\ref{range.iterators.input}) concept, the types
...
\rSec4[range.iterator.assoc.types.difference_type]{\tcode{difference_type}}
...
\begin{codeblock}
  ...
  struct difference_type<T*>
    : enable_if<is_object@\newtxt{_v}@<T>@\oldtxt{::value}@, ptrdiff_t> { };
...
\rSec4[range.iterator.assoc.types.value_type]{\tcode{value_type}}
...
\begin{codeblock}
  ...
  template <class T>
  struct value_type<T*>
    : enable_if<is_object@\newtxt{_v}@<T>@\oldtxt{::value}@, remove_cv_t<T>> { };
  ...
  template <class I>
  struct value_type<const I> : value_type<@\oldtxt{decay}\newtxt{remove_const}@_t<I>> { };

  template <class T>
    requires requires { typename T::value_type; }
  struct value_type<T>
    : enable_if<is_object@\newtxt{_v}@<typename T::value_type>@\oldtxt{::value}@, typename T::value_type> { };

  template <class T>
    requires requires { typename T::element_type; }
  struct value_type<T>
    : enable_if<
        is_object@\newtxt{_v}@<typename T::element_type>@\oldtxt{::value}@,
...
\rSec4[range.iterator.assoc.types.iterator_category]{\tcode{iterator_category}}
...
\begin{codeblock}
  ...
  struct iterator_category<T*>
    : enable_if<is_object@\newtxt{_v}@<T>@\oldtxt{::value}@, random_access_iterator_tag> { };
...
\rSec3[range.iterators.readable]{Concept \tcode{Readable}}
...
{\color{newclr}
\pnum
Given a value \tcode{i} of type \tcode{I}, \tcode{Readable<I>} is satisfied only
if the expression \tcode{*i} (which is indirectly required to be valid via the
exposition-only \placeholder{dereferenceable} concept~(\ref{range.synopsis})) is
equality-preserving.
} % \color{newclr}

\rSec3[range.iterators.writable]{Concept \tcode{Writable}}
...
\rSec3[range.iterators.weaklyincrementable]{Concept \tcode{WeaklyIncrementable}}
...
\rSec3[range.iterators.incrementable]{Concept \tcode{Incrementable}}
...
\rSec3[range.iterators.iterator]{Concept \tcode{Iterator}}
...
to compare iterators with sentinels~(\ref{range.iterators.sentinel}), to
read~(\ref{range.iterators.input}) or write~(\ref{range.iterators.output}) values, or
to provide a richer set of iterator movements~(\ref{range.iterators.forward},
\ref{range.iterators.bidirectional}, \ref{range.iterators.random.access}).)
...
\rSec3[range.iterators.sentinel]{Concept \tcode{Sentinel}}
\pnum
...
\begin{itemdecl}
  ...
  concept @\oldtxt{bool}@ Sentinel =
    Semiregular<S> &&
    Iterator<I> &&
    @\xname{WeaklyEqualityComparableWith}@<S, I>;
\end{itemdecl}
...
\rSec3[range.iterators.sizedsentinel]{Concept \tcode{SizedSentinel}}
\pnum
...
\tcode{true}~(\cxxref{structure.requirements}). \exitnote

\pnum
\enternote The \tcode{SizedSentinel}
concept is satisfied by pairs of
\tcode{RandomAccessIterator}s~(\ref{range.iterators.random.access}) and by
counted iterators and their sentinels~(\ref{range.counted.iterator}).\exitnote

\rSec3[range.iterators.input]{Concept \tcode{InputIterator}}

\pnum
The \tcode{InputIterator} concept is a refinement of
\tcode{Iterator}~(\ref{range.iterators.iterator}). It
defines requirements for a type whose referenced values can be read (from the requirement for
\tcode{Readable}~(\ref{range.iterators.readable})) and which can be both pre- and post-incremented.
\enternote
\oldtxt{Unlike in ISO/IEC 14882, input iterators are not required to satisfy
\tcode{EqualityComparable}~(\cxxref{concepts.lib.compare.equalitycomparable}).}
\newtxt{Unlike the input iterator requirements in \cxxref{iterators.input},
...
\rSec3[range.iterators.output]{Concept \tcode{OutputIterator}}

\pnum
The \tcode{OutputIterator} concept is a refinement of
\tcode{Iterator}~(\ref{range.iterators.iterator}). It defines requirements for a type that
can be used to write values (from the requirement for
\tcode{Writable}~(\ref{range.iterators.writable})) and which can be both pre- and post-incremented.
...
\rSec3[range.iterators.forward]{Concept \tcode{ForwardIterator}}

\pnum
The \tcode{ForwardIterator} concept refines \tcode{InputIterator}~(\ref{range.iterators.input}),
...
\rSec3[range.iterators.bidirectional]{Concept \tcode{BidirectionalIterator}}

\pnum
The \tcode{BidirectionalIterator} concept refines \tcode{ForwardIterator}~(\ref{range.iterators.forward}),
...
\rSec3[range.iterators.random.access]{Concept \tcode{RandomAccessIterator}}

\pnum
The \tcode{RandomAccessIterator} concept refines \tcode{BidirectionalIterator}~(\ref{range.iterators.bidirectional})
...
\rSec2[range.indirectcallable]{Indirect callable requirements}
...
\begin{codeblock}
  ...
  concept @\oldtxt{bool}@ IndirectUnaryInvocable =
    ...
    CommonReference<
      @\oldtxt{result_of_t<F\&(value_type_t<I>\&)>}\newtxt{invoke_result_t<F\&, value_type_t<I>\&>}@,
      @\oldtxt{result_of_t<F\&(reference_t<I>\&\&)>}\newtxt{invoke_result_t<F\&, reference_t<I>{}>}@>;
...
\rSec3[range.projected]{Class template \tcode{projected}}
...
\rSec2[range.commonalgoreq]{Common algorithm requirements}

\rSec3[range.commonalgoreq.general]{General}
...
\pnum
\enternote The \tcode{equal_to<>} and \tcode{less<>}~(\ref{range.comparisons}) function types used in the
...
\rSec3[range.commonalgoreq.indirectlymovable]{Concept \tcode{IndirectlyMovable}}
...
\begin{codeblock}
value_type_t<In> obj(ranges::iter_move(i));
\end{codeblock}
\tcode{obj} is equal to the value previously denoted by \tcode{*i}. If
\tcode{rvalue_reference_t<In>} is an rvalue reference type, the resulting state
of the value denoted by \tcode{*i} is valid but
unspecified~(\cxxref{lib.types.movedfrom}).
} %

\rSec3[range.commonalgoreq.indirectlycopyable]{Concept \tcode{IndirectlyCopyable}}
...
initialization of the object \tcode{obj} in
\begin{codeblock}
value_type_t<In> obj(*i);
\end{codeblock}
\tcode{obj} is equal to the value previously denoted by \tcode{*i}. If
\tcode{reference_t<In>} is an rvalue reference type, the resulting state
of the value denoted by \tcode{*i} is valid but
unspecified~(\cxxref{lib.types.movedfrom}).
} %

\rSec3[range.commonalgoreq.indirectlyswappable]{Concept \tcode{IndirectlySwappable}}
...
\rSec3[range.commonalgoreq.indirectlycomparable]{Concept \tcode{IndirectlyComparable}}
...
\rSec3[range.commonalgoreq.permutable]{Concept \tcode{Permutable}}
...
\rSec3[range.commonalgoreq.mergeable]{Concept \tcode{Mergeable}}
...
\rSec3[range.commonalgoreq.sortable]{Concept \tcode{Sortable}}
...
\rSec2[range.iterator.primitives]{Iterator primitives}

\pnum
To simplify the task of defining iterators, the library provides
several classes and functions:

\rSec3[range.iterator.traits]{Iterator traits}
...
\rSec3[range.iterator.stdtraits]{Standard iterator traits}

\pnum
To facilitate interoperability between new code using iterators conforming to
\oldtxt{this document}\newtxt{the concepts defined in this clause}
and older code using iterators that conform to the iterator
requirements specified in \oldtxt{ISO/IEC 14882} \newtxt{\cxxref{iterator.requirements}},
three specializations of \tcode{std::iterator_traits}
...
namespace std {
  ...
  struct iterator_traits<Out> {
    using difference_type   = @\oldtxt{experimental::}@ranges::difference_type_t<Out>;
...
\begin{codeblock}
  ...
  struct iterator_traits<In> {
    using difference_type   = @\oldtxt{experimental::}@ranges::difference_type_t<In>;
    using value_type        = @\oldtxt{experimental::}@ranges::value_type_t<In>;
    using reference         = @\seebelow@;
    using pointer           = @\seebelow@;
...
\item Otherwise, \tcode{std::iterator_traits<In>::reference} is
      \tcode{\oldtxt{experimental::}}\-\tcode{ranges::\-reference_t<In>}.
\end{itemize}
...
\item Otherwise, \tcode{std::iterator_traits<In>::pointer} is
      \tcode{\oldtxt{experimental::}ranges::\brk{}iterator_\-traits<\brk{}In\brk{}>::\brk{}pointer}.
\end{itemize}

\pnum
Let type \tcode{C} be \tcode{\oldtxt{experimental::}ranges::iterator_category_t<In>}.
...
\item If \tcode{C} is the same as or inherits from \tcode{std::input_iterator_tag} or
      ...
      is \tcode{C}.
\item Otherwise, if \tcode{\oldtxt{experimental::}ranges::reference_t<In>} is not a reference type,
      \tcode{std::\brk{}iterator_traits<In>::\brk{}iterator_category} is \tcode{std::input_iterator_tag}.
\item Otherwise, if \tcode{C} is the same as or inherits from \tcode{\oldtxt{experimental::}ranges::\brk{}random_access_iter\-at\-or_tag},
      \tcode{std::\brk{}iterator_traits<In>::\brk{}iterator_category} is \tcode{std::random_access_iterator_tag}.
\item Otherwise, if \tcode{C} is the same as or inherits from \tcode{\oldtxt{experimental::}ranges::\brk{}bidirectional_iter\-at\-or_tag},
      \tcode{std::\brk{}iterator_traits<In>::\brk{}iterator_category} is \tcode{std::bidirectional_iterator_tag}.
\item Otherwise, if \tcode{C} is the same as or inherits from \tcode{\oldtxt{experimental::}ranges::\brk{}forward_iter\-at\-or_tag},
...
\pnum
\enternote
Some implementations may find it necessary to add additional constraints to
these partial specializations to prevent them from being considered for types that
conform to the iterator requirements specified in \oldtxt{ISO/IEC 14882}
\newtxt{\cxxref{iterator.requirements}}.
\exitnote

\rSec3[range.iterator.tags]{Standard iterator tags}
...
\rSec3[range.iterator.operations]{Iterator operations}
...
operator, the library provides \oldtxt{customization point objects~(\cxxref{customization.point.object})}
...
\begin{itemdecl}
template <Iterator I>
  constexpr void @\oldtxt{operator()}\newtxt{advance}@(I& i, difference_type_t<I> n) @\oldtxt{const}@;
\end{itemdecl}
...
\begin{itemdecl}
template <Iterator I, Sentinel<I> S>
  constexpr difference_type_t<I> @\oldtxt{operator()}\newtxt{advance}@(I& i, difference_type_t<I> n, S bound) @\oldtxt{const}@;
\end{itemdecl}
...
\begin{itemdecl}
template <Range R>
  constexpr difference_type_t<iterator_t<R>> @\oldtxt{operator()}\newtxt{distance}@(R&& r) @\oldtxt{const}@;
\end{itemdecl}
...
\pnum
\effects
Equivalent to: \tcode{return ranges::size(r);}~(\ref{range.primitives.size})
...
\begin{itemdecl}
template <Iterator I>
  constexpr I @\oldtxt{operator()}\newtxt{next}@(I x, difference_type_t<I> n) @\oldtxt{const}@;
\end{itemdecl}
...
\begin{itemdecl}
template <Iterator I, Sentinel<I> S>
  constexpr I @\oldtxt{operator()}\newtxt{next}@(I x, difference_type_t<I> n, S bound) @\oldtxt{const}@;
\end{itemdecl}
...
\begin{itemdecl}
template <BidirectionalIterator I>
  constexpr I @\oldtxt{operator()}\newtxt{prev}@(I x, difference_type_t<I> n) @\oldtxt{const}@;
\end{itemdecl}
...
\begin{itemdecl}
template <BidirectionalIterator I>
  constexpr I @\oldtxt{operator()}\newtxt{prev}@(I x, difference_type_t<I> n, I bound) @\oldtxt{const}@;
\end{itemdecl}
...
\rSec2[range.iterators.predef]{Iterator adaptors}

\rSec3[range.iterators.reverse]{Reverse iterators}
...
\rSec4[range.reverse.iterator]{Class template \tcode{reverse_iterator}}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  public:
    ...
    @\newtxt{template <ConvertibleTo<I> U>}@
      constexpr reverse_iterator(const reverse_iterator<@\oldtxt{ConvertibleTo<I>}\newtxt{U}@>& i);
    @\newtxt{template <ConvertibleTo<I> U>}@
      constexpr reverse_iterator& operator=(const reverse_iterator<@\oldtxt{ConvertibleTo<I>}\newtxt{U}@>& i);
...
\rSec4[range.reverse.iter.ops]{\tcode{reverse_iterator} operations}

\rSec5[range.reverse.iter.cons]{\tcode{reverse_iterator} constructor}
...
\rSec5[range.reverse.iter.op=]{\tcode{reverse_iterator::operator=}}

\indexlibrary{\idxcode{operator=}!\tcode{reverse_iterator}}%
\begin{itemdecl}
@\newtxt{template<ConvertibleTo<I> U>}@
  constexpr reverse_iterator&
    operator=(const reverse_iterator<@\oldtxt{ConvertibleTo<I>}\newtxt{U}@>& i);
\end{itemdecl}
...
\rSec5[range.reverse.iter.conv]{Conversion}
...
\rSec5[range.reverse.iter.op.star]{\tcode{operator*}}
...
\rSec5[range.reverse.iter.opref]{\tcode{operator->}}
...
\rSec5[range.reverse.iter.op++]{\tcode{operator++}}
...
\rSec5[range.reverse.iter.op\dcr]{\tcode{operator\dcr}}
...
\rSec5[range.reverse.iter.opindex]{\tcode{operator[]}}
...
\rSec5[range.reverse.iter.opdiff]{\tcode{operator-}}
...
\rSec5[range.reverse.iter.opsum]{\tcode{operator+}}
...
\rSec5[range.reverse.iter.iter_move]{\tcode{iter_move}}
...
\rSec5[range.reverse.iter.iter_swap]{\tcode{iter_swap}}
...
\rSec5[range.reverse.iter.make]{Non-member function \tcode{make_reverse_iterator()}}
...
\rSec3[range.iterators.insert]{Insert iterators}
...
\rSec4[range.back.insert.iterator]{Class template \tcode{back_insert_iterator}}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  public:
    ...
    back_insert_iterator&
      operator=(const @\oldtxt{value_type_t<Container>}\newtxt{typename Container::value_type}@& value);
    back_insert_iterator&
      operator=(@\oldtxt{value_type_t<Container>}\newtxt{typename Container::value_type}@&& value);
...
\rSec4[range.back.insert.iter.ops]{\tcode{back_insert_iterator} operations}

\rSec5[range.back.insert.iter.cons]{\tcode{back_insert_iterator} constructor}
...
\rSec5[range.back.insert.iter.op=]{\tcode{back_insert_iterator::operator=}}
...
\rSec5[range.back.inserter]{ \tcode{back_inserter}}
...
\rSec4[range.front.insert.iterator]{Class template \tcode{front_insert_iterator}}
...
\rSec4[range.front.insert.iter.ops]{\tcode{front_insert_iterator} operations}

\rSec5[range.front.insert.iter.cons]{\tcode{front_insert_iterator} constructor}
...
\rSec5[range.front.insert.iter.op=]{\tcode{front_insert_iterator::operator=}}
...
\rSec5[range.front.inserter]{\tcode{front_inserter}}
...
\rSec4[range.insert.iterator]{Class template \tcode{insert_iterator}}
...
\rSec4[range.insert.iter.ops]{\tcode{insert_iterator} operations}

\rSec5[range.insert.iter.cons]{\tcode{insert_iterator} constructor}
...
\rSec5[range.insert.iter.op=]{\tcode{insert_iterator::operator=}}
...
\rSec5[range.inserter]{\tcode{inserter}}
...
\rSec3[range.iterators.move]{Move iterators and sentinels}

\rSec4[range.move.iterator]{Class template \tcode{move_iterator}}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  public:
    ...
    @\newtxt{template <ConvertibleTo<I> U>}@
      constexpr move_iterator(const move_iterator<@\oldtxt{ConvertibleTo<I>}\newtxt{U}@>& i);
    @\newtxt{template <ConvertibleTo<I> U>}@
      constexpr move_iterator& operator=(const move_iterator<@\oldtxt{ConvertibleTo<I>}\newtxt{U}@>& i);
...
\rSec4[range.move.iter.ops]{\tcode{move_iterator} operations}

\rSec5[range.move.iter.op.const]{\tcode{move_iterator} constructors}
...
\rSec5[range.move.iter.op=]{\tcode{move_iterator::operator=}}
...
\rSec5[range.move.iter.op.conv]{\tcode{move_iterator} conversion}
...
\rSec5[range.move.iter.op.star]{\tcode{move_iterator::operator*}}
...
\rSec5[range.move.iter.op.incr]{\tcode{move_iterator::operator++}}
...
\rSec5[range.move.iter.op.decr]{\tcode{move_iterator::operator-{-}}}
...
\rSec5[range.move.iter.op.index]{\tcode{move_iterator::operator[]}}
...
\rSec5[range.move.iter.op.comp]{\tcode{move_iterator} comparisons}
...
\rSec5[range.move.iter.nonmember]{\tcode{move_iterator} non-member functions}
...
\rSec4[range.move.sentinel]{Class template \tcode{move_sentinel}}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  public:
    ...
    @\newtxt{template <ConvertibleTo<S> U>}@
      move_sentinel(const move_sentinel<@\oldtxt{ConvertibleTo<S>}\newtxt{U}@>& s);
    @\newtxt{template <ConvertibleTo<S> U>}@
      move_sentinel& operator=(const move_sentinel<@\oldtxt{ConvertibleTo<S>}\newtxt{U}@>& s);
...
\rSec4[range.move.sent.ops]{\tcode{move_sentinel} operations}

\rSec5[range.move.sent.op.const]{\tcode{move_sentinel} constructors}
...
\tcode{last}. If \tcode{is_\-trivially_\-default_\-constr\-uct\-ible\-\newtxt{_v}<\brk{}S>\oldtxt{::value}} is \tcode{true}, then this constructor
...
\rSec5[range.move.sent.op=]{\tcode{move_sentinel::operator=}}
...
\rSec5[range.move.sent.op.comp]{\tcode{move_sentinel} comparisons}
...
\rSec5[range.move.sent.nonmember]{\tcode{move_sentinel} non-member functions}
...
\rSec3[range.iterators.common]{Common iterators}
...
\rSec4[range.common.iterator]{Class template \tcode{common_iterator}}
...
namespace std@\newtxt{::ranges}@ { @\oldtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  public:
    ...
    @\newtxt{template <ConvertibleTo<I> II, ConvertibleTo<S> SS>}@
      @\newtxt{constexpr common_iterator(const common_iterator<II, SS>\& u);}@
    @\oldtxt{common_iterator\& operator=(const common_iterator<ConvertibleTo<I>, ConvertibleTo<S>>\& u);}@
    @\newtxt{template <ConvertibleTo<I> II, ConvertibleTo<S> SS>}@
      @\newtxt{common_iterator\& operator=(const common_iterator<II, SS>\& u);}@
...
\rSec4[range.common.iter.ops]{\tcode{common_iterator} operations}

\rSec5[range.common.iter.op.const]{\tcode{common_iterator} constructors}
...
\rSec5[range.common.iter.op=]{\tcode{common_iterator::operator=}}
...
\rSec5[range.common.iter.op.star]{\tcode{common_iterator::operator*}}
...
\rSec5[range.common.iter.op.ref]{\tcode{common_iterator::operator->}}
...
Readable<const I> &&
  (requires(const I& i) { i.operator->(); } ||
   is_reference@\newtxt{_v}@<reference_t<I>>@\oldtxt{::value}@ ||
   Constructible<value_type_t<I>, reference_t<I>>)
\end{codeblock}
\end{itemdescr}


\rSec5[range.common.iter.op.incr]{\tcode{common_iterator::operator++}}
...
\rSec5[range.common.iter.op.comp]{\tcode{common_iterator} comparisons}
...
\rSec5[range.common.iter.op.iter_move]{\tcode{iter_move}}
...
\rSec5[range.common.iter.op.iter_swap]{\tcode{iter_swap}}
...
\rSec3[range.default.sentinels]{Default sentinels}

\rSec4[range.default.sent]{Class \tcode{default_sentinel}}
...
of their range (e.g., \tcode{counted_iterator}~(\ref{range.counted.iterator})).

\rSec3[range.iterators.counted]{Counted iterators}

\rSec4[range.counted.iterator]{Class template \tcode{counted_iterator}}
...
\rSec4[range.counted.iter.ops]{\tcode{counted_iterator} operations}

\rSec5[range.counted.iter.op.const]{\tcode{counted_iterator} constructors}
...
\rSec5[range.counted.iter.op=]{\tcode{counted_iterator::operator=}}
...
\rSec5[range.counted.iter.op.conv]{\tcode{counted_iterator} conversion}
...
\rSec5[range.counted.iter.op.cnt]{\tcode{counted_iterator} count}
...
\rSec5[range.counted.iter.op.star]{\tcode{counted_iterator::operator*}}
...
\rSec5[range.counted.iter.op.incr]{\tcode{counted_iterator::operator++}}
...
\rSec5[range.counted.iter.op.decr]{\tcode{counted_iterator::operator-{-}}}
...
\rSec5[range.counted.iter.op.index]{\tcode{counted_iterator::operator[]}}
...
\rSec5[range.counted.iter.op.comp]{\tcode{counted_iterator} comparisons}
...
sequence~(\ref{range.iterators.counted}).
...
elements of the same sequence~(\ref{range.iterators.counted}).
...
\rSec5[range.counted.iter.nonmember]{\tcode{counted_iterator} non-member functions}
...
\rSec3[range.unreachable.sentinels]{Unreachable sentinel}

\rSec4[range.unreachable.sentinel]{Class \tcode{unreachable}}
...
\rSec4[range.unreachable.sentinel.ops]{\tcode{unreachable} operations}

\rSec5[range.unreachable.sentinel.op==]{\tcode{operator==}}
...
\rSec2[range.iterators.stream]{Stream iterators}
...
\rSec3[range.istream.iterator]{Class template \tcode{istream_iterator}}

\pnum
\indexlibrary{\idxcode{istream_iterator}}%
The class template
\tcode{istream_iterator}
is an input iterator~(\ref{range.iterators.input}) that
reads (using
\tcode{operator\shr})
...
\rSec4[range.istream.iterator.cons]{\tcode{istream_iterator} constructors and destructor}
...
\rSec4[range.istream.iterator.ops]{\tcode{istream_iterator} operations}
...
\rSec3[range.ostream.iterator]{Class template \tcode{ostream_iterator}}
...
\rSec4[range.ostream.iterator.cons.des]{\tcode{ostream_iterator} constructors and destructor}
...
\rSec4[range.ostream.iterator.ops]{\tcode{ostream_iterator} operations}
...
\rSec3[range.istreambuf.iterator]{Class template \tcode{istreambuf_iterator}}

\pnum
The
class template
\tcode{istreambuf_iterator}
defines an input iterator~(\ref{range.iterators.input}) that
reads successive
\textit{characters}
...
\rSec4[range.istreambuf.iterator::proxy]{Class template \tcode{istreambuf_iterator::proxy}}
...
\rSec4[range.istreambuf.iterator.cons]{\tcode{istreambuf_iterator} constructors}
...
\rSec4[range.istreambuf.iterator::op*]{\tcode{istreambuf_iterator::operator*}}
...
\rSec4[range.istreambuf.iterator::equal]{\tcode{istreambuf_iterator::equal}}
...
\rSec4[range.istreambuf.iterator::op==]{\tcode{operator==}}
...
\rSec3[range.ostreambuf.iterator]{Class template \tcode{ostreambuf_iterator}}
...
\rSec4[range.ostreambuf.iter.cons]{\tcode{ostreambuf_iterator} constructors}
...
\rSec4[range.ostreambuf.iter.ops]{\tcode{ostreambuf_iterator} operations}
