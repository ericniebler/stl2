...
the result of incrementing \tcode{i} \tcode{n} times. A counted range
...
\pnum
If \tcode{iterator_traits<I>} names an instantiation of the primary template,
then \tcode{iter_difference_t<I>} denotes
\tcode{incrementable_traits<I>::difference_type};
otherwise, it denotes
\tcode{iterator_traits<I>::difference_type}.
...
\begin{addedblock}
\pnum
Specializations of \tcode{iterator_traits}
that are not instantiations of the primary template may
...
\pnum
\begin{note}
\tcode{disable_sized_sentinel} enables use of sentinels and iterators with
...
the \libconcept{InputIterator} concept need not require
...
It bundles a \libconcept{Readable} type \tcode{I} and
...
\pnum
\begin{note}
The \tcode{ranges::less<>}\iref{range.comparisons} function object type
...
function call operator of \tcode{ranges::less<>} requires its arguments to model
...
\tcode{I1} and \tcode{I2} model \tcode{IndirectlySwappable<I1, I2>}
only if after either
\tcode{ranges::iter_swap(i1, i2)}
or
\tcode{ranges::iter_swap(i2, i1)},
the value of \tcode{*i1} is equal to the value of \tcode{*i2} before the call,
...
may be used to opt in or out of conformance to
...
\item Otherwise,
  \begin{itemize}
  \item if \tcode{n} is non-negative, increments \tcode{i}
    either \tcode{n} times or until
    \tcode{bool(i == bound)} is \tcode{true},
    whichever comes first.
  \item Otherwise, decrements \tcode{i} by \tcode{-n}
    or until \tcode{bool(i == bound)} is \tcode{true}.
  \end{itemize}
\end{itemize}
...
It can be used together with class \tcode{default_sentinel}
...
\rSec3[counted.iter.const]{Constructors}
