...
\begin{itemize}
\item If \tcode{a} and \tcode{b} are decrementable, then
  \begin{itemize}
  \item \tcode{addressof(--a) == addressof(a)},
  \item \tcode{bool(a-- == b)},
  \item after evaluating both \tcode{a--} and \tcode{--b},
    \tcode{bool(a == b)} is still \tcode{true}, and
  \item \tcode{bool(++(--a) == b)}.
  \end{itemize}
...
\begin{codeblock}
  is_nothrow_copy_constructible_v<Iterator> &&
    noexcept(ranges::iter_swap(std::move(--declval<Iterator&>()), declval<Iterator>()))
\end{codeblock}
\end{itemdescr}
\end{addedblock}
...
Class \tcode{unreachable} can be used with
