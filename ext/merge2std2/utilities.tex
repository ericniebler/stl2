...
\begin{codeblock}
  namespace ranges {
    ...
    template<InputRange IRng, @\placeholdernc{no-throw-forward-range}@ ORng>
      ...
      @\newtxt{uninitialized_copy_result<safe_iterator_t<IRng>, safe_iterator_t<ORng>>}@
...
\pnum
\begin{note}
\oldtxt{The distinction between \libconcept{InputIterator} and
...
\pnum
\begin{note}
\oldtxt{The distinction between \libconcept{InputRange} and
...
\pnum
\begin{note}
\oldtxt{The distinction between \libconcept{ForwardIterator} and
...
\pnum
\begin{note}
\oldtxt{The distinction between \libconcept{ForwardRange} and
...
\ednote{\tcode{destroy} in the International Standard requires a
\oldconcept{ForwardIterator} to ensure that the iterator's \tcode{reference}
type is a true reference type. This requirement is relaxed for
\tcode{ranges::destroy}, since \tcode{\placeholder{no-throw-input-iterator}}
requires that \tcode{iter_reference_t<I>} is an lvalue reference type.\\
\\
...
\pnum
In this subclause, \tcode{\textit{BUILTIN_PTR_CMP}(T, $op$, U)} for types \tcode{T}
