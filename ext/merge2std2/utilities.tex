...
\begin{codeblock}
  namespace ranges {
    // \ref{range.swap}, \tcode{ranges::swap}
...
\tcode{ranges::swap(\brk{}E1, E2)} for some subexpressions \tcode{E1}
...
\item
  ...
  is equal to \tcode{is_nothrow_move_constructible_v<T>
...
\begin{codeblock}
  namespace ranges {
    ...
    template<InputRange IRng, @\placeholdernc{no-throw-forward-range}@ ORng>
        requires Constructible<iter_value_t<iterator_t<ORng>>, iter_reference_t<iterator_t<IRng>>>
...
\begin{codeblock}
  namespace ranges {
    ...
    template<InputRange IRng, @\placeholdernc{no-throw-forward-range}@ ORng>
        requires Constructible<iter_value_t<iterator_t<ORng>>, iter_rvalue_reference_t<iterator_t<IRng>>>
...
\begin{codeblock}
  namespace ranges {
    ...
    template<@\placeholdernc{no-throw-forward-range}@ Rng, class T>
        requires Constructible<iter_value_t<iterator_t<Rng>>, const T&>
...
\begin{codeblock}
  namespace ranges {
    ...
    template<@\placeholdernc{no-throw-input-range}@ Rng>
        requires Destructible<iter_value_t<iterator_t<Rng>>
...
\pnum
\removed{Throughout this subclause,} \added{For the algorithms in this subclause
...
subclause\iref{specialized.algorithms}) on ranges of
...
\range{first}{\changed{std::next(first,n)}{first + n}}
are left in a valid but unspecified state.
\end{itemdescr}
