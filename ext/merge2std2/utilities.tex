...
\rSec2[std2.utility.swap]{\tcode{swap}}

\indexlibrary{\idxcode{swap}}%
\pnum The name \tcode{swap} denotes a customization point
object~(\ref{std2.customization.point.object}). The effect of the expression
\tcode{\changed{ranges}{std2}\colcol{}swap(\brk{}E1, E2)} for some expressions \tcode{E1}
...
\end{codeblock}
  and does not include a declaration of \tcode{\changed{ranges}{std2}::swap}.
...
\item
  ...
  of equal extent and \tcode{\changed{ranges}{std2}::swap(*(E1), *(E2))}
  is a valid expression, except that
  \tcode{noexcept(\brk{}\changed{ranges}{std2}::swap(E1, E2))} is equal to
  \tcode{noexcept(\brk{}\changed{ranges}{std2}::swap(*(E1), *(E2)))}.
...
\item
  ...
  \tcode{\changed{ranges}{std2}\colcol{}swap(\brk{}E1, E2)} is a constant expression if
  ...
  constexpr function. \tcode{noexcept(\changed{ranges}{std2}\colcol{}swap(\brk{}E1, E2))}
...
\item
  Otherwise, \tcode{\changed{ranges}{std2}::swap(E1, E2)} is ill-formed.
\end{itemize}

\pnum
\remark Whenever \tcode{\changed{ranges}{std2}::swap(E1, E2)} is a valid
...
\rSec2[std2.func.invoke]{Function template \tcode{invoke}}
\begin{itemdecl}
template <class F, class... Args>
result_of_t<F&&(Args&&...)> invoke(F&& f, Args&&... args);
\end{itemdecl}
...
\rSec1[std2.meta]{Metaprogramming and type traits}

\rSec2[std2.meta.type.synop]{Header \tcode{<experimental/ranges/type_traits>} synopsis}
\begin{codeblock}
namespace std { namespace experimental { namespace ranges { inline namespace v1 {
  // \ref{std2.meta.unary.prop}, type properties:
  ...
  // \ref{std2.meta.trans.other}, other transformations:
...
\rSec2[std2.meta.unary.prop]{Type properties}
...
\rSec2[std2.meta.trans.other]{Other transformations}
