...
\end{codeblock}\begin{addedblock}\begin{codeblock}
  namespace ranges {
    @\newtxt{template<class I1, class I2>}@
    @\newtxt{using copy_backward_result = copy_result<I, I2>;}@
...
\end{codeblock}\begin{addedblock}\begin{codeblock}
  namespace ranges {
    @\newtxt{template<class I1, class O>}@
...
Let $E$ be:
\begin{itemize}
\item \tcode{pred(*i)} for the overloads in namespace \tcode{std}, or
\item \tcode{invoke(pred, invoke(proj, *i))} for the overloads in namespace \tcode{ranges}.
\end{itemize}
} %% \color{newclr}
\begin{addedblock}
...
\tcode{n < (last-first)},
performs
\tcode{*(result + n) = std::move(*(first + n))}.
...
shall not overlap.
\tcode{*(first1 + n)} shall be swappable with\cxxref{swappable.requirements}
\tcode{*(first2 + n)}.
...
\range{first2}{last2}
shall not overlap.
\oldtxt{\tcode{*(first1 + n)} shall be swappable with\iref{concept.swappable}
\tcode{*(first2 + n)}.}
...
\tcode{\{first1 + $N$, result + $N$\}}
 or \tcode{\oldtxt{make_tagged_tuple<tag::in1, tag::in2, tag::out\brk{}>(}\newtxt{\{}\brk{}first1 + $N$, first2 + $N$, result + $N$\oldtxt{)}\newtxt{\}}}.
