...
\rSec2[range.algorithms.general]{General}

\pnum
This subclause describes components that \Cpp programs may use to perform
algorithmic operations on
\oldtxt{containers (Clause~\cxxref{containers}) and other sequences}
\newtxt{ranges}.
...
as summarized in Table~\ref{tab:range.algorithms.summary}.

\begin{libsumtab}{Algorithms library summary}{tab:range.algorithms.summary}
\ref{range.alg.nonmodifying}         & Non-modifying sequence operations & \tcode{\newtxt{<range>}} \\
\ref{range.alg.modifying.operations} & Mutating sequence operations      & \tcode{\oldtxt{<experimental/ranges/algorithm>}} \\
\ref{range.alg.sorting}              & Sorting and related operations    & \\ \hline
\end{libsumtab}

\pnum
\oldtxt{To ease transition, implementations provide additional algorithm signatures that
are deprecated in this document~(Annex~\tsref{depr.algo.range-and-a-half}).}
...
\rSec2[range.alg.nonmodifying]{Non-modifying sequence operations}

\rSec3[range.alg.all_of]{All of}
...
\rSec3[range.alg.none_of]{None of}
...
\rSec3[range.alg.foreach]{For each}

\indexlibrary{\idxcode{for_each}}%
\begin{itemdecl}
template <InputIterator I, Sentinel<I> S, class Proj = identity,
    IndirectUnaryInvocable<projected<I, Proj>> Fun>
  @\newtxt{constexpr}@ tagged_pair<tag::in(I), tag::fun(Fun)>
    for_each(I first, S last, Fun f, Proj proj = Proj{});

template <InputRange Rng, class Proj = identity,
    IndirectUnaryInvocable<projected<iterator_t<Rng>, Proj>> Fun>
  @\newtxt{constexpr}@ tagged_pair<tag::in(safe_iterator_t<Rng>), tag::fun(Fun)>
...
satisfy \textit{Cpp98CopyConstructible}, whereas the algorithm in the \Cpp Standard
requires only \textit{Cpp98MoveConstructible}. \exitnote
\end{itemdescr}

\rSec3[range.alg.find]{Find}
...
\rSec3[range.alg.find.end]{Find end}
...
\rSec3[range.alg.find.first.of]{Find first of}
...
\rSec3[range.alg.adjacent.find]{Adjacent find}
...
template <InputIterator I, Sentinel<I> S, class T, class Proj = identity>
  requires IndirectRelation<equal_to<>, projected<I, Proj>, const T*>
  @\newtxt{constexpr}@ difference_type_t<I>
    count(I first, S last, const T& value, Proj proj = Proj{});

template <InputRange Rng, class T, class Proj = identity>
  requires IndirectRelation<equal_to<>, projected<iterator_t<Rng>, Proj>, const T*>
  @\newtxt{constexpr}@ difference_type_t<iterator_t<Rng>>
...
template <InputIterator I1, Sentinel<I1> S1, InputIterator I2, Sentinel<I2> S2,
    class Proj1 = identity, class Proj2 = identity,
    IndirectRelation<projected<I1, Proj1>, projected<I2, Proj2>> Pred = equal_to<>>
  @\newtxt{constexpr}@ tagged_pair<tag::in1(I1), tag::in2(I2)>
...
template <InputRange Rng1, InputRange Rng2,
    ...
    IndirectRelation<projected<iterator_t<Rng1>, Proj1>,
      projected<iterator_t<Rng2>, Proj2>> Pred = equal_to<>>
  @\newtxt{constexpr}@ tagged_pair<tag::in1(safe_iterator_t<Rng1>), tag::in2(safe_iterator_t<Rng2>)>
...
\rSec3[range.alg.is_permutation]{Is permutation}
...
\rSec3[range.alg.copy]{Copy}

\indexlibrary{\idxcode{copy}}%
\begin{itemdecl}
template <InputIterator I, Sentinel<I> S, WeaklyIncrementable O>
  requires IndirectlyCopyable<I, O>
  @\newtxt{constexpr}@ tagged_pair<tag::in(I), tag::out(O)>
    copy(I first, S last, O result);

template <InputRange Rng, WeaklyIncrementable O>
  requires IndirectlyCopyable<iterator_t<Rng>, O>
  @\newtxt{constexpr}@ tagged_pair<tag::in(safe_iterator_t<Rng>), tag::out(O)>
...
template <BidirectionalIterator I1, Sentinel<I1> S1, BidirectionalIterator I2>
  ...
  @\newtxt{constexpr}@ tagged_pair<tag::in(I1), tag::out(I2)>
    copy_backward(I1 first, S1 last, I2 result);

template <BidirectionalRange Rng, BidirectionalIterator I>
  requires IndirectlyCopyable<iterator_t<Rng>, I>
  @\newtxt{constexpr}@ tagged_pair<tag::in(safe_iterator_t<Rng>), tag::out(I)>
...
\rSec3[range.alg.transform]{Transform}
...
template <InputIterator I1, Sentinel<I1> S1, InputIterator I2, Sentinel<I2> S2,
  ...
  @\newtxt{constexpr}@ tagged_tuple<tag::in1(I1), tag::in2(I2), tag::out(O)>
...
template <InputRange Rng1, InputRange Rng2, WeaklyIncrementable O,
  ...
  @\newtxt{constexpr}@ tagged_tuple<tag::in1(safe_iterator_t<Rng1>),
...
\rSec3[range.alg.replace]{Replace}
...
template <class T, OutputIterator<const T&> O>
  @\newtxt{constexpr}@ O fill_n(O first, difference_type_t<O> n, const T& value);
\end{itemdecl}
...
\rSec3[range.alg.generate]{Generate}
...
template <Iterator O, CopyConstructible F>
    requires Invocable<F&> && Writable<O, @\oldtxt{result_of_t<F\&()>}\newtxt{invoke_result_t<F\&>}@>
  @\newtxt{constexpr}@ O generate_n(O first, difference_type_t<O> n, F gen);
\end{itemdecl}
...
\rSec3[range.alg.reverse]{Reverse}
...
\rSec3[range.alg.random.shuffle]{Shuffle}
...
\rSec3[range.alg.partitions]{Partitions}
...
template <InputIterator I, Sentinel<I> S, WeaklyIncrementable O1, WeaklyIncrementable O2,
  ...
  @\newtxt{constexpr}@ tagged_tuple<tag::in(I), tag::out1(O1), tag::out2(O2)>
...
template <InputRange Rng, WeaklyIncrementable O1, WeaklyIncrementable O2,
  ...
  @\newtxt{constexpr}@ tagged_tuple<tag::in(safe_iterator_t<Rng>), tag::out1(O1), tag::out2(O2)>
...
\pnum
All the operations in~\ref{range.alg.sorting} take an optional binary callable
predicate of type \tcode{Comp} that defaults to \tcode{less<>}.
...
\rSec3[range.alg.sort]{Sorting}

\rSec4[range.sort]{\tcode{sort}}
...
\rSec4[range.stable.sort]{\tcode{stable_sort}}
...
\rSec4[range.partial.sort]{\tcode{partial_sort}}
...
\rSec4[range.partial.sort.copy]{\tcode{partial_sort_copy}}
...
\rSec4[range.is.sorted]{\tcode{is_sorted}}
...
\rSec3[range.alg.nth.element]{Nth element}
...
\rSec4[range.lower.bound]{\tcode{lower_bound}}
...
\rSec4[range.upper.bound]{\tcode{upper_bound}}
...
template <ForwardIterator I, Sentinel<I> S, class T, class Proj = identity,
    IndirectStrictWeakOrder<const T*, projected<I, Proj>> Comp = less<>>
  @\newtxt{constexpr}@ tagged_pair<tag::begin(I), tag::end(I)>
    equal_range(I first, S last, const T& value, Comp comp = Comp{}, Proj proj = Proj{});

template <ForwardRange Rng, class T, class Proj = identity,
    IndirectStrictWeakOrder<const T*, projected<iterator_t<Rng>, Proj>> Comp = less<>>
  @\newtxt{constexpr}@ tagged_pair<tag::begin(safe_iterator_t<Rng>),
...
\rSec4[range.binary.search]{\tcode{binary_search}}
...
\rSec3[range.alg.set.operations]{Set operations on sorted structures}
...
\rSec4[range.includes]{\tcode{includes}}
...
\rSec4[range.set.union]{\tcode{set_union}}
...
\rSec4[range.set.intersection]{\tcode{set_intersection}}
...
\rSec4[range.set.difference]{\tcode{set_difference}}
...
template <InputIterator I1, Sentinel<I1> S1, InputIterator I2, Sentinel<I2> S2,
    WeaklyIncrementable O, class Comp = less<>, class Proj1 = identity, class Proj2 = identity>
  requires Mergeable<I1, I2, O, Comp, Proj1, Proj2>
  @\newtxt{constexpr}@ tagged_pair<tag::in1(I1), tag::out(O)>
...
template <InputRange Rng1, InputRange Rng2, WeaklyIncrementable O,
    class Comp = less<>, class Proj1 = identity, class Proj2 = identity>
  requires Mergeable<iterator_t<Rng1>, iterator_t<Rng2>, O, Comp, Proj1, Proj2>
  @\newtxt{constexpr}@ tagged_pair<tag::in1(safe_iterator_t<Rng1>), tag::out(O)>
...
\rSec4[range.set.symmetric.difference]{\tcode{set_symmetric_difference}}
...
\rSec3[range.alg.heap.operations]{Heap operations}
...
\rSec4[range.push.heap]{\tcode{push_heap}}
...
\rSec4[range.pop.heap]{\tcode{pop_heap}}
...
\rSec4[range.make.heap]{\tcode{make_heap}}
...
\rSec4[range.sort.heap]{\tcode{sort_heap}}
...
\rSec4[range.is.heap]{\tcode{is_heap}}
...
\rSec3[range.alg.min.max]{Minimum and maximum}
...
template <InputRange Rng, class Proj = identity,
  ...
  @\newtxt{constexpr}@ value_type_t<iterator_t<Rng>>
...
template <InputRange Rng, class Proj = identity,
    IndirectStrictWeakOrder<projected<iterator_t<Rng>, Proj> Comp = less<>>
  requires Copyable<value_type_t<iterator_t<Rng>>>
  @\newtxt{constexpr}@ tagged_pair<tag::min(value_type_t<iterator_t<Rng>>),
...
template <ForwardIterator I, Sentinel<I> S, class Proj = identity,
    IndirectStrictWeakOrder<projected<I, Proj>> Comp = less<>>
  @\newtxt{constexpr}@ tagged_pair<tag::min(I), tag::max(I)>
    minmax_element(I first, S last, Comp comp = Comp{}, Proj proj = Proj{});

template <ForwardRange Rng, class Proj = identity,
    IndirectStrictWeakOrder<projected<iterator_t<Rng>, Proj>> Comp = less<>>
  @\newtxt{constexpr}@ tagged_pair<tag::min(safe_iterator_t<Rng>),
...
\rSec3[range.alg.lex.comparison]{Lexicographical comparison}
...
\rSec3[range.alg.permutation.generators]{Permutation generators}
