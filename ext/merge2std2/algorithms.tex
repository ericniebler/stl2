...
\begin{itemdescr}
\pnum
\added{Let $P$ be \tcode{pred(*i)} and \tcode{invoke(pred, invoke(proj, *i))}
for the overloads in namespace \tcode{std} and \tcode{std::ranges}, respectively.}
...
\added{\returns \tcode{false} if $P$ is \tcode{true} for some iterator \tcode{i}
...
\begin{addedblock}
\pnum
Let \tcode{last2} be \tcode{first2 + (last1 - first1)} for the overloads with no parameter \tcode{last2} or \tcode{rng2}.
...
\item \added{\tcode{\{last + $M$, result + $M$\}}
...
requirements~(\cxxref{swappable.requirements})\changed{. The}{, and the}
...
comparisons\added{, and exactly twice as many applications of any projection}.
\end{itemdescr}
...
\added{for the overloads in namespace \tcode{std}, or}
\item
\added{\tcode{bool(invoke(comp, invoke(proj, e), value))}
shall imply
\tcode{!invoke(comp, value, invoke(proj, e))}
for the overloads in namespace \tcode{ranges}}.
\end{itemize}
...
\pnum
\returns
\added{Let \tcode{R} be the return type of the overload in question. Returns}
\tcode{\removed{pair<const T\&, const T\&>(}\added{R\{}b, a\added{\}}\removed{)}} if \tcode{b} is smaller
than \tcode{a}, and
\tcode{\removed{pair<const T\&, const T\&>(}\added{R\{}a, b\added{\}}\removed{)}} otherwise.
...
\pnum
\returns \added{Let \tcode{R} be the return type of the overload in question.
Returns}
\tcode{\removed{pair<T, T>(}\added{R\{}x, y\added{\}}\removed{)}},
...
\returns \added{Let \tcode{R} be the return type of the overload in question.
Returns}
\tcode{\removed{make_pair(}\added{R\{}first, first\added{\}}\removed{)}}
if \range{first}{last} is empty, otherwise
\tcode{\removed{make_pair(}\added{R\{}m, M\added{\}}\removed{)}}, where \tcode{m} is
