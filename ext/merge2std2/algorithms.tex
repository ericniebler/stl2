...
\changed{is}{are} the same as that of
...
it does not have to be defined. In these cases the semantics of}
\tcode{b\added{ }-\added{ }a}
\changed{is}{are} the same as \added{that} of
...
\pnum
A sequence is
\term{sorted with respect to a comparator \added{and projection}}
...
\added{\tcode{bool(invoke(comp, invoke(proj, *(i + n)), invoke(proj, *i)))} is \tcode{false}.}

\begin{addedblock}
\pnum
A sequence is
\term{sorted with respect to a comparator}
if it is sorted with respect to that comparator and the projection \tcode{identity\{\}}.
\end{addedblock}
...
\begin{addedblock}
\pnum
Let \tcode{comp} be \tcode{equal_to\{\}} for the overloads
with no parameter \tcode{comp}.
\end{addedblock}
...
\added{with respect to \tcode{comp} and any projection}.
...
\range{first}{last}
\added{as sorted with respect to \tcode{comp} and any projection,}
into the range
\range{first}{middle}.
...
\pnum
\expects
For iterators \tcode{i1} and \tcode{i2} in \range{first}{last}, and
iterators \tcode{j1} and \tcode{j2} in \range{result_first}{result_last},
after evaluating the assignment \tcode{*j2 = *i2},
\tcode{bool(invoke(comp, invoke(proj1, *i1), invoke(proj2, *j2)))}
shall be equal to
\tcode{bool(invoke(comp, invoke(proj2, *j1), invoke(proj2, *j2)))}
after evaluating the assignment \tcode{*j1 = *i1}.
\begin{note}
Writing a value from the input range into the output range does not affect
how it is ordered by \tcode{comp}.
\end{note}
\end{addedblock}
...
range \range{first}{i} is sorted \added{with respect to \tcode{comp} and any projection}.
...
\begin{addedblock}
\pnum
Let \tcode{comp} be \tcode{less\{\}} for the overloads with no parameter
named \tcode{comp}.
\end{addedblock}
...
sorted \added{with respect to \tcode{comp} and any projection},
...
\item \tcode{bool(invoke(comp, invoke(proj, *j), invoke(proj, *i)))} is
  \tcode{false} for the overload in namespace \tcode{ranges}.
\end{itemize}
...
Exactly $N$ applications of the predicate \added{and projection}.
\item For the overload with an \tcode{ExecutionPolicy},
\bigoh{N \log N} swaps and \bigoh{N} applications of the predicate \added{and projection}.
\end{itemize}
\end{itemdescr}
...
\item For the overloads with an \tcode{ExecutionPolicy},
  \bigoh{N} comparisons \added{and applications of each projection}.
\end{itemize}
...
\end{itemize}
\added{In either case, exactly twice as many projections as comparisons.}
...
\range{a}{b} such that:}
\added{A random access range \range{a}{b} is a
\term{heap with respect to a comparator and projection}
...
and exactly twice as many projections}.
\end{itemdescr}
...
\pnum
Let \tcode{comp} be \tcode{less\{\}} for the overloads with no parameter
\tcode{comp}, and let $E$ be
\begin{itemize}
\item \tcode{bool(comp(*j, *i))} for the overloads in namespace \tcode{std}, or
\item \tcode{bool(invoke(comp, invoke(proj, *j), invoke(proj, *i)))}
  for the overloads in namespace \tcode{ranges}.
\end{itemize}
\end{addedblock}
...
\tcode{comp}, and let $E$ be
\begin{itemize}
\item \tcode{bool(comp(*i, *j))} for the overloads in namespace \tcode{std}, or
\item \tcode{bool(invoke(comp, invoke(proj, *i), invoke(proj, *j)))}
  for the overloads in namespace \tcode{ranges}.
\end{itemize}
\end{addedblock}
...
\added{\tcode{ranges::lexicographical_compare(I1, S1, I2, S2, Comp, Proj1, Proj2)}
...
\pnum
Let \tcode{comp} be \tcode{less\{\}}
and \tcode{proj} be \tcode{identity\{\}} for overloads
...
transforms the sequence into the smallest permutation;
...
\tcode{comp} \added{and \tcode{proj}}.
\added{If no such permutation exists,
transforms the sequence into the largest permutation;
