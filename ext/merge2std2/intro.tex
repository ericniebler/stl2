...
\begin{itemize}
\item Use \tcode{R} (\tcode{r}) instead of \tcode{Rng} (\tcode{rng}) for
  \tcode{Range} template (function) parameter names.
\item Use \tcode{string_view} instead of \tcode{char*}
  in [defns.projection] example.
\item Rephrase final sentence of [allocator.requirements].
\item Define semantics of the swap concepts completely in the \tcode{ranges::swap}
  customization point, which moves to [concepts.swappable] from [utility].
\item Vastly simplify conditional \tcode{noexcept} for the exposition-only
  \placeholder{iter-exchange-move}.
\item Merge \tcode{subrange} deduction guides with identical
  parameter-deeclaration-clauses that otherwise conflict per
  [temp.deduct.guide]/3.
\item Simplify algorithm declarations with the form:
\begin{codeblock}
