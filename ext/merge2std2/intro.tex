...
\item \ref{iterator.concept.random.access} Clarify p1; strike note and add a
  \tcode{// not defined} comment in the class definition.
...
\item \ref{move.sentinel}, \ref{move.sent.ops}, \ref{common.iterator},
  ...
  and \ref{counted.iter.op.const} Fix misuses of \tcode{ConvertibleTo}.
...
\item \ref{common.iterator} Coalesce tiny subclauses. Strike unnecessary
  specialization of \tcode{readable_traits}
...
\item \ref{counted.iterator} A counted iterator knows the distance to the
  \textit{end} of its range. Strike unnecessary specialization of
  \tcode{readable_traits} (given that \tcode{iterator_traits} is specialized).
