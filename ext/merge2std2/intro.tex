...
\item [defns.projection] Use \tcode{string_view} instead of \tcode{char*}
  in example.
\item [allocator.requirements] Rephrase final sentence
\item [concepts.swappable] Define semantics of the swap concepts completely in
  the \tcode{ranges::swap} customization point, which here from [utility]
\item [special.mem.concepts] require nothrow SMFs for no-throw-sentinel;
  qualify references to \tcode{ranges::begin} and \tcode{::end}
\item [iterators.general] Strike subclauses from Table 73
\item [iterator.requirements.general] Several editorial clarifications from
  LWG review.
\item [iterator.synopsis] Replace bogus \tcode{-> auto\&\&}
  deduction constraints with a legit requirement that the expression has
  a referenceable type
\item [incrementable.traits] \& [readable.traits] Strike noise paragraph
  ``XXX is implemented as if:''
\item [incrementable.traits] Replace use of \tcode{decay_t} with
  \tcode{remove_const_t}
\item [iterator.traits] Fix broken wording in P1.
\item [iterator.custpoints.iter_swap] Vastly simplify conditional
  \tcode{noexcept} for the exposition-only \placeholder{iter-exchange-move}.
\item [range.subrange] Merge \tcode{subrange} deduction guides with identical
  parameter-deeclaration-clauses that otherwise conflict per
  [temp.deduct.guide]/3.
\item [range.adaptors.single_view] Remove extraneous \tcode{single_view}
  deduction guide.
\item [algorithm.syn] Simplify algorithm declarations with the form:
\begin{codeblock}
...
\item Require semantics for the \libconcept{Constructible} requirements of
  \tcode{IndirectlyMovableStorable} and \tcode{IndirectlyCopyableStorable}.
