...
\begin{itemize}
\item
  \removed{\tcode{ranges::begin(static_cast<const T\&>(E))} if \tcode{E} is an rvalue of
...
\item \added{If \tcode{E} is an lvalue:}
  \begin{itemize}
  \item
...
\item \added{If \tcode{E} is an lvalue:}
  \begin{itemize}
  \item
    \removed{Otherwise,} \tcode{(E) + extent\added{_v}<T>\removed{::value}} if \tcode{E} has array
...
is expression-equivalent to \tcode{\changed{ranges}{::std2}::\brk{}begin(\changed{static_cast<const T\&>}{as_const}(E))}.

\pnum
\removed{Use of \tcode{\changed{ranges}{::std2}::cbegin(E)} with rvalue \tcode{E} is deprecated.
...
is expression-equivalent to \tcode{\changed{ranges}{::std2}::end(\changed{static_cast<const T\&>}{as_const}(E))}.

\pnum
\removed{Use of \tcode{\changed{ranges}{::std2}::cend(E)} with rvalue \tcode{E} is deprecated.
...
\begin{itemize}
\item
  \removed{\tcode{ranges::rbegin(static_cast<const T\&>(E))} if \tcode{E} is an rvalue of
  type \tcode{T}. This usage is deprecated.
  \enternote This deprecated usage exists so that
...
\item \added{If \tcode{E} is an lvalue:}
  \begin{itemize}
  \item
    \removed{Otherwise,} \tcode{\textit{DECAY_COPY}((E).rbegin())} if it is a valid expression and its type \tcode{I} meets the
...
\begin{itemize}
\item
  \removed{\tcode{ranges::rend(static_cast<const T\&>(E))} if \tcode{E} is an rvalue of
  type \tcode{T}. This usage is deprecated.
  \enternote This deprecated usage exists so that
...
\item \added{If \tcode{E} is an lvalue:}
  \begin{itemize}
  \item
    \removed{Otherwise,} \tcode{\textit{DECAY_COPY}((E).rend())} if it is a valid expression and its type \tcode{S} meets the
    syntactic requirements of
...
is expression-equivalent to \tcode{\changed{ranges}{::std2}::\brk{}rbegin(\changed{static_cast<const T\&>}{as_const}(E))}.
...
is expression-equivalent to \tcode{\changed{ranges}{::std2}::rend(\changed{static_cast<const T\&>}{as_const}(E))}.
...
\begin{itemize}
\item
  \removed{\tcode{ranges::data(static_cast<const T\&>(E))} if \tcode{E} is an rvalue of
...
is expression-equivalent to \tcode{\changed{ranges}{::std2}::data(\changed{static_cast<const T\&>}{as_const}(E))}.
