...
\ednote{Modify paragraphs 2 and 3 as follows:}
...
\tcode{std} \added{or \tcode{std2}}
or namespaces nested within namespace
\tcode{std} \added{or \tcode{std2}}.\footnote{The C standard library headers~(\cxxref{depr.c.headers}) also define
names within the global namespace, while the \Cpp{} headers for C library
facilities~(\cxxref{headers}) may also define names within the global namespace.}%
\indextext{namespace}
...
\tcode{::std::x}, \added{if \tcode{x} is defined in namespace \tcode{std}, or \tcode{::std2::x}, if \tcode{x} is defined in namespace \tcode{std2},}
...
\added{which is defined in namespace \tcode{std}},
the function
\tcode{::std::G}
is meant.
