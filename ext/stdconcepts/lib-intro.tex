...
\tcode{std} \newtxt{or \tcode{std2}}.\footnote{The C standard library headers~(\cxxref{depr.c.headers}) also define
...
\tcode{::std::x}, \newtxt{if \tcode{x} is defined in namespace \tcode{std}, or \tcode{::std2::x}, if \tcode{x} is defined in namespace \tcode{std2},}
...
\newtxt{which is defined in namespace \tcode{std}},
the function
\tcode{::std::G}
is meant.
