...
@\added{namespace std2 \{ inline namespace v1 \{}@
...
\item
  Otherwise, \tcode{\oldtxt{(void)swap_ranges(E1, E2)}}
  \tcode{\newtxt{(void)__std2_swap_ranges(std::begin(E1), std::end(E1), std::begin(E2))}}
  if \tcode{E1} and \tcode{E2} are lvalues of array types~(\cxxref{basic.compound})
  of equal extent and \tcode{std2::swap(*(E1), *(E2))}
  is a valid expression, except that
  \oldtxt{\tcode{noexcept(std2::swap(E1, E2))} is equal to
  \tcode{noexcept(std2::swap(*(E1), *(E2)))}}
  \newtxt{\tcode{E1} is evaluated only once}.
  \tcode{\newtxt{__std2_swap_ranges}}\newtxt{ is the exposition-only function:}
  \begin{codeblock}
  @\newtxt{template <class T, class U>}@
    @\newtxt{constexpr void __std2_swap_ranges(T* first1, T* last1, U* first2)}@
      @\newtxt{noexcept(noexcept(std2::swap(*(E1), *(E2))))}@
  {
