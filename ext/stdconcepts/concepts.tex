...
as if the expressions \tcode{a = b} and \tcode{a = \newtxt{std::}move(c)} had been declared.
...
is unspecified whether \oldtxt{or not} an implementation diagnoses as ill-formed a program
...
\begin{itemdescr}
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{Same<T, U>} and
\tcode{is_same_v<T, U>}.
...
concept DerivedFrom = is_base_of_v<Base, Derived> &&
  ...
  @\newtxt{is_convertible_v<const volatile Derived*, const volatile Base*>;}@ @\oldtxt{// \seebelow}@
\end{itemdecl}

\begin{itemdescr}
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between
\tcode{DerivedFrom<Derived, Base>}
and either
\tcode{is_base_of_v<Base, Derived>}
or
\tcode{\oldtxt{is_convertible_v<remove_cv_t<Derived>*, remove_cv_t<Base>*>}}
\tcode{\newtxt{is_convertible_v<const volatile Derived*, const volatile Base*>}}.
...
{\color{newclr}
\pnum
The \tcode{ConvertibleTo} concept requires an expression of a particular type
...
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{ConvertibleTo<From, To>}
...
type. \oldtxt{\tcode{C} need not be unique.}\exitnote
...
it could be a different type. \tcode{C} \oldtxt{may} \newtxt{might} not be unique.\exitnote
...
\begin{itemdescr}
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{Integral<T>} and
...
\begin{itemdescr}
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{SignedIntegral<T>} and
\tcode{is_signed_v<T>}.

\pnum
\enternote \tcode{SignedIntegral<T>} \oldtxt{may} \newtxt{can} be satisfied even \oldtxt{for} \newtxt{by}
...
\begin{itemdescr}
\pnum
\enternote \tcode{UnsignedIntegral<T>} \oldtxt{may} \newtxt{can} be satisfied even \oldtxt{for} \newtxt{by}
...
\item \tcode{rhs} be an expression such that \tcode{decltype((rhs))} is \tcode{RHS}, and
\item \tcode{rcopy} be a distinct object that is equal to \tcode{rhs}.
\end{itemize}
\tcode{Assignable<LHS, RHS>} is satisfied only if
...
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{Assignable<LHS, RHS>}
...
@\newtxt{concept Swappable = is_swappable_v<T>;}@
\end{itemdecl}
...
\tcode{T}. \tcode{Swappable<T>} is satisfied only if after evaluating either
\tcode{swap(a1, b1)} or \tcode{swap(b1, a1)} in the context described below,
\tcode{a1} is equal to \tcode{b2} and \tcode{b1} is equal to \tcode{a2}.
...
concept SwappableWith =
  @\newtxt{is_swappable_with_v<T, T> \&\& is_swappable_with_v<U, U> \&\&}@
  CommonReference<const remove_reference_t<T>&, const remove_reference_t<U>&> &&
  @\newtxt{is_swappable_with_v<T, U> \&\& is_swappable_with_v<U, T>;}@
...
above, \tcode{C(t1)} is equal to \tcode{C(u2)} and \tcode{C(u1)} is equal to \tcode{C(t2)}.
...
@\oldtxt{// Requires: \tcode{std::forward<T>(t)} shall be swappable with \tcode{std::forward<U>(u)}.}@
template <class T, @\oldtxt{class}\newtxt{SwappableWith<T>}@ U>
void value_swap(T&& t, U&& u) {
  @\newtxt{using std::swap;}@
  @\oldtxt{ranges::}@swap(std::forward<T>(t), std::forward<U>(u)); // OK: uses ``swappable with'' conditions
                                                        // for rvalues and lvalues
}

@\oldtxt{// Requires: lvalues of \tcode{T} shall be swappable.}@
template <@\oldtxt{class}\newtxt{Swappable}@ T>
void lv_swap(T& t1, T& t2) {
  @\newtxt{using std::swap;}@
  @\oldtxt{ranges::}@swap(t1, t2);                                 // OK: uses swappable conditions for
...
namespace N {
  ...
  void swap(A& x, Proxy p) {
    @\oldtxt{ranges}\newtxt{std}@::swap(x.m, p.a->m);             // OK: uses context equivalent to swappable
...
\begin{itemdescr}
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{Destructible<T>}
...
\begin{itemdescr}
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{Constructible<T, Args...>}
...
\pnum
Given \oldtxt{\tcode{const}} lvalues \tcode{b1} and \tcode{b2} of type
\tcode{\newtxt{const} remove_reference_t<B>}, then \tcode{Boolean<B>} is satisfied only if
...
\begin{itemdescr}
\pnum
Let \tcode{t} and \tcode{u} be \oldtxt{\tcode{const}} lvalues of types
\tcode{\newtxt{const} remove_reference_t<T>} and
\tcode{\newtxt{const} remove_reference_t<U>} respectively.
...
\tcode{EqualityComparable<T>} is satisfied only if \tcode{bool(a == b)}
\oldtxt{if and only if} \newtxt{is \tcode{true} when} \tcode{a} is equal to
\tcode{b}~(\ref{concepts.lib.general.equality}), and \tcode{false} otherwise.
...
\begin{itemdescr}
\pnum
Let \tcode{t} be a\newtxt{n} \oldtxt{\tcode{const}} lvalue of type
\tcode{\newtxt{const} remove_reference_t<T>}, \tcode{u} be a\newtxt{n}
\oldtxt{\tcode{const}} lvalue of type \tcode{\newtxt{const} remove_reference_t<U>},
and \tcode{C} be:
\begin{codeblock}
...
\begin{itemdescr}
\pnum
Let \tcode{a}, \tcode{b}, and \tcode{c} be \oldtxt{\tcode{const}} lvalues of type
\tcode{\newtxt{const} remove_reference_t<T>}.
...
\begin{itemdescr}
\pnum
There need \oldtxt{not} be \oldtxt{any} \newtxt{no} subsumption relationship between \tcode{Movable<T>} and
\tcode{is_object_v<T>}.
\end{itemdescr}
...
behave similarly to built-in types like \tcode{int}, except that they \oldtxt{may} \newtxt{might} not be
...
\pnum
\begin{example}
\newtxt{A function that generates random numbers can satisfy \tcode{Invocable},}
since the \tcode{invoke} function call expression is not required to be
equality-preserving~(\ref{concepts.lib.general.equality})\oldtxt{,
a function that generates random numbers may satisfy \tcode{Invocable}}.
\end{example}
\end{itemdescr}
