...
\ednote{Consider relocating this subclause into [description].}
...
concept DerivedFrom = is_base_of_v<Base, Derived> &&
  is_convertible_v<remove_cv_t<Derived>*, remove_cv_t<Base>*>; // \seebelow
\end{itemdecl}
...
\begin{itemdecl}
template <class T>
concept Swappable = requires(T& a, T& b) { std2::swap(a, b); };

template <class T, class U>
concept SwappableWith =
  CommonReference<const remove_reference_t<T>&, const remove_reference_t<U>&> &&
  requires(T&& t, U&& u) {
    std2::swap(std::forward<T>(t), std::forward<T>(t));
    std2::swap(std::forward<U>(u), std::forward<U>(u));
    std2::swap(std::forward<T>(t), std::forward<U>(u));
    std2::swap(std::forward<U>(u), std::forward<T>(t));
  };
\end{itemdecl}
...
\tcode{std2::swap(t, u)} or \tcode{std2::swap(u, t)}, \tcode{t2} is equal to
...
void value_swap(T&& t, U&& u) {
  std2::swap(std::forward<T>(t), std::forward<U>(u)); // OK: uses ``swappable with'' conditions
...
void lv_swap(T& t1, T& t2) {
  std2::swap(t1, t2);                                 // OK: uses swappable conditions for
...
namespace N {
  ...
  void swap(A& x, Proxy p) {
    std2::swap(x.m, p.a->m);                // OK: uses context equivalent to swappable
...
\pnum
\enternote Unlike the \tcode{Destructible} \oldtxt{library concept in the \Cpp
