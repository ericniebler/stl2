...
\bigoh{N} probe obvious conflicts with that. Second, the probe would return a
...
\pnum
Unless otherwise specified, references to \removed{other} entities described in this
document \added{or the Ranges TS} are assumed to be qualified with \tcode{\added{::}std::experimental::ranges::}, and
references to entities described in the International Standard are assumed to be
qualified with \tcode{\added{::}std::}.
