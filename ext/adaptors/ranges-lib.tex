...
\pnum
\effects Initializes \tcode{tagged_pair<tag::begin(I), tag::end(S)>} with
\tcode{ranges::begin(r)} and \tcode{ranges::end(r)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{this->first == this->second}.
\end{itemdescr}

\rSec4[ranges.iterator_range.nonmember]{\tcode{iterator_range} non-member functions}

\indexlibrary{\idxcode{get}!\idxcode{iterator_range}}%
\begin{itemdecl}
template <std::size_t N, class I, class S>
  requires N < 2
constexpr decltype(auto) get(iterator_range<I, S>&& r) noexcept;
...
\begin{itemdescr}
\pnum
\effects Initializes \tcode{rng_} with \tcode{r}, and initializes
...
\rSec4[ranges.iterator_range.nonmember]{\tcode{sized_iterator_range} non-member functions}

\indexlibrary{\idxcode{get}!\idxcode{sized_iterator_range}}%
\begin{itemdecl}
template <std::size_t N, class I, class S>
    requires N < 3
  constexpr auto get(const sized_iterator_range<I, S>& r);
...
\begin{itemize}
\item \tcode{\textit{DECAY_COPY}(E)} if the type of \tcode{E} satisfies the concept
  \tcode{View}.
\item \tcode{sized_iterator_range\{E\}} if \tcode{E} is an lvalue and has a type
  that satisfies concept \tcode{SizedRange}.
\item \tcode{iterator_range\{E\}} if \tcode{E} is an lvalue and has a type that
  satisfies concept \tcode{Range}.
...
namespace std { namespace experimental { namespace ranges { inline namespace v1 {
  ...
  template <InputRange R, CopyConstructible Pred>
    requires IndirectUnaryPredicate<Pred, iterator_t<R>> &&
      (std::is_lvalue_reference_v<R> || View<decay_t<R>>)
...
\rSec4[ranges.adaptors.iota_view.iterator.op.cmp]{\tcode{iota_view::iterator} comparisons}
...
\rSec4[ranges.adaptors.iota_view.iterator.op.nonmember]{\tcode{iota_view::iterator} non-member functions}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{\xname{sentinel}<false>\{ranges::end(base_)\}} and
...
\rSec4[ranges.adaptors.take_view.sentinel.op]{\tcode{take_view::\xname{sentinel}} operations}
\rSec5[ranges.adaptors.take_view.sentinel.op.ctor]{\tcode{take_view::\xname{sentinel}} constructors}
...
\rSec5[ranges.adaptors.take_view.sentinel.op.conv]{\tcode{take_view::\xname{sentinel}} conversion}
...
\tcode{iterator_category} is \tcode{ranges::bidirectional_iterator_tag}.
\item Otherwise, if \tcode{Base} satisfies \tcode{ForwardlRange}, and if
\tcode{is_reference_v<reference_t<iterator_t<Base>{>}>} is \tcode{true}, and if
...
\rSec4[ranges.adaptors.join_view.iterator.op]{\tcode{join_view::\xname{iterator}} operations}
\rSec5[ranges.adaptors.join_view.iterator.op.ctor]{\tcode{join_view::\xname{iterator}} constructors}
...
\rSec5[ranges.adaptors.join_view.iterator.op.star]{\tcode{join_view::iterator::operator*}}
...
\rSec5[ranges.adaptors.join_view.iterator.op.inc]{\tcode{join_view::iterator::operator++}}
...
\rSec5[ranges.adaptors.join_view.iterator.op.dec]{\tcode{join_view::iterator::operator\dcr}}
...
\rSec4[ranges.adaptors.join_view.sentinel.op]{\tcode{join_view::\xname{sentinel}} operations}
\rSec5[ranges.adaptors.join_view.sentinel.op.ctor]{\tcode{join_view::\xname{sentinel}} constructors}
...
\pnum
The purpose of \tcode{empty_view} is to produce an emtpy range of elements of
a particular type.
