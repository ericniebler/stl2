...
@\added{template <Iterator I, Sentinel<I> S = I, bool Sized = SizedSentinel<S, I>{>}}@
...
namespace std {
template <class I, class S@\added{, bool Sized}@>
  struct tuple_size<ranges::@\changed{iterator_range}{subrange}@<I, S@\added{, Sized}@>>
    : std::integral_constant<size_t, 2> {};
template <class I, class S@\added{, bool Sized}@>
  struct tuple_element<0, ranges::@\changed{iterator_range}{subrange}@<I, S@\added{, Sized}@>> {
    using type = I;
  };
template <class I, class S@\added{, bool Sized}@>
  struct tuple_element<1, ranges::@\changed{iterator_range}{subrange}@<I, S@\added{, Sized}@>> {
    using type = S;
  };
...
namespace std { namespace experimental { namespace ranges { inline namespace v1 {
  ...
  template <Iterator I, Sentinel<I> S = I, bool Sized = SizedSentinel<S, I>>
  struct subrange : view_interface<subrange<I, S, Sized>> {
  private:
    static constexpr bool StoreSize = Sized && !SizedSentinel<S, I>;
    I begin_ {}; // \expos
    S end_ {}; // \expos
  ...
  public:
    ...
    constexpr subrange(subrange<X, Y, B> r) requires !StoreSize || B;
  
    template <ConvertibleTo<I> X, ConvertibleTo<S> Y, bool B>
    constexpr subrange(subrange<X, Y, B> r, difference_type_t<I> n) requires Sized;
  
    template <@\textit{pair-like-convertible-to}@<I, S> PairLike>
    constexpr subrange(PairLike r) requires !StoreSize;
  
    template <@\textit{pair-like-convertible-to}@<I, S> PairLike>
    constexpr subrange(PairLike r, difference_type_t<I> n) requires Sized;
  ...
  subrange(P, difference_type_t<tuple_element_t<0, P>>) ->
    subrange<tuple_element_t<0, P>, tuple_element_t<1, P>, true>;
...
\begin{itemdescr}
\pnum
\requires \tcode{B} is \tcode{false} or \tcode{n == r.size()}.
...
\indexlibrary{\idxcode{operator bool}!\idxcode{subrange}}%
\begin{itemdecl}
constexpr explicit operator bool() const;
\end{itemdecl}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{return !empty();}.
\end{itemdescr}

\indexlibrary{\idxcode{operator!}!\idxcode{subrange}}%
\begin{itemdecl}
constexpr bool operator!() const;
\end{itemdecl}
...
\item Otherwise, \tcode{return end_ - begin;}.
\end{itemize}
\end{itemdescr}
...
\begin{itemdecl}
template <std::size_t N, class I, class S, bool B>
  requires N < 2
constexpr auto get(const subrange<I, S, B>& r) noexcept;
\end{itemdecl}
...
\item Otherwise, \tcode{\changed{iterator_range}{subrange}\{make_counted_iterator(E, F), default_sentinel\{\}\}}
