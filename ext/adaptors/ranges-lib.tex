...
\rSec4[ranges.iterator_range.ctor]{\tcode{iterator_range} constructors}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{return \{this->first, this->second\};}.
\end{itemdescr}
...
\rSec4[ranges.sized_iterator_range.ctor]{\tcode{sized_iterator_range} constructors}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{return \{rng_.first, rng_.second\};}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{return rng_.first;}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{return rng_.second;}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to \tcode{return size_;}.
\end{itemdescr}
...
namespace std { namespace experimental { namespace ranges { inline namespace v1 {
  ...
  public:
    ...
    using iterator = @\xname{outer_iterator}@<false>;
    using sentinel = @\xname{outer_sentinel}@<false>;
...
\item \begin{codeblock}
InputRange<T> && (is_lvalue_reference_v<T> || View<decay_t<T>>) &&
...
\begin{itemize}
\item \tcode{iterator_range\{E, E + F\}} is \tcode{T} is a pointer to an object type,
and if \tcode{U} is implicitly convertible to \tcode{ptrdiff_t}.
\item Otherwise, \tcode{iterator_range\{make_counted_iterator(E, F), default_sentinel\{\}\}}
if \tcode{Iterator<T> \&\& ConvertibleTo<U, difference_type_t<T>{>}} is satisfied.
