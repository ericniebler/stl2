...
namespace std {
  ...
  namespace ranges {
    ...
    template <@\placeholder{dereferenceable}@ T>
        requires @\seebelow@ using @\added{iter_}@rvalue_reference_t
...
\end{codeblock}
\added{If \tcode{Iterator} has a valid member type \tcode{pointer}, then
...
\begin{addedblock}
\pnum
The member type \tcode{iterator_concept} of \tcode{reverse_iterator<Iterator>} is
defined as follows:
\begin{itemize}
\item If \tcode{Iterator} satisfies \tcode{ranges::ContiguousIterator}, then
\tcode{contiguous_iterator_tag}.
\item Otherwise, if \tcode{Iterator} satisfies \tcode{ranges::RandomAccessIterator}, then
\tcode{random_access_iterator_tag}.
...
namespace std {
  ...
  public:
    ...
    @\added{constexpr back_insert_iterator() = default;}@
...
namespace std {
  ...
  public:
    ...
    @\added{constexpr front_insert_iterator() = default;}@
