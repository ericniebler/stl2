...
\removed{and} stream buffers\iref{stream.buffers}\added{, and other ranges\iref{range}}.
...
\tcode{STL1DefaultConstructible} requirements, using a value-initialized iterator
...
[range.iterator.requirements] (and P0944's \tcode{ContiguousIterator} concept)
all get moved into a new section [iterator.concepts].}
...
satisfies the \tcode{WeaklyIncrementable} concept~(\ref{iterator.concept.weaklyincrementable})\changed{,}{ and}
\tcode{R} is the name of a type that
satisfies the \tcode{Readable} concept~(\ref{iterator.concept.readable})\removed{, and
...
\pnum
\ednote{TODO describe this section}
...
\begin{itemize}
\item \tcode{X} satisfies the \tcode{STL1CopyConstructible}, \tcode{STL1CopyAssignable}, and
\tcode{STL1Destructible} requirements\iref{utility.arg.requirements} and lvalues
...
\tcode{STL1EqualityComparable} (\tref{equalitycomparable}) requirements and
...
\term{single pass}
algorithms.
Value type \tcode{T} is not required to be a \tcode{STL1CopyAssignable} type (\tref{copyassignable}).
...
\item \tcode{X} satisfies the \tcode{STL1DefaultConstructible}
