...
\rSec2[concepts.lib.corelangconvertible]{Concept Convertible}
...
\rSec2[concepts.lib.corelangcommon]{Concept Common}
...
\pnum
\enternote Users are free to specialize \text{common_type} when at least one parameter is a
user-defined type. Those specializations are considered by the \tcode{Common} concept.\exitnote

\end{itemdescr}

\rSec2[concepts.lib.corelangintegral]{Concept Integral}
...
\pnum
\tcode{Integral<T>} is an alias for \tcode{is_integral<T>::value}~(\cxxref{meta.unary.prop}).
\end{itemdescr}

\rSec2[concepts.lib.corelangsignedintegral]{Concept SignedIntegral}
...
signed integral type~(\cxxref{basic.fundamental}).

\end{itemdescr}

\rSec2[concepts.lib.corelangsignedintegral]{Concept UnsignedIntegral}
...
unsigned integral type~(\cxxref{basic.fundamental}).
\end{itemdescr}

\rSec2[concepts.lib.corelangdefaultconstructible]{Concept DefaultConstructible}
...
\begin{itemize}
\item The declaration \tcode{T t;} default-initializes~(\cxxref{dcl.init})
object \tcode{t}.
\item The declaration \tcode{T u\{\};} value-initializes~(\cxxref{dcl.init})
object \tcode{u}.
...
\rSec2[concepts.lib.corelangmoveconstructible]{Concept MoveConstructible}
...
\rSec2[concepts.lib.corelangcopyconstructible]{Concept CopyConstructible}
...
\rSec2[concepts.lib.corelangconstructible]{Concept Constructible}
...
\rSec2[concepts.lib.corelangdestructible]{Concept Destructible}
...
\rSec2[concepts.lib.corelangmoveassignable]{Concept MoveAssignable}
...
\rSec2[concepts.lib.corelangcopyassignable]{Concept CopyAssignable}
...
\rSec2[concepts.lib.corelangswappable]{Concept Swappable}
...
\item \tcode{a} equals \tcode{a}; that is, if the value of \tcode{a} is not volatile.
\end{itemize}
...
concept bool Regular =
  Semiregular<T>() && EqualtiyComparable<T>;
\end{itemdecl}
...
\rSec2[concepts.lib.functions.callable]{Concept Callable}

\indexlibrary{\idxcode{Callable}}%
\begin{itemdecl}
// Exposition only
template <class T>
auto __as_function(T&& t, std::true_type)
{
  return mem_fn(t);
}
template <class T>
T __as_function(T&& t, std::false_type)
{
  return forward<T>(t);
}
...
template <class T>
using FunctionType =
  decltype(__as_function(declval<T>(), is_member_pointer<decay_t<T>>{}));
...
\begin{itemdescr}
\pnum
\enternote The \tcode{Callable} concept is used to describe types that can be used in arguments to
\tcode{\textit{INVOKE}}~(\cxxref{func.require}).\exitnote
\end{itemdescr}

\rSec2[concepts.lib.functions.regularcallable]{Concept RegularCallable}
...
\rSec2[concepts.lib.functions.callablepredicate]{Concept CallablePredicate}

\indexlibrary{\idxcode{CallablePredicate}}%
\begin{itemdecl}
template <class F, class...Args>
concept bool CallablePredicate =
  RegularCallable<F, Args...> &&
  Convertible<ResultType<FunctionType<F>, Args...>, bool>;
\end{itemdecl}

\rSec2[concepts.lib.functions.callablerelation]{Concept CallableRelation}

\indexlibrary{\idxcode{CallableRelation}}%
\begin{itemdecl}
template <class F, class T>
concept bool CallableRelation() {
  return CallablePredicate<F, T, T> &&
    Relation<FunctionType<F>, T>();
}
