...
subsequent chapters should be renumbered as appropriate, but they aren't here for the sake of
simplicity).}
...
\ref{concepts.lib.corelang}   & Core language concepts         &   \tcode{<experimental/ranges\oldtxt{_v1}/concepts>}      \\
\ref{concepts.lib.compare}    & Comparison concepts            &                      \\
\ref{concepts.lib.object}     & Object concepts                &                      \\
\ref{concepts.lib.callables}  & \oldtxt{Function}\newtxt{Callable} concepts              &                      \\
\end{libsumtab}
...
\pnum
The concepts in this Clause are defined in the namespace \tcode{std::experimental::ranges\oldtxt{_v1}\newtxt{::v1}}.
...
equal outputs. The inputs to an expression \oldtxt{is}\newtxt{are} the set of the expression's operands. The
...
does not preclude the expression \tcode{a / b} being equality preserving. \newtxt{The
...
@\oldtxt{using CommonType = common_type_t<T, U>;}@

template <class T, class U>
concept bool Common() {
  return requires (T t, U u) {
    typename @\oldtxt{CommonType}\newtxt{common_type_t}@<T, U>;
    typename @\oldtxt{CommonType}\newtxt{common_type_t}@<U, T>;
    requires Same<@\oldtxt{CommonType}\newtxt{common_type_t}@<U, T>, @\oldtxt{CommonType}\newtxt{common_type_t}@<T, U>>();
    @\oldtxt{CommonType}\newtxt{common_type_t}@<T, U>(std::forward<T>(t));
    @\oldtxt{CommonType}\newtxt{common_type_t}@<T, U>(std::forward<U>(u));
  };
}
\end{itemdecl}
...
\begin{itemdescr}
\pnum
Let \tcode{C} be \tcode{\oldtxt{CommonType}}\tcode{\newtxt{common_type_t}}\tcode{<T, U>}. Let \tcode{t1} and \tcode{t2} be objects
...
concept bool Assignable() {
  return @\newtxt{Common<T, U>() \&\& }@requires(T&& a, U&& b) {
...
\begin{itemdescr}
\pnum
Let \tcode{t} be an lvalue of type \tcode{T}\newtxt{, and \tcode{R} be the
type \tcode{remove_reference_t<U>}}. If \tcode{U} is an lvalue reference
type, let \tcode{v} be a lvalue of type \tcode{\oldtxt{U}\newtxt{R}};
otherwise, let \tcode{v} be an rvalue of type \tcode{\oldtxt{U}\newtxt{R}}.
\newtxt{Let \tcode{uu} be a distinct object of type \tcode{R} such that
\tcode{uu == v}.}
Then \tcode{Assignable<T, U>()} is satisfied if and only if

\begin{itemize}
\item \tcode{std::addressof(t = v) == std::addressof(t)}.
\item \newtxt{After evaluating \tcode{t = v}:}
\begin{itemize}
\item \newtxt{\tcode{t == uu}.}
\item \newtxt{If \tcode{v} is a non-\tcode{const} rvalue, its resulting
...
\item \newtxt{Otherwise, \tcode{v} is not modified.}
\end{itemize}
\end{itemize}
\end{itemdescr}
...
concept bool Swappable() {
  return requires(T&& @\oldtxt{t}\newtxt{a}@, T&& @\oldtxt{u}\newtxt{b}@) {
    @\oldtxt{(void)}\newtxt{ranges::}@swap(std::forward<T>(@\oldtxt{t}\newtxt{a}@), std::forward<T>(@\oldtxt{u}\newtxt{b}@));
  };
}
...
concept bool Swappable() {
  return Swappable<T>() &&
    ...
    Common<T, U>() &&
    requires(T&& t, U&& u) {
      @\oldtxt{(void)}\newtxt{ranges::}@swap(std::forward<T>(t), std::forward<U>(u));
      @\oldtxt{(void)}\newtxt{ranges::}@swap(std::forward<U>(u), std::forward<T>(t));
    };
}
\end{itemdecl}
...
\begin{itemdescr}
\pnum
\oldtxt{\enternote The casts to \tcode{void} in the required expressions indicate
...
and only if\oldtxt{:} \newtxt{given distinct objects \tcode{tt} equal to \tcode{t}
...
void value_swap(T&& t, U&& u) {
  using std::@\newtxt{experimental::ranges::}@swap;
...
namespace N {
  ...
  void swap(A& x, Proxy p) {
    std::@\newtxt{experimental::ranges::}@swap(x.m, p.a->m);  // OK: uses context equivalent to swappable
...
concept bool Boolean() {
  return @\newtxt{MoveConstructible<B>() \&\& // (see \ref{concepts.lib.object.moveconstructible})}@
...
concept bool EqualityComparable() {
  return @\newtxt{WeaklyEqualityComparable<T, T>();}@
  @\oldtxt{requires(const T a, const T b) \{}@
    @\oldtxt{\{a == b\} -> Boolean;}@
    @\oldtxt{\{a != b\} -> Boolean;}@
  @\oldtxt{\};}@
}
...
concept bool EqualityComparable() {
  return Common<T, U>() &&
    ...
    EqualityComparable<@\oldtxt{CommonType}\newtxt{common_type_t}@<T, U>>() &&
    @\newtxt{WeaklyEqualityComparable<T, U>();}@
    @\oldtxt{requires(const T a, const U b) \{}@
      @\oldtxt{\{a == b\} -> Boolean;}@
      @\oldtxt{\{b == a\} -> Boolean;}@
      @\oldtxt{\{a != b\} -> Boolean;}@
      @\oldtxt{\{b != a\} -> Boolean;}@
...
\tcode{\oldtxt{CommonType}}\tcode{\newtxt{common_type_t}}\tcode{<T, U>}. Then \tcode{EqualityComparable<T, U>()}
is satisfied if and only if:

\begin{itemize}
\item \tcode{bool(a == b) == bool(C(a) == C(b))}.
\item \oldtxt{\tcode{bool(a != b) == bool(C(a) != C(b))}.}
\item \oldtxt{\tcode{bool(b == a) == bool(C(b) == C(a))}.}
\item \oldtxt{\tcode{bool(b != a) == bool(C(b) != C(a))}.}
\end{itemize}
\end{itemdescr}
...
\tcode{Equal\-ity\-Comp\-arable<\brk{}T, \brk{}U>()} and
\tcode{Weakly\-Equal\-ity\-Comp\-arable<T, U>()} is purely semantic.
\exitnote
} %% color{newclr}
...
\tcode{LessThanComparable} with \tcode{\newtxt{Strict}TotallyOrdered} (acknowledging that this is a breaking
change that makes type requirements stricter). Replace references to [lessthancomparable] with
references to [concepts.lib.compare.stricttotallyordered]}
...
Then \tcode{\newtxt{Strict}TotallyOrdered<T>()} is satisfied if and only if
...
does not mean that the type does not satisfy \tcode{\newtxt{Strict}TotallyOrdered}.\exitnote
}
\end{itemdescr}
...
concept bool @\newtxt{Strict}@TotallyOrdered() {
  return Common<T, U>() &&
    @\newtxt{Strict}@TotallyOrdered<T>() &&
    @\newtxt{Strict}@TotallyOrdered<U>() &&
    @\newtxt{Strict}@TotallyOrdered<@\oldtxt{CommonType}\newtxt{common_type_t}@<T, U>>() &&
...
of type \tcode{U}, and \tcode{C} be \tcode{\oldtxt{CommonType}}\tcode{\newtxt{common_type_t}}\tcode{<T, U>}.
Then  \tcode{\newtxt{Strict}TotallyOrdered<T, U>()} is satisfied if and only if
...
\pnum
The \oldtxt{function }concepts in this section describe the requirements on function
objects~(\ref{function.objects}) and their arguments.

\rSec2[concepts.lib.callables.callable]{Concept Callable}

\pnum
\newtxt{The \tcode{Callable} concept specifies a relationship between a callable
...
\begin{itemdecl}
@\oldtxt{template <class F, class...Args>}@
@\oldtxt{using ResultType = result_of_t<F(Args...)>;}@

template <class F, class...Args>
concept bool @\oldtxt{Function}\newtxt{Callable}@() {
  return CopyConstructible<F>() &&
    requires (F f, Args&&...args) {
      @\newtxt{invoke(f, std::forward<Args>(args)...); // not required to be equality preserving}@
      @\oldtxt{typename ResultType<F, Args...>;}@
      @\oldtxt{\{ f(std::forward<Args>(args)...) \} // not required to be equality preserving}@
        @\oldtxt{-> Same<ResultType<F, Args...>{}>;}@
    };
}
\end{itemdecl}
...
\begin{itemdescr}
\pnum
\enternote Since the \newtxt{\tcode{invoke}} function call
expression \oldtxt{of \tcode{Function}} is not required to be
equality-preserving~(\ref{concepts.lib.general.equality}), a function that generates random numbers
may satisfy \tcode{\oldtxt{Function}\newtxt{Callable}}.\exitnote
\end{itemdescr}
...
\begin{itemdecl}
template <class F, class...Args>
concept bool Regular@\oldtxt{Function}\newtxt{Callable}@() {
  return @\oldtxt{Function}\newtxt{Callable}@<F, Args...>();
}
\end{itemdecl}

\begin{itemdescr}
\pnum
The \newtxt{\tcode{invoke}} function call expression \oldtxt{of \tcode{RegularFunction} }shall be
equality-preserving~(\ref{concepts.lib.general.equality}). \enternote This requirement supersedes the
annotation \oldtxt{on the function call expression }in the definition of \tcode{\oldtxt{Function}\newtxt{Callable}}. \exitnote

\pnum
\enternote A random number generator does not satisfy
\tcode{Regular\oldtxt{Function}\newtxt{Callable}}.\exitnote

\pnum
\enternote The\oldtxt{re is no syntactic difference}\newtxt{ distinction} between \tcode{\oldtxt{Function}\newtxt{Callable}} and
\tcode{Regular\oldtxt{Function}\newtxt{Callable}}\newtxt{ is purely semantic}.\exitnote
\end{itemdescr}
...
concept bool Predicate() {
  return Regular@\oldtxt{Function}\newtxt{Callable}@<F, Args...>() &&
    Boolean<@\oldtxt{ResultType<F, Args...>}\newtxt{result_of_t<F\&(Args...)>}@>();
}
\end{itemdecl}
...
object of type \tcode{U}, and \tcode{C} be \tcode{\oldtxt{CommonType}}\tcode{\newtxt{common_type_t}}\tcode{<T, U>}.
