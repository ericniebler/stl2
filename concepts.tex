...
\item \tcode{C(t())} equals \tcode{C(t())} if and only if \tcode{t} is an
  equality preserving function~(\ref{concepts.lib.general.equality}).
\item \tcode{C(u())} equals \tcode{C(u())} if and only if \tcode{u} is an
  equality preserving function~(\ref{concepts.lib.general.equality}).
\end{itemize}

\pnum
\enternote Users are free to specialize \tcode{common_reference} and
\tcode{basic_common_reference} when at least one parameter depends on a
user-defined type. Those specializations are considered by the
\tcode{CommonReference} concept.\exitnote
\end{itemdescr}
} % \color{newclr}
...
\item \tcode{C(\oldtxt{t1}\newtxt{t()})} equals \tcode{C(\oldtxt{t2}\newtxt{t()})} if and only if
  ...
  function~(\ref{concepts.lib.general.equality})}.
