...
concept bool Constructible@\newtxt{() \{}\oldtxt{ =}@
  ...
  @\newtxt{(is_reference<T>::value ||}@
    @\newtxt{(Destructible<T>() \&\& requires (Args\&\&...args) \{}@
      @\newtxt{new T\{std::forward<Args>(args)...\}; // not \newtxt{required to be} equality preserving}@
    @\newtxt{\}));}@
@\newtxt{\}}@
\end{itemdecl}
...
\pnum
\newtxt{\enternote The expression \tcode{[](T)\{\}(std::move(t))} requires that \tcode{T}
...
\tcode{CommonType<T, U>}. Then \tcode{types \tcode{R}, \tcode{T}, and \tcode{U} model}
\tcode{Relation\newtxt{<T,U>()}} \newtxt{is satisfied} if and only if
...
\begin{itemdescr}
\pnum
Let \tcode{comp} be a well-formed object of type \tcode{Comp}. Then \oldtxt{type \tcode{Comp}
models concept} \tcode{StrictWeakOrder\newtxt{<Comp>()}} \newtxt{is satisfied} if and only if
\oldtxt{it}\newtxt{\tcode{comp}} imposes a \techterm{strict weak ordering} on its arguments.
