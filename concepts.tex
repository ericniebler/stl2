...
concept bool Constructible@\newtxt{() \{}\oldtxt{ =}@
  ...
  @\newtxt{(is_reference<T>::value ||}@
    @\newtxt{(Destructible<T>() \&\& requires (Args\&\&...args) \{}@
      @\newtxt{new T\{forward<Args>(args)...\};}@
    @\newtxt{\}));}@
@\newtxt{\}}@
\end{itemdecl}

\ednote{\newtxt{Requiring \tcode{T\{forward<Args>(args)...\}} to be equality preserving
implies that construction from equal arguments must result in equal objects, i.e., the
value of a \tcode{Constructible} object must be totally determined by the constructor
arguments.}}
