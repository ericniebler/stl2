...
\pnum
\newtxt{An expression is \techterm{equality preserving} if, given equal inputs, the
expression results in equal outputs. Any expression appearing in a
\techterm{requires-expression} in this document is required to be equality preserving,
except for those annotated with the comment ``not equality preserving.'' An
expression so annotated may be equality preserving, but it is not required to be so.
\enternote Not all input values must be valid for a given expression, e.g., for
integers \tcode{a} and \tcode{b}, the expression \tcode{a / b} is not well-defined
when \tcode{b} is \tcode{0}. This does not preclude the expression \tcode{a / b}
being equality preserving. \exitnote}
...
when violated, make the function non-regular. \enternote A function that returns void
is necessarily a regular function. \exitnote}
...
\newtxt{
\pnum
The concepts in this section are all defined in the \tcode{std::experimental::ranges_v1}
namespace.
}
...
\rSec1[concepts.lib.comparison]{Comparison concepts}

\rSec2[concepts.lib.comparison.general]{In general}
...
\item \newtxt{\tcode{bool(b1 == b2)}, \tcode{bool(b1 == bool(b2))}, and
      \tcode{bool(bool(b1) == b2)} are all semantically identical to
...
\rSec2[concepts.lib.comparison.equalitycomparable]{Concept EqualityComparable}
...
\rSec2[concepts.lib.comparison.totallyordered]{Concept TotallyOrdered}
...
\pnum
\oldtxt{A}\newtxt{Given an object \tcode{t} of type \tcode{T} and pointer \tcode{p}
...
\pnum
\newtxt{\enternote The expression \tcode{[](T)\{\}(move(t))} requires that \tcode{T}
can be copy-initialized from an rvalue of type \tcode{remove_cv_t<T>}; the constructor
selected by overload resolution must be non-explicit. \exitnote}
...
concept bool CopyConstructible@\newtxt{() \{}\oldtxt{ =}@
  @\newtxt{return}@ MoveConstructible<T>@\newtxt{()}@ &&
    @\newtxt{Constructible<T, remove_cv_t<T>\&>() \&\&}@
    @\newtxt{Constructible<T, const remove_cv_t<T>\&>() \&\&}@
    @\newtxt{Constructible<T, const remove_cv_t<T>\&\&>() \&\&}@
    requires (@\oldtxt{T}\newtxt{remove_cv_t<T>\&}@ a, const @\oldtxt{T}\newtxt{remove_cv_t<T>}@& b) {
      @\oldtxt{T(a)} \newtxt{[](T)\{\}(a)}@;
      @\oldtxt{T(b)} \newtxt{[](T)\{\}(b)}@;
      @\oldtxt{T((const T\&\&)b)} \newtxt{[](T)\{\}(move(b))}@;
    };
...
to \tcode{v} and \tcode{v} is unchanged.
\end{itemize}

\pnum
\newtxt{\enternote The expressions \tcode{[](T)\{\}(a)}, \tcode{[](T)\{\}(b)}, and
\tcode{[](T)\{\}(move(b))}, require that \tcode{T} can be copy-initialized from an
lvalue of type (possibly \tcode{const}) \tcode{remove_cv_t<T>} and an rvalue of type
\tcode{const remove_cv_t<T>}; the constructor selected by overload resolution must
be non-explicit. \exitnote}

\ednote{\newtxt{Ideally, \tcode{CopyConstructible} would include array allocation
requirements \tcode{new T[1]\{a\}}, \tcode{new T[1]\{b\}}, and
\tcode{new T[1]\{move(b)\}}. This is not currently possible since [expr.new]/19
requires an accessible default constructor even when all array elements are
initialized.}}
\end{itemdescr}
...
\item \newtxt{\tcode{t} is not a volatile object or const volatile object~(\cxxref{basic.type.qualifier}).}
\end{itemize}

%% FIXME: I've relocated the non-volatile requirement here from Semiregular for the time being,
%% but ideally it will be obsoleted by a more general requirement that covers all side effects
%% pending the outcome of the discussion that begins at
%% https://github.com/CaseyCarter/stl2/commit/2d54e6b6275c9d1f97b3c5d899171ef53ef5dc04#commitcomment-11773953
...
\item After the assignment \newtxt{\tcode{t = v},} \tcode{t} is equal to \tcode{v} and \tcode{v} is unchanged.
\end{itemize}
\end{itemdescr}
