...
\item \oldtxt{\tcode{T(rv)}} \newtxt{\tcode{T\{rv\}}, \tcode{*new T\{rv\}}, or
\tcode{*new T[1]\{rv\}}} is equal to the value of \tcode{rv} before the construction.
\end{itemize}

\pnum
\newtxt{\enternote The array allocation expression \tcode{new T[1]\{move(t)\}}
requires that \tcode{T} can be copy-initialized from an rvalue of type
\tcode{remove_cv_t<T>}; the constructor selected by overload resolution must be
non-explicit. \exitnote}
...
to \tcode{u}.
\item \oldtxt{\tcode{T(v)}} \newtxt{\tcode{T\{v\}}, \tcode{*new T\{v\}}, or
\tcode{*new T[1]\{v\}}} is equal to \tcode{v} and \tcode{v} is unchanged.
\end{itemize}
\end{itemdescr}

\pnum
\newtxt{\enternote The array allocation expressions \tcode{new T[1]\{a\}},
\tcode{new T[1]\{b\}}, and \tcode{new T[1]\{move(b)\}} require that \tcode{T} can
be copy-initialized from an lvalue of type (possibly \tcode{const})
\tcode{remove_cv_t<T>} and an rvalue of type \tcode{const remove_cv_t<T>};
