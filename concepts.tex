...
\begin{itemdescr}
\pnum
Let \tcode{f} be any object of type \tcode{F}, and \tcode{args} be any function parameter pack with
the pattern \tcode{Args}. Then types \tcode{F} and \tcode{Args...} model \tcode{Function} if an only
if
...
\pnum
There is no requirement that the function call operator be equality-preserving. That is, function
\tcode{f} is allowed to return non-equal objects when called with equal arguments, and still model
\tcode{Function}.
...
\begin{itemdescr}
\pnum
Models of \tcode{RegularFunction} are required to be equality-preserving; that is, when passed
equal arguments, they are required to return equal objects.
