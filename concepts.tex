...
\begin{itemdescr}
\pnum
\enternote The casts to \tcode{void} in the required expressions indicate
...
\begin{itemdescr}
\pnum
Let \tcode{a} and \tcode{b} be \oldtxt{well-formed} objects
...
\rSec2[concepts.lib.compare.totallyordered]{Concept \added{Strict}TotallyOrdered}

\ednote{Remove table [lessthancomparable] in [utility.arg.requirements]. Replace uses of
\tcode{LessThanComparable} with \tcode{\added{Strict}TotallyOrdered} (acknowledging that this is a breaking
...
\begin{itemdecl}
template <class T>
concept bool @\added{Strict}@TotallyOrdered() {
...
\begin{itemdescr}
\pnum
Let \tcode{a}, \tcode{b}, and \tcode{c} be \oldtxt{well-formed} objects of type \tcode{T}.
Then \tcode{\added{Strict}TotallyOrdered<T>()} is satisfied if and only if
...
\enternote Not all arguments will be well-formed for a given type. For example, $NaN$ is not a
well-formed floating point value, and many types' moved-from states are not well-formed. This
does not mean that the type does not satisfy \tcode{\added{Strict}TotallyOrdered}.\exitnote
}
\end{itemdescr}
...
concept bool @\added{Strict}@TotallyOrdered() {
  return Common<T, U>() &&
    @\added{Strict}@TotallyOrdered<T>() &&
    @\added{Strict}@TotallyOrdered<U>() &&
    @\added{Strict}@TotallyOrdered<@\oldtxt{CommonType}\newtxt{common_type_t}@<T, U>>() &&
...
Then  \tcode{\added{Strict}TotallyOrdered<T,U>()} is satisfied if and only if
...
\begin{itemdescr}
\pnum
Let \tcode{r} be any \oldtxt{well-formed} object of type \tcode{R}, \tcode{a} be any
\oldtxt{well-formed} object of type \tcode{T}, \tcode{b} be any \oldtxt{well-formed}
