...
@\newtxt{concept bool \xname{ConstructibleObject} = // exposition only}@
  @\newtxt{Destructible<T>() \&\& requires (Args\&\& ...args) \{}@
    @\newtxt{T\{forward<Args>(args)...\} // not equality preserving}@
    @\newtxt{new T\{forward<Args>(args)...\} // not equality preserving}@
  @\newtxt{\};}@

@\newtxt{template <class T, class ...Args>}@
@\newtxt{concept bool \xname{BindableReference} = // exposition only}@
...
concept bool CopyConstructible@\newtxt{() \{}\oldtxt{ =}@
  @\newtxt{return}@ MoveConstructible<T>@\newtxt{()}@ &&
    ...
    requires (@\oldtxt{T a, const T\& b}\newtxt{const remove_cv_t<T> t}@) {
      @\newtxt{[](T)\{\}(t);}@
      @\oldtxt{T(a);}@
      @\oldtxt{T(b);}@
      @\oldtxt{T((const T\&\&)b);}@
...
\pnum
\newtxt{\enternote The expression \tcode{[](T)\{\}(t)} (and its implicit variants)
require that \tcode{T} can be copy-initialized from an lvalue of type (possibly
\tcode{const}) \tcode{remove_cv_t<T>} and an rvalue of type \tcode{const remove_cv_t<T>};
the constructor selected by overload resolution must be non-explicit. \exitnote}
