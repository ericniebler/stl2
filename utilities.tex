...
{\color{addclr}
\pnum
\synopsis{Header \tcode{<experimental/ranges\oldtxt{_v1}/utility>} synopsis}
}

\begin{codeblock}
@\added{namespace std \{ namespace experimental \{ namespace ranges\oldtxt{_v1}\newtxt{ \{ inline namespace v1} \{}@
  @\added{// \ref{utility.swap}, swap:}@
  @\oldtxt{using std::swap;}@
  @\newtxt{namespace \{}@
    @\newtxt{constexpr \unspec swap = \unspec;}@
  @\newtxt{\}}@

  // \ref{utility.exchange}, exchange:
  template <@\removed{class}\added{\oldtxt{Movable}\newtxt{MoveConstructible}}@ T, class U=T>
...
\begin{codeblock}
  ...
  @\oldtxt{// \ref{tagged.special}, tagged specialized algorithms}@
  @\oldtxt{template <class Base, class... Tags>}@
    @\oldtxt{requires Swappable<Base\&>()}@
  @\oldtxt{void swap(tagged<Base, Tags...>\& x, tagged<Base, Tags...>\& y) noexcept(\seebelow);}@
...
namespace std {
  ...
  struct tuple_size<experimental::ranges@\oldtxt{_v1}@::tagged<Base, Tags...>>;

  template <size_t N, class Base, class... Tags>
  struct tuple_element<N, experimental::ranges@\oldtxt{_v1}@::tagged<Base, Tags...>>;
}
\end{codeblock}
}
...
that are not already defined in namespace \tcode{std::experimental::ranges\oldtxt{_v1}\newtxt{::v1}} in header
\tcode{<experimental/ranges\oldtxt{_v1}/utility>} are imported with
...
\tcode{<experimental/ranges\oldtxt{_v1}/utility>} are aliased with a
...
\ednote{\oldtxt{The following edits to the two \tcode{swap} overloads are to be applied to the versions in
namespace \tcode{std}.}}
...
\setcounter{Paras}{1}
\pnum
\synopsis{Header \tcode{<\added{experimental/ranges\oldtxt{_v1}/}functional>} synopsis}

\begin{codeblock}
@\added{namespace std \{ namespace experimental \{ namespace ranges\oldtxt{_v1}\newtxt{ \{ inline namespace v1} \{}@
  @\newtxt{// \ref{func.invoke}, invoke:}@
  @\newtxt{template <class F, class... Args>}@
  @\newtxt{result_of_t<F\&\&(Args\&\&...)> invoke(F\&\& f, Args\&\&... args);}@

  @\newtxt{// \ref{comparisons}, comparisons:}@
  template <class T = void>
    @\oldtxt{requires EqualityComparable<T>() || Same<T, void>()}@
  struct equal_to; @\newtxt{// not defined}@
  @\newtxt{template<EqualityComparable T> struct equal_to<T>;}@

  template <class T = void>
    @\oldtxt{requires EqualityComparable<T>() || Same<T, void>()}@
  struct not_equal_to; @\newtxt{// not defined}@
  @\newtxt{template<EqualityComparable T> struct not_equal_to<T>;}@

  template <class T = void>
    @\oldtxt{requires StrictTotallyOrdered<T>() || Same<T, void>()}@
  struct greater; @\newtxt{// not defined}@
  @\newtxt{template<StrictTotallyOrdered T> struct greater<T>;}@
  @\newtxt{template<class T> struct greater<T*>;}@

  template <class T = void>
    @\oldtxt{requires StrictTotallyOrdered<T>() || Same<T, void>()}@
  struct less; @\newtxt{// not defined}@
  @\newtxt{template<StrictTotallyOrdered T> struct less<T>;}@
  @\newtxt{template<class T> struct less<T*>;}@

  template <class T = void>
    @\oldtxt{requires StrictTotallyOrdered<T>() || Same<T, void>()}@
  struct greater_equal; @\newtxt{// not defined}@
  @\newtxt{template<StrictTotallyOrdered T> struct greater_equal<T>;}@
  @\newtxt{template<class T> struct greater_equal<T*>;}@

  template <class T = void>
    @\oldtxt{requires StrictTotallyOrdered<T>() || Same<T, void>()}@
  struct less_equal; @\newtxt{// not defined}@
  @\newtxt{template<StrictTotallyOrdered T> struct less_equal<T>;}@
  @\newtxt{template<class T> struct less_equal<T*>;}@

  @\newtxt{template<> struct equal_to<void>;}@
  @\newtxt{template<> struct not_equal_to<void>;}@
  @\newtxt{template<> struct greater<void>;}@
  @\newtxt{template<> struct less<void>;}@
  @\newtxt{template<> struct greater_equal<void>;}@
  @\newtxt{template<> struct less_equal<void>;}@

  @\newtxt{// \ref{func.identity}, identity:}@
...
that are not already defined in namespace \tcode{std::experimental::ranges\oldtxt{_v1}} in header
\tcode{<experimental/ranges\oldtxt{_v1}/functional>} are imported with
\grammarterm{using-declaration}{s}~(\cxxref{namespace.udecl}). \enterexample
\begin{codeblock}
namespace std { namespace experimental { namespace ranges@\oldtxt{_v1}\newtxt{ \{ inline namespace v1}@ {
...
that are not already defined in namespace \tcode{std::experimental::ranges\oldtxt{_v1}} in header
\tcode{<experimental/ranges\oldtxt{_v1}/functional>} are aliased with a
\grammarterm{namespace-alias-definition}~(\cxxref{namespace.alias}). \enterexample
\begin{codeblock}
namespace std { namespace experimental { namespace ranges@\oldtxt{_v1}\newtxt{ \{ inline namespace v1}@ {
...
\begin{itemdecl}
template <@\oldoldtxt{class T = void}\newtxt{EqualityComparable T}@>
  @\oldtxt{requires EqualityComparable<T>() || Same<T, void>()}@
struct equal_to@\newtxt{<T>}@ {
...
@\newtxt{struct greater<T*> \{}@
  @\newtxt{constexpr bool operator()(T* x, T* y) const;}@
@\newtxt{\};}@
\end{itemdecl}
...
template <> struct greater<void> {
  template <class T, class U>
    @\added{requires StrictTotallyOrdered<T, U>() \newtxt{|| \textit{BUILTIN_PTR_CMP}(T, >, U) // \seebelow}}@
  constexpr auto operator()(T&& t, U&& u) const
    -> decltype(std::forward<T>(t) > std::forward<U>(u));
...
\newtxt{For template specializations \tcode{greater<void>}, \tcode{less<void>},
...
\newtxt{
\pnum
Given a relational operator \tcode{\textit{OP}}, an object \tcode{x} of type \tcode{X},
and an object \tcode{y} of type \tcode{Y}, let \tcode{\textit{BUILTIN_PTR_CMP}(X, \textit{OP}, Y)}
be true if an only if \tcode{\textit{OP}} in the expression \tcode{(X\&\&)x \textit{OP} (Y\&\&)y} resolves
to a built-in operator comparing pointers.
}
...
In addition, \tcode{Relation<greater<T>, T>()}~(\ref{concepts.lib.callables.relation}), \tcode{Relation<greater_equal<T>, T>()},
and \tcode{Relation<less_equal<T>, T>()} shall be satisfied and those class template
instantiations shall be semantically consistent with \tcode{less<T>}~(\cxxref{operators}).
}

\newtxt{
\pnum
For all object types \tcode{T} for which there exists a specialization \tcode{equal_to<T>},
the instantiation \tcode{equal_to<T>} shall satisfy \tcode{Relation<equal_to<T>, T>()},
and \tcode{equal_to<T>} shall induce an equivalence relation on its arguments.
In addition, \tcode{Relation<not_equal_to<T>, T>()} shall be satisfied and that class template
instantiation shall be semantically consistent with \tcode{equal_to<T>}~(\cxxref{operators}).
}
...
struct identity {
  ...
  @\newtxt{typedef \unspec is_transparent;}@
};
\end{itemdecl}
...
\begin{codeblock}
// defined in header <experimental/ranges\oldtxt{_v1}/utility>
...
namespace std { namespace experimental { namespace ranges@\oldtxt{_v1} \newtxt{\{ inline namespace v1}@ {
  ...
  struct tagged :
    ...
    @\newtxt{friend void swap(tagged\&, tagged\&) noexcept(\seebelow)}@
      @\newtxt{requires Swappable<Base\&>();}@
  };
...
\synopsis{Header \tcode{<experimental/ranges\oldtxt{_v1}/tuple>} synopsis}
...
that are not already defined in namespace \tcode{std::experimental::ranges\oldtxt{_v1}} in header
\tcode{<experimental/ranges\oldtxt{_v1}/tuple>} are imported with
\grammarterm{using-declaration}{s}~(\cxxref{namespace.udecl}). \enterexample
\begin{codeblock}
namespace std { namespace experimental { namespace ranges@\oldtxt{_v1}\newtxt{ \{ inline namespace v1}@ {
  using std::tuple;
