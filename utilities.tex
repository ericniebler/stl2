...
\ednote{\oldtxt{Before [refwrap], insert the following section and renumber subsequent sections
...
namespace std {
  ...
  @\added{template <class T, class U> struct is_swappable_with;}@
  @\added{template <class T> struct is_swappable;}@

  template <class T> struct is_destructible;
  [@...@]
  template <class T> struct is_nothrow_move_assignable;

  @\added{template <class T, class U> struct is_nothrow_swappable_with;}@
  @\added{template <class T> struct is_nothrow_swappable;}@
  ...
  [@...@]
  @\added{template <class T, class U> constexpr bool is_swappable_with_v}@
    @\added{= is_swappable_with<T, U>::value;}@
  @\added{template <class T> constexpr bool is_swappable_v}@
    @\added{= is_swappable<T>::value;}@
  [@...@]
  @\added{template <class T, class U> constexpr bool is_nothrow_swappable_with_v}@
    @\added{= is_nothrow_swappable_with<T, U>::value;}@
  @\added{template <class T> constexpr bool is_nothrow_swappable_v}@
    @\added{= is_nothrow_swappable<T>::value;}@
  [@...@]
}
\end{codeblock}
...
\ednote{Change [meta.unary.prop], Table 49 -- ``Type property predicates'' in
ISO/IEC 14882:2014 as indicated. The following is taken from the current
Working Draft of C++17.}
...
\added{\tcode{template <class T, class U>}}\br
  \added{\tcode{struct is_swappable_with;}} &
  \added{The expressions \tcode{swap(declval<T>(),}}\brk\added{\tcode{ declval<U>())}
  and \tcode{swap(}}\brk\added{\tcode{declval<U>(),}}\brk\added{\tcode{ declval<T>())}
  ...
  considered.}
  \begin{note}
  \added{The compilation of the expressions can result in side effects such as the
  ...
  \end{note} &
  \added{\tcode{T} and \tcode{U} shall be complete types,
  (possibly cv-qualified) \tcode{void}, or
  arrays of unknown bound.}  \\ \rowsep

\indexlibrary{\idxcode{is_swappable}}%
\added{\tcode{template <class T>}}\br
  \added{\tcode{struct is_swappable;}} &
  \added{For a referenceable type \tcode{T},
  the same result as \tcode{is_\brk\-swappable_\brk\-with_\brk\-v<}}\brk\added{\tcode{T\&,\brk T\&>},
  otherwise \tcode{false}.} &
  \added{\tcode{T} shall be a complete type,
  (possibly cv-qualified) \tcode{void}, or
...
\tcode{\added{template <class T, class U>}}\br
  \added{\tcode{struct is_nothrow_swappable_with;}} &
  \added{\tcode{is_swappable_with_v<T, }}\brk\added{\tcode{U>} is \tcode{true} and
...
\tcode{\added{template <class T>}}\br
  \added{\tcode{struct is_nothrow_swappable;}} &
  \added{For a referenceable type \tcode{T},
  the same result as \tcode{is_nothrow_swappable_}}\brk\added{\tcode{with_v<T\&, T\&>},
  otherwise \tcode{false}.} &
  \added{\tcode{T} shall be a complete type,
  (possibly cv-qualified) \tcode{void}, or
  an array of unknown bound.} \\ \rowsep
...
... & & \\ \rowsep
 ...
 user-defined type\added{ and \tcode{sizeof...(T) == 2}}. \begin{note} Such
...
\tcode{type}.}
\begin{note} \added{Such specializations may be used to influence
