...
namespace std { @\newtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  template <@\oldtxt{class}\newtxt{InputIterator}@ @\oldtxt{InputIterator}\newtxt{I}@, @\newtxt{Sentinel<I> S, }@
            @\oldtxt{class ForwardIterator}\newtxt{ForwardIterator O}@>
    @\newtxt{requires Constructible<value_type_t<O>, reference_t<O>{>}() \&\&}@
             @\newtxt{Same<value_type_t<O>\&, reference_t<O>{>}}@
  @\oldtxt{ForwardIterator}\newtxt{tagged_pair<tag::in(I), tag::out(O)>}@
    uninitialized_copy(@\oldtxt{InputIterator}\newtxt{I}@ first, @\oldtxt{InputIterator}\newtxt{S}@ last, @\oldtxt{ForwardIterator}\newtxt{O}@ result);
\end{codeblock}
...
\pnum
\complexity Exactly \tcode{last - first} copies.

\begin{codeblock}
  template <@\oldtxt{class}\newtxt{InputIterator}@ @\oldtxt{InputIterator}\newtxt{I}@, @\oldtxt{class Size, class ForwardIterator}\newtxt{OutputIterator O}@>
    @\newtxt{requires Constructible<value_type_t<O>, reference_t<I>{>}() \&\&}@
             @\newtxt{Same<value_type_t<O>\&, reference_t<O>{>}}@
  @\oldtxt{ForwardIterator}\newtxt{tagged_pair<tag::in(I), tag::out(O)>}@
    uninitialized_copy_n(@\oldtxt{InputIterator}\newtxt{I\&\&}@ first, @\oldtxt{Size}\newtxt{difference_type_t<I>}@ n, @\oldtxt{ForwardIterator}\newtxt{O\&\&}@ out)
\end{codeblock}
...
\pnum Exactly \tcode{n} copies.
} %% \color{newclr}

\rSec3[uninitialized.fill]{uninitialized_fill}
\begin{codeblock}
  template <@\oldtxt{class}\newtxt{CopyConstructible}@ @\oldtxt{ForwardIterator}\newtxt{T}@, @\oldtxt{class}\newtxt{OutputIterator<const T\&>}@ @\oldtxt{T}\newtxt{O},@
            @\newtxt{Sentinel<O> S}@>
    @\newtxt{requires Same<value_type_t<T>\&, reference_t<T>{>}}@
    @\oldtxt{void}\newtxt{O}@ fill(@\oldtxt{ForwardIterator}\newtxt{O}@ first, @\oldtxt{ForwardIterator}\newtxt{S}@ last, const T& value);

  @\newtxt{template <CopyConstructible T, OutputRange<const T\&> Rng>}@
    @\newtxt{requires Same<value_type_t<T>\&, reference_t<T>{>}}@
  @\newtxt{safe_iterator_t<Rng>}@
    @\newtxt{fill(Rng\&\& rng, const T\& value);}@

  template <@\oldtxt{class}\newtxt{CopyConstructible}@ @\oldtxt{ForwardIterator}\newtxt{T}@, @\oldtxt{class Size, class}\newtxt{OutputIterator<const T\&>}@ @\oldtxt{T}\newtxt{O}@>
    @\newtxt{requires Same<value_type_t<T>\&, reference_t<T>{>}}@
  @\oldtxt{ForwardIterator}\newtxt{O}@ fill_n(@\oldtxt{ForwardIterator}\newtxt{O}@ first, @\oldtxt{Size}\newtxt{difference_type_t<O>}@, const T& value);
}@\newtxt{\}\}\}}@
\end{codeblock}
...
\setcounter{Paras}{0}
\pnum
\effects \tcode{fill} copies \tcode{value} through all the iterators in range
\tcode{[first, last)}. \tcode{fill_n} copies \tcode{value} through all the
iterators in the range \tcode{[first, first + n)}, otherwise it does nothing.

\pnum
\returns \tcode{fill} returns \tcode{last}. \tcode{fill_n} returns \tcode{first + n} for
non-negative values of \tcode{n} and \tcode{first} for negative values.

\pnum
\complexity Exactly \tcode{last - first}, \tcode{n}, or 0 assignments, respectively.
} %% color{newclr}
