...
constrained with \tcode{LessThanComparable} today are constrained by \tcode{\added{Strict}TotallyOrdered} in this
...
\tcode{\added{Strict}TotallyOrdered}). Similar constraints are added to the other function objects
in \tcode{<functional>}. \oldtxt{Also, the so-called transparent relational function objects
...
\tcode{\oldtxt{Weak}InputIterator}.
...
\tcode{Sized\oldtxt{IteratorRange}\newtxt{Sentinel}}~(\ref{iterators.sizedsentinel}). The \tcode{Sized\oldtxt{IteratorRange}\newtxt{Sentinel}}
...
\tcode{OutputIterator} concept\oldtxt{ and the more frequently-needed \tcode{WeakOutputIterator}}. Even
if\oldtxt{ they are} not strictly needed, \oldtxt{their}\newtxt{its} absence would be surprising.
