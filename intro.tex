...
\techterm{sentinels} to denote the end of a range.
...
\item The addition new overloads of the Standard Library algorithms that take iterable
objects.
\item Support of \techterm{invokable} (as opposed to callable) objects as arguments to
the higher-order algorithms. (An invokable is an object that can be used as the first
argument of the \tcode{\textit{INVOKE}}) pseudo-macro~([func.require])).
...
\pnum
TODO based on the previous range paper.

\pnum
Does not (yet) extend the concept-ification of the STL to the containers or the
numeric algorithms. Does not specify new range views, actions, or facade or adaptor
