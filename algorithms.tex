...
\pnum
\returns
\tcode{\changed{result + (last1 - first1)}{make_pair(first1 + $N$, result + $N$)}}
\added{ or }\tcode{\added{make_tuple(first1 + $N$, first2 + $N$, result + $N$)}}.
...
type~(\cxxref{function.objects})}{a\oldtxt{n} callable object~(\cxxref{func.require})}. The return value of \changed{the
...
\end{removedblock}
\begin{codeblock}
make_pair(lower_bound(first, last, value, comp@\added{, proj}@),
          upper_bound(first, last, value, comp@\added{, proj}@))
\end{codeblock}
...
\pnum
\returns
\tcode{\changed{result + (last1 - first1) + (last2 - first2)}{
make_tuple(last1, last2, result + (last1 - first1) + (last2 - first2))}}.
...
\pnum
\returns
\tcode{make_tuple(last1, last2, result + (last1 - first1) + (last2 - first2))}.
...
\tcode{\added{make_tuple(last1, last2, result + $n$)}}\added{, where \tcode{$n$} is
...
\pnum
\returns
\removed{The end of the constructed range}\tcode{\added{make_pair(last1, result + $n$)}, where $n$
