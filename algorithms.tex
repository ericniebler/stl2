...
\changed{\tcode{pred(*i)}}{\tcode{invoke(pred, invoke(proj, *i))}}
...
\added{invoke(pred, invoke(proj, *(i + n)), *(first2 + n)) != false}}.
Returns \tcode{last1}
if
...
\added{invoke(pred,}\brk\added{ invoke(proj1, *i), invoke(proj2, *j)) != false}}.
Returns \tcode{last1}
...
\added{invoke(pred, invoke(proj, *i), invoke(proj, *(i + 1))) != false}}.
Returns \tcode{last}
...
{invoke(proj, *i) == value, invoke(pred, invoke(proj, *i)) != false}}.
...
{invoke(pred, invoke(proj, *(i + n)), value) != false}}.
Returns \tcode{last}
if no such iterator is found.
...
for which \tcode{\changed{pred(*i)}{invoke(pred, invoke(proj, *i))}} is \tcode{true}.

\pnum
\returns \changed{The end of the resulting range}{\tcode{\{last, result + (last - first)\}}}.

\pnum
\complexity Exactly \tcode{last - first} applications of the corresponding predicate\added{ and projection}.
...
\tcode{\changed{op(*(first1 + (i - result)))}{invoke(op, invoke(proj, *(first1 + (i - result))))}}
or
\tcode{\changed{binary_op(*(first1 + (i - result)), *(first2 + (i - result)))}
{invoke(\brk{}binary_op, invoke(proj1, *(first1 + (i - result))), invoke(proj2, *(first2 + (i - result))))}}.
...
\tcode{\changed{*i == old_value}{invoke(proj, *i) == old_value}},
\tcode{\changed{pred(*i) != false}{invoke(pred, invoke(proj, *i)) != false}}.
...
\pnum
\effects
\removed{The first algorithm invokes the function object \tcode{gen}
...
\added{Assigns the value of \tcode{invoke(gen)} through successive iterators in the
...
\pnum
\returns \changed{\tcode{generate_n} returns
...
\complexity
Exactly
\tcode{last - first}\changed{,
\tcode{n}, or 0
invocations of \tcode{gen}}{ evaluations of \tcode{invoke(gen)}} and assignments\removed{, respectively}.
\end{itemdescr}
...
in the range \range{first}{last}
for which the following corresponding conditions hold:
\tcode{\changed{*i == value}{invoke(proj, *i) == value}},
\tcode{\changed{pred(*i) != false}{invoke(pred, invoke(proj, *i)) != false}}.
...
\tcode{\changed{*(i - 1) == *i}{invoke(proj, *(i - 1)) == invoke(proj, *i)}}
or
\tcode{\changed{pred(*(i - 1), *i) != false}{invoke(pred, invoke(proj, *(i - 1)), invoke(proj, *i)) != false}}.
...
\range{first}{last}
for which the following corresponding conditions hold:
\tcode{\changed{*i == *(i - 1)}{invoke(proj, *i) == invoke(proj, *(i - 1))}}
or
\tcode{\changed{pred(*i, *(i - 1)) != false}{invoke(pred, invoke(proj, *i), invoke(proj, *(i - 1))) != false}}.
...
\tcode{invoke(pred, invoke(proj, *i)) != false} come} before those that do not\added{,
...
\tcode{\changed{pred(*j) != false}{invoke(pred, invoke(proj, *j)) != false}},
and for every iterator \tcode{k} in the range \range{i}{last},
\tcode{\changed{pred(*k) == false}{invoke(pred, invoke(proj, *k)) == false}}.
...
\tcode{\changed{pred(*i)}{invoke(pred, invoke(proj, *i))}} is \tcode{true}, or to
...
function call operation}{the \tcode{invoke} operation} applied to
...
\tcode{\changed{comp(*(i + n), *i) == false}{invoke(comp, invoke(proj, *(i + n)), invoke(proj, *i)) == false}}.
...
\tcode{\changed{e < value}{invoke(comp, invoke(proj, e), value)}}
\removed{or
\tcode{comp(e, value)}}.
...
or}
\tcode{\changed{comp(*j, value) != false}{invoke(comp, invoke(proj, *j), value) != false}}.
...
\range{first}{last}
shall be partitioned with respect to the expression
\tcode{\changed{!(value < e)}{!invoke(comp, value, invoke(proj, e))}}
\removed{
or
...
or}
\tcode{\changed{comp(value, *j) == false}{invoke(comp, value, invoke(proj, *j)) == false}}.
...
is \tcode{result + (last1 - first1) + (last2 - first2)}\removed{, such that the resulting range satisfies
\tcode{is_sorted(result, result_last)} or
\tcode{is_sorted(result, result_last, comp)}, respectively}.
\added{If an element \tcode{a} precedes \tcode{b} in an input range,
...
\tcode{\added{make_tagged_tuple<tag::in1,}}{\\}
\tcode{\added{tag::in2, tag::out>(last1, last2, result_last)}}.
...
invoke(comp, invoke(proj, *i), invoke(proj, *(i - 1)))}}
will be false.
...
\tcode{t.size() > 0}}\tcode{\added{distance(rng) > 0}}.
...
\pnum
\complexity
Exactly one comparison \added{and exactly two applications of the projection}.
\end{itemdescr}
...
\pnum
\complexity At most \tcode{\changed{(3/2) * t.size()}{(3/2) * distance(rng)}}
applications of the corresponding predicate\added{, and at most twice as many applications of the projection}.
\end{itemdescr}
...
invoke(comp, invoke(proj, *j), invoke(proj, *i)) == false}}.
Returns
\tcode{last}
if
\tcode{first == last}.
...
\added{exactly twice as many applications of the} projection}.
\end{itemdescr}
...
\removed{\tcode{!(*i < *j)}
or}
\tcode{\changed{comp(*i, *j) == false}{invoke(comp, invoke(proj, *i), invoke(proj, *j)) == false}}.
Returns
\tcode{last}
if
\tcode{first == last}.
