...
\pnum
Let \tcode{a} be a valid iterator of type \tcode{I} and let \tcode{b} be a valid sentinel of type
...
\begin{codeblock}
  ...
  struct difference_type<T, enable_if_t<is_integral<T>::value>>
    : make_unsigned< decltype(declval<T>() - declval<T>()) > {
  };
  template <class T>
...
// \expos
template<typename R>
struct @\xname{is_sized_iterable_impl}@ :
  std::integral_constant<bool, @\xname{SizedIterableLike}@<R>> {
};
...
struct is_sized_iterable :
  conditional<
    ...
    @\xname{is_sized_iterable_impl}@<R>,
    is_sized_iterable<remove_const_t<remove_reference_t<R>>>
  >::type {
};
\end{codeblock}
\ednote{REVIEW: The handling of top-level reference here is inconsistent with the other
type traits.}
\exitnote
...
// \expos
template<typename T>
struct @\xname{is_range_impl}@
  : std::integral_constant<
      bool,
      Iterable<T> && (!@\xname{ContainerLike}@<T> || Derived<T, range_base>)
    >
{};
