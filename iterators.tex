...
any type that \oldtxt{models}\newtxt{satisfies} \tcode{WeakIterator}.
\end{addedblock}
...
\pnum
The \tcode{Readable} concept is \oldtxt{modeled}\newtxt{satisfied} by types that are readable by
...
be a dereferenceable object of type \tcode{Out}. Then \oldtxt{types \tcode{T} and
\tcode{Out} model} \tcode{MoveWritable\newtxt{<Out,T>()}} \newtxt{is satisfied} if and only if
...
\tcode{Out}. Then \oldtxt{types \tcode{T} and \tcode{Out} model} \tcode{Writable\newtxt{<Out,T>()}}
\newtxt{is satisfied} if and only if

\begin{itemize}
\item If \tcode{Out} is \tcode{Readable}, after the assignment \tcode{*o = v},
\tcode{*o} is equal to the value of \tcode{v}\oldtxt{ and \tcode{v} is unchanged}.
\end{itemize}
...
dereferenceable object of type \tcode{Out}. Then \oldtxt{types \tcode{I} and \tcode{Out} model}
\tcode{IndirectlyMovable\newtxt{<I,Out>()}} \newtxt{is satisfied} if and only if
...
Let \tcode{i} be an object of type \tcode{I}, and let \tcode{o} be a
dereferenceable object of type \tcode{Out}. Then \oldtxt{types \tcode{I} and \tcode{Out} model}
\tcode{IndirectlyCopyable\newtxt{<I,Out>()}} \newtxt{is satisfied} if and only if

\begin{itemize}
\item If \tcode{Out} is \tcode{Readable}, after the assignment \tcode{*o = *i},
\tcode{*o} is equal to the value of \tcode{*i}\oldtxt{ and the value of
\tcode{*i} is unchanged}.
\end{itemize}
...
\pnum
Let \tcode{i} be an object of type \tcode{I}. \newtxt{When both pre and post-increment
are valid, \tcode{i} is said to be \techterm{incrementable}.} Then \oldtxt{the type
\tcode{I} models} \tcode{WeaklyIncrementable\newtxt{<I>()}} \newtxt{is satisfied} if and only if

\begin{itemize}
\item \tcode{++i} is valid if and only if \tcode{i++} is valid.
\item If \tcode{i} is incrementable, then \newtxt{either }\tcode{++i} \newtxt{or \tcode{i++} }moves \tcode{i} to the next element.
\item If \tcode{i} is incrementable, then \tcode{\oldtxt{(}\&++i == \&i\oldtxt{) != false}}.
\item \oldtxt{If \tcode{i} is incrementable, then \tcode{i++} moves \tcode{i} to the next element.}
\end{itemize}

\ednote{REVIEW Can we be more specific than ``moves to the next element''?}

\pnum
Not all \oldtxt{arguments will}\newtxt{values must} be incrementable for a given type.
\enternote For example, $NaN$ is not a well-formed floating point value and hence is
not incrementable. Likewise, past-the-end iterators \oldtxt{are also}\newtxt{may} not
\newtxt{be} incrementable. This does not mean that the\newtxt{ir} type\newtxt{s}
do\oldtxt{es} not \oldtxt{model}\newtxt{satisfy} \tcode{WeaklyIncrementable}. \exitnote
...
Then \oldtxt{the type \tcode{I} models} \tcode{Incrementable\newtxt{<I>()}} \newtxt{is satisfied}
if and only if

\begin{itemize}
\item \oldtxt{If \tcode{(a == b) != false} then \tcode{(++a == ++b) != false}.} %% Implied by equality preservation of ++i
\item \oldtxt{If \tcode{(a == b) != false} then \tcode{((a++, a) == (b++, b)) != false}.} %% Implied by equality preservation of i++ and the following requirements
\item If \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}} then \tcode{\newtxt{bool}(a++ == b)\oldtxt{ != false}}.
\item If \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}} then \tcode{\newtxt{bool}((a++, a) == ++b)\oldtxt{ != false}}.
\end{itemize}
...
algorithms with types that \oldtxt{model}\newtxt{satisfy} \tcode{Incrementable}.\exitnote
  ...
  concept bool WeakInputIterator@\newtxt{() \{}\oldtxt{ =}@
    @\newtxt{return}@ WeakIterator<I>@\newtxt{()}@ &&
      ...
      requires(I i@\newtxt{, const I ci}@) {
        ...
        { i++ } -> Readable; @\newtxt{// not \newtxt{required to be} equality preserving}@
        @\newtxt{requires Same<ValueType<I>, ValueType<decltype(i++)>{}>();}@
        requires Derived@\newtxt{From}@<IteratorCategory<I>, weak_input_iterator_tag>@\newtxt{()}@;
        @\newtxt{\{ *ci \} -> const ValueType<I>\&;}@
      };
  @\newtxt{\}}@
\end{codeblock}
\end{addedblock}
...
\begin{codeblock}
  ...
  concept bool InputIterator@\newtxt{() \{}\oldtxt{ =}@
    @\newtxt{return}@ WeakInputIterator<I>@\newtxt{()}@ &&
      Iterator<I>@\newtxt{()}@ &&
      Derived@\newtxt{From}@<IteratorCategory<I>, input_iterator_tag>@\newtxt{()}@;
  @\newtxt{\}}@
\end{codeblock}
...
\pnum
\enternote
\oldoldtxt{For input iterators}\newtxt{Since \tcode{InputIterator} is only \tcode{WeaklyIncrementable}},
\tcode{a == b}
does not imply
\tcode{++a == ++b}.
...
\term{single pass}
algorithms.
\oldoldtxt{Value type T}\newtxt{\tcode{ValueType<I>}} is not required to be a \oldoldtxt{\tcode{CopyAssignable}}\newtxt{\tcode{Copyable}}
...
\begin{codeblock}
  ...
  concept bool ForwardIterator@\newtxt{() \{}\oldtxt{ =}@
    @\newtxt{return}@ InputIterator<I>@\newtxt{()}@ &&
      Incrementable<I>@\newtxt{()}@ &&
      Derived@\newtxt{From}@<IteratorCategory<I>, forward_iterator_tag>@\newtxt{()}@;
  @\newtxt{\}}@
\end{codeblock}
\end{addedblock}
...
underlying sequence. However, value-initialized iterators \newtxt{of the same type}
may be compared and shall compare equal\oldoldtxt{ to other value-initialized iterators of the same type}.
...
\item \oldoldtxt{\tcode{X} is a pointer type or t}\newtxt{T}he expression
\tcode{\oldoldtxt{(void)++X(a)}\newtxt{([](X x)\{++x;\}(a)}, *a\newtxt{)}} is equivalent to the expression \tcode{*a}.
\end{itemize}
...
implies
\tcode{++a == ++b}
(which is not true for \oldoldtxt{input and output}\newtxt{weaker} iterators)
and the removal of the restrictions on the number of\oldoldtxt{ the} assignments through
a mutable iterator
(which applies to output iterators)
allow\oldoldtxt{s} the use of multi-pass one-directional algorithms with forward iterators.
\exitnote

\ednote{Remove Table 109}

\pnum
\oldoldtxt{If \tcode{a} and \tcode{b} are equal, then either \tcode{a} and \tcode{b}
are both dereferenceable
or else neither is dereferenceable.}

\pnum
\oldoldtxt{If \tcode{a} and \tcode{b} are both dereferenceable, then \tcode{a == b}
if and only if }\removed{\tcode{*a} and \tcode{*b} are bound to the same object}\oldtxt{
\tcode{*a} is equal to \tcode{*b}}\oldoldtxt{.}
...
\begin{codeblock}
  ...
  concept bool BidirectionalIterator@\newtxt{() \{}\oldtxt{ =}@
    @\newtxt{return}@ ForwardIterator<I>@\newtxt{()}@ &&
      Derived@\newtxt{From}@<IteratorCategory<I>, bidirectional_iterator_tag>@\newtxt{()}@ &&
      requires (I i) {
        { @\dcr@i } -> Same<I&>;
        { i@\dcr@ } -> Same<I>;
      };
...
\pnum
Let \tcode{a} and \tcode{b} be decrementable objects of type \tcode{I}. Then \oldtxt{\tcode{I}
models} \tcode{BidirectionalIterator\newtxt{<I>()}} \newtxt{is satisfied} if and only if:

\begin{itemize}
\item \tcode{\&\dcr{}a == \&a}.
\item If \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}}, then \tcode{\newtxt{bool}(a\dcr{} == \oldtxt{j}\newtxt{b})\oldtxt{ != false}}.
\item If \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}}, then \tcode{\newtxt{bool}((a\dcr{}, a) == \dcr{}\oldtxt{j}\newtxt{b})\oldtxt{ != false}}.
\item If \tcode{a} is incrementable and \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}}, then
      \tcode{\newtxt{bool}(\dcr{}(++a) == b)\oldtxt{ != false}}.
\item If \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}}, then \tcode{\newtxt{bool}(++(\dcr{}a) == b)\oldtxt{ != false}}.
\end{itemize}
\end{addedblock}
...
\begin{codeblock}
  ...
  concept bool RandomAccessIterator@\newtxt{() \{}\oldtxt{ =}@
    @\newtxt{return}@ BidirectionalIterator<I>@\newtxt{()}@ &&
      TotallyOrdered<I>() &&
      Derived@\newtxt{From}@<IteratorCategory<I>, random_access_iterator_tag>@\newtxt{()}@ &&
      SizedIteratorRange<I, I>@\newtxt{()}@ && // see below
      requires (I i, @\newtxt{const }@I j, @\newtxt{const }@DifferenceType<I> n) {
        { i += n } -> Same<I&>;
        { @\oldtxt{i}\newtxt{j}@ + n } -> Same<I>;
        { n + @\oldtxt{i}\newtxt{j}@ } -> Same<I>;
        { i -= n } -> Same<I&>;
        { @\oldtxt{i}\newtxt{j}@ - n } -> Same<I>;
        @\newtxt{\{ j[n] \} -> const ValueType<I>\&;}@
      @\newtxt{\} \&\&}@
      (@\newtxt{!}\xname{MutableIterator}@<I> @\oldtxt{?}\newtxt{||}@
        requires(@\newtxt{const }@I i, @\oldtxt{int}\newtxt{const DifferenceType<I>}@ n) { i[n] = *i; *i = i[n]; }@\oldtxt{ :}\newtxt{);}@
        @\oldtxt{requires(I i, int n) \{ \{ i[n] \} -> const ValueType<I>\&; \})}@
      @\oldtxt{\};}@
  @\newtxt{\}}@
\end{codeblock}
\end{addedblock}
...
\oldtxt{\tcode{I} models} \tcode{RandomAccessIterator\newtxt{<I>()}} \newtxt{is satisfied} if and
only if:
...
concept~(\ref{weaklyincrementable.iterators}), \tcode{Readable} is the name of a type that
\oldtxt{models}\newtxt{satisfies} the Readable concept~(\ref{readable.iterators}), and
\tcode{WeakInputIterator} is the name of a type that \oldtxt{models}\newtxt{satisfies} the
WeakInputIterator~(\ref{weakinput.iterators}) concept}, the types
...
concept bool Range@\newtxt{() \{}\oldtxt{ =}@
  @\newtxt{return}@ requires(T@\newtxt{\&}@ t) {
    ...
    { begin(t) } -> @\newtxt{Same<}@IteratorType<T>@\newtxt{>}@; @\newtxt{// not \newtxt{required to be} equality preserving}@
    { end(t) } -> @\newtxt{Same<}@SentinelType<T>@\newtxt{>}@; @\newtxt{// not \newtxt{required to be} equality preserving}@
...
\pnum
\newtxt{Given an object \tcode{t} of type \tcode{T}, \oldtxt{the type \tcode{T} models}
\tcode{Range\newtxt{<T>()}} \newtxt{is satisfied} if and only if}

\begin{itemize}
\item \newtxt{Both \tcode{begin(t)} and \tcode{end(t)} are amortized constant time
and non-modifying. (\tcode{begin(t)} and \tcode{end(t)} do not require
implicit expression variants.)}
\item \newtxt{Either \tcode{SizedRange<T>()} is satisfied, \tcode{T} does not meet
the syntactic requirements of \tcode{SizedRange}, or \tcode{is_sized_range<T>::value}
is \tcode{false}~(\ref{sized.iterables}).}
\end{itemize}
\end{itemdescr}
...
@\newtxt{template <\xname{SizedRangeLike} R>}@
@\newtxt{struct is_sized_range;}@

template <class T>
concept bool SizedRange@\newtxt{() \{}\oldtxt{ =}@
  @\newtxt{return Range<T>() \&\&}@
    @\xname{SizedRangeLike}@<T> &&
    is_sized_range<T>::value;
@\newtxt{\}}@
\end{codeblock}

\pnum
\newtxt{Given an object \tcode{t} of type \tcode{T}, \tcode{SizedRange<T>()} is satisfied if and
only if \tcode{size(t)} returns the number of elements in \tcode{t}.}

\pnum
For a\oldtxt{ny} type \tcode{T} \newtxt{that satisfies \tcode{\xname{SizedRangeLike<T>}}},
\tcode{is_sized_range<T>} derives from \tcode{true_type} if \oldtxt{\tcode{T} models
\tcode{\xname{SizedRangeLike}}} \newtxt{\tcode{Same<T, remove\_cv\_t<remove\_reference\_t<T>{}>{}>()} is satisfied}, and \oldtxt{\tcode{false_type}} \newtxt{\tcode{is_sized_range<remove_cv_t<remove_reference_t<T>{}>{}>}} otherwise.

\pnum
Users are free to specialize \tcode{is_sized_range}.
\enternote \newtxt{As indicated in the requirements for the \tcode{Range}
concept above,} users \oldtxt{may want to}\newtxt{must} specialize
\tcode{is_sized_range} \newtxt{so that its member \tcode{value} is \tcode{false}}
to override the default \oldtxt{in the case of accidental conformance}
\newtxt{for \tcode{Range} types that satisfy \tcode{\xname{SizedRangeLike}} but
do not satisfy the additional semantic requirements of \tcode{SizedRange}}. \exitnote
...
template <@\oldtxt{class}\newtxt{\xname{SizedRangeLike}}@ R>
struct is_sized_range : @\oldtxt{false_type}\newtxt{is_sized_range<\xname{uncvref}<R>{}>}@ { };
...
template <Range T>
  requires Derived@\newtxt{From}@<remove_reference_t<T>, view_base>@\newtxt{()}@ ||
...
\enternote The standard containers~(\cxxref{containers}) \oldtxt{are models of}
...
for which \tcode{begin} returns a \oldtxt{model of}\newtxt{type that satisfies}
