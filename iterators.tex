...
are valid, \tcode{i} is said to be \techterm{incrementable}.} T\oldtxt{hen t}he type
\tcode{I} models \tcode{WeaklyIncrementable} if and only if
...
do\oldtxt{es} not model \tcode{WeaklyIncrementable}. \exitnote
...
\tcode{n} applications of \tcode{++a}, \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}}. Then \tcode{I} models
...
\tcode{n} applications of \tcode{++a}, \tcode{\newtxt{bool}(a == b)\oldtxt{ != false}}. Then types \tcode{I} and
...
\pnum
\newtxt{Given an object \tcode{t} of type \tcode{T}, the type \tcode{T} models
\tcode{Range} if and only if}
...
\item \newtxt{Either \tcode{T} models \tcode{SizedRange}, \tcode{T} does not meet
...
\pnum
\newtxt{Given an object \tcode{t} of type \tcode{T}, the type \tcode{T} models
\tcode{SizedRange} if and only if \tcode{size(t)} returns the number of elements
in \tcode{t}.}

\pnum
For any type \tcode{T}, \tcode{is_sized_range<T>} derives from
\tcode{true_type} if \tcode{T} \oldtxt{models}\newtxt{meets the syntactic requirements of} \tcode{\xname{SizedRangeLike}},
