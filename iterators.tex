...
\rSec2[weakiterator.iterators]{Concept WeakIterator}
...
sequence~(\ref{counted.iterators}).
}
...
elements of the same sequence~(\ref{counted.iterators}).
}
...
\pnum
The \tcode{OutputRange} concept describes requirements of an \tcode{Range} type
for which \tcode{begin} returns a type that satisfies
\tcode{OutputIterator}~(\ref{output.iterators}).
