...
\begin{codeblock}
  ...
  concept bool RandomAccessIterator() {
    return BidirectionalIterator<I>() &&
      ...
      SizedIteratorRange<I, I>() && // (\ref{iteratorranges.sizediteratorrange})
...
\newtxt{\enternote The semantics of \tcode{end}~(\ref{iterator.range.end})
ensure that for an expression \tcode{E}, \tcode{begin(E)} is valid whenever
\tcode{end(E)} is valid, and the types of the two expressions satisfy
\tcode{Iterator} and \tcode{Sentinel}. \exitnote}
...
\item
  ...
  \tcode{Iterator<I>()} is not satisfied, the program is ill-formed
...
\item
  ...
  \tcode{Sentinel<S, decltype(ranges::begin(E)>()}. If
...
\item
  ...
  syntactic requirements of
  \tcode{Sentinel<S, decltype(ranges::begin(E))>()} with overload
  resolution performed in a context that includes the declaration
...
types of \tcode{ranges::end(E)} and \tcode{ranges::begin(E)} satisfy
\tcode{Sentinel}.
...
object~(\ref{customization.point.object}). The effect of the expression
\tcode{ranges::cbegin(E)} for some expression \tcode{E} of type \tcode{T}
is equivalent to \tcode{ranges::begin((const T\&)(E))}.
...
\item
  ...
  syntactic requirements of
  \tcode{Sentinel<S, decltype(ranges::rbegin(E))>()}. If
  \tcode{Sentinel} is not satisfied, the program is ill-formed with
  no diagnostic required.
...
object~(\ref{customization.point.object}). The effect of the expression
\tcode{ranges::crbegin(E)} for some expression \tcode{E} of type \tcode{T}
is equivalent to \tcode{ranges::rbegin((const T\&)(E))}.
...
\item
  ...
  a declaration of \tcode{ranges::size}.
...
\item
  ...
  syntactic requirements of
  \tcode{SizedIteratorRange<I, S>()}~(\ref{iteratorranges.sizediteratorrange}).
  If \tcode{SizedIteratorRange} is not satisfied, the program is
  ill-formed with no diagnostic required.
...
\item
  ...
  similarly to \tcode{std::data(E)} as defined in the C++ Working
  Paper when \tcode{E} is an rvalue. \exitnote

\item
  Otherwise, \tcode{(E) + 0} if \tcode{E} has array type~(\cxxref{basic.compound}).
