...
\pnum
Let \tcode{v} be an rvalue or a lvalue of type (possibly \tcode{const}) \tcode{T}, and let \tcode{o} be an
object of type \tcode{Out}. Then types \tcode{T} and \tcode{Out} model \tcode{MoveWritable} if and
only if

\begin{itemize}
\item If \tcode{o} is dereferenceable, then after the assignment \tcode{*o = move(v)}, the value
referred to by \tcode{*o} is equal to the value of \tcode{v} before the assignment.
\end{itemize}
...
\begin{itemize}
\item After the assignment \tcode{*o = v}, the value referred to by \tcode{*o} is equal to
the value of \tcode{v} and \tcode{v} is unchanged.
\end{itemize}
