...
\begin{itemize}
\item If \tcode{o} is dereferencable, then after the assignment \tcode{*o = move(v)}, the value
referred to by \tcode{*o} is equal to the value of \tcode{v} before the assignment.
\end{itemize}

\pnum
After the expression \tcode{*o = move(v)}, object \tcode{o} is not required to be dereferencable.
...
\begin{codeblock}
  ...
  concept bool IndirectlySwappable =
    ...
    Swappable<ReferenceType<I1>> &&
    Swappable<ReferenceType<I2>>;
\end{codeblock}
...
\pnum
The relation \tcode{R} must represent a strict weak ordering.
...
\ednote{REVIEW: Re-specify this in terms of a Container concept? Or Iterable? Or leave it?}
\begin{codeblock}
namespace std {
  template <class Container>
...
requirements on an \tcode{Iterable} type with constant-time copy and assign
operations.
...
\pnum
The \tcode{SizedIterable} concept describes the requirements of an \tcode{Iterable}
type that knows its size in constant time with the \tcode{size} function.

\begin{codeblock}
// \expos
template <class T>
concept bool @\xname{SizedIterableLike}@ =
  Iterable<T> &&
  requires(T t) {
    { size(t) } -> Integral;
...
concept bool SizedIterable =
  @\xname{SizedIterableLike}@<T> && is_sized_iterable<T>::value;
\end{codeblock}

\pnum
For any type \tcode{T}, \tcode{is_sized_iterable<T>} derives from
\tcode{true_type} if \tcode{T} models \tcode{\xname{SizedIterableLike}},
and \tcode{false_type} otherwise.

\pnum
Users are free to specialize \tcode{is_sized_iterable}. \enternote Users
may specialize \tcode{is_sized_iterable} to override the default in the
case of accidental conformance.\exitnote

\pnum
\enternote A possible implementation for \tcode{is_sized_iterable} is given below:

\begin{codeblock}
template <class R>
struct is_sized_iterable : false_type { };

template <@\xname{SizedIterableLike}@ R>
struct is_sized_iterable<R> : true_type { };
\end{codeblock}
\exitnote
...
\pnum
The \tcode{Range} concept describes the requirements of an \tcode{Iterable} type that
...
\pnum
\enterexample
Examples of \tcode{Range}s are:

\begin{itemize}
\item An \tcode{Iterable} type that wraps a pair of iterators.

\item An \tcode{Iterable} type that hold its elements by \tcode{shared_ptr}
and shares ownership with all its copies.

\item An \tcode{Iterable} type that generates its elements on demand.
\end{itemize}

A container~(\cxxref{containers}) is not a \tcode{Range} since copying the
...
\pnum
Since the difference between \tcode{Iterable} and \tcode{Range} is largely semantic, the
...
following heuristics to determine whether an \tcode{Iterable} type \tcode{T} is a \tcode{Range}:
...
template <Iterable T>
  requires Derived<remove_reference_t<T>, range_base> ||
    !@\xname{ContainerLike}@<remove_reference_t<T>>
struct is_range<T> : true_type { };
\end{codeblock}
\ednote{REVIEW: Will this create ambiguities with user specializations?}
\exitnote

\rSec3[bounded.iterables]{Bounded iterables}

\pnum
The \tcode{BoundedIterable} concept describes requirements of an \tcode{Iterable} type
...
\pnum
The \tcode{InputIterable} concept describes requirements of an \tcode{Iterable} type
...
\tcode{InputIterable} type for which \tcode{begin} returns a model of
...
\tcode{ForwardIterable} type for which \tcode{begin} returns a model of
...
\tcode{BidirectionalIterable} type for which \tcode{begin} returns a model of
