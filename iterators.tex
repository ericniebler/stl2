...
A
\term{sentinel}
is an abstraction of a past-the-end iterator. \oldoldtxt{Sentinels
are \tcode{Regular} types that can be used to denote the end of
a range.} \oldtxt{A sentinel and an iterator denoting} \newtxt{An iterator
and a sentinel denoting a range} \newtxt{are comparable}\oldtxt{shall
be \tcode{EqualityComparable}}.
...
\begin{codeblock}
  ...
  concept bool @\oldtxt{Move}@Writable() {
    return Semiregular<Out>() &&
      requires(Out o, T@\newtxt{\&\&}@ \oldtxt{v}\newtxt{e}) {
        *o = std::@\oldtxt{move}\newtxt{forward<T>}@(\oldtxt{v}\newtxt{e}); // not required to be equality preserving
      };
  }
\end{codeblock}

\pnum
Let \oldtxt{\tcode{v}}\newtxt{\tcode{e}} be \oldtxt{an rvalue or a lvalue of type (possibly \tcode{const})} \newtxt{an expression such that \tcode{decltype(e)} is} \tcode{T}, and let \tcode{o}
...
to the value of \oldtxt{\tcode{v}}\newtxt{\tcode{e}} before the assignment.
\end{itemize}
...
\pnum
\oldoldtxt{\tcode{v}'s state}\newtxt{If \tcode{e}}\oldtxt\tcode{v}} \newtxt{is an xvalue~(\cxxref{basic.lval}), the resulting
...
\ednote{``If \oldtxt{\tcode{v}}\newtxt{e} is an xvalue'' suffices for now, but incorporating P0022's proxy
...
\oldtxt{~({\ednote{This concept also gives us a way to demote the category of \tcode{move_iterator}s to Input
...
\begin{codeblock}
  ...
  template <class T>
  using @\oldtxt{FunctionType}as_function_t@ =
