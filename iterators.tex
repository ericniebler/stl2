...
namespace std {
  ...
  struct iterator_traits<Out> {
    using difference_type   = experimental::ranges_v1::@\oldtxt{DifferenceType}\newtxt{difference_type_t}@<Out>;
...
\begin{codeblock}
  ...
  struct iterator_traits<In> {
    ...
    using value_type        = experimental::ranges_v1::@\oldtxt{ValueType}\newtxt{value_type_t}@<In>;
    using reference         = @\seebelow@;
    using pointer           = @\seebelow@;
...
\item Otherwise, \tcode{std::iterator_traits<In>::reference} is
      \tcode{experimental::ranges_v1::\oldtxt{ReferenceType}\newtxt{reference_t}<In>}.
\end{itemize}
...
\pnum
Let type \tcode{C} be \tcode{experimental::ranges_v1::}\tcode{\oldtxt{IteratorCategory}}\tcode{\newtxt{iterator_category_t}}\tcode{<In>}.
...
\item If \tcode{C} is the same as or inherits from \tcode{std::input_iterator_tag} or
      ...
      is \tcode{C}.
\item If \tcode{experimental::ranges_v1::\oldtxt{ReferenceType}\newtxt{reference_t}<In>} is not a reference type,
