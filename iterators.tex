...
types \tcode{\removed{iterator_traits<X>::difference_type}}\tcode{\oldtxt{DifferenceType<X>}}\tcode{\newtxt{difference_type_t<X>}} and
\tcode{\removed{iterator_traits<X>::ref\-erence}}~\tcode{\oldtxt{ReferenceType<X>}}\tcode{\newtxt{reference_t<X>}}, respectively,
\tcode{n}
denotes a value of
...
\tcode{\oldtxt{DifferenceType<X>}}\tcode{\newtxt{difference_type_t<X>}} and
\tcode{\oldtxt{ReferenceType<X>}}\tcode{\newtxt{reference_t<X>}}
must be well-formed}. \exitnote
...
\newtxt{such that \tcode{T::value_type} is valid and denotes a type},
...
\begin{codeblock}
  ...
  concept bool IndirectlyMovable() {
    return Readable<I>() && MoveWritable<Out, @\oldtxt{ValueType}\newtxt{value_type_t}@<I>>();
  }
\end{codeblock}
...
\begin{codeblock}
  ...
  concept bool IndirectlyCopyable() {
    return IndirectlyMovable<I, Out>() && Writable<Out, @\oldtxt{ValueType}\newtxt{value_type_t}@<I>>();
  }
\end{codeblock}
  ...
  concept bool WeakInputIterator() {
    return WeakIterator<I>() &&
      ...
      requires(I i, const I ci) {
        ...
        requires DerivedFrom<@\oldtxt{ValueType}\newtxt{value_type_t}@<I>, weak_input_iterator_tag>();
...
\begin{codeblock}
  ...
  concept bool InputIterator() {
    return WeakInputIterator<I>() &&
      Iterator<I>() &&
      DerivedFrom<@\oldtxt{ValueType}\newtxt{value_type_t}@<I>, input_iterator_tag>();
  }
\end{codeblock}
...
\term{single pass}
algorithms.
\removed{Value type T}\added{\tcode{\oldtxt{ValueType}\newtxt{value_type_t}<I>}} is not required
to be a \removed{\tcode{CopyAssignable}}\added{\tcode{Copyable}}
...
\begin{codeblock}
  ...
  concept bool ForwardIterator() {
    return InputIterator<I>() &&
      Incrementable<I>() &&
      DerivedFrom<@\oldtxt{ValueType}\newtxt{value_type_t}@<I>, forward_iterator_tag>();
  }
\end{codeblock}
\end{addedblock}
...
\begin{codeblock}
  ...
  concept bool BidirectionalIterator() {
    return ForwardIterator<I>() &&
      DerivedFrom<@\oldtxt{ValueType}\newtxt{value_type_t}@<I>, bidirectional_iterator_tag>() &&
      requires (I i) {
        { @\dcr@i } -> Same<I&>;
        { i@\dcr@ } -> Same<I>;
      };
...
\begin{codeblock}
  ...
  concept bool RandomAccessIterator() {
    return BidirectionalIterator<I>() &&
      ...
      DerivedFrom<@\oldtxt{ValueType}\newtxt{value_type_t}@<I>, random_access_iterator_tag>() &&
      ...
      requires (I i, const I j, const @\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I> n) {
        ...
        { i -= n } -> Same<I&>;
        { j - n } -> Same<I>;
        { j[n] } -> const @\oldtxt{ValueType}\newtxt{value_type_t}@<I>&;
      } &&
      (!@\xname{MutableIterator}@<I> ||
        requires(const I i, const @\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I> n) { i[n] = *i; *i = i[n]; });
  }
\end{codeblock}
\end{addedblock}
...
\begin{codeblock}
  ...
  concept bool IndirectCallable() {
    return (Readable<Is>() && ...) &&
      Function<@\oldtxt{FunctionType}\newtxt{as_function_t}@<F>, @\oldtxt{ValueType}\newtxt{value_type_t}@<Is>...>();
  }

  template <class F, class...Is>
  concept bool IndirectRegularCallable() {
    return (Readable<Is>() && ...) &&
      RegularFunction<@\oldtxt{FunctionType}\newtxt{as_function_t}@<F>, @\oldtxt{ValueType}\newtxt{value_type_t}@<Is>...>();
  }

  template <class F, class...Is>
  concept bool IndirectCallablePredicate() {
    return (Readable<Is>() && ...) &&
      Predicate<@\oldtxt{FunctionType}\newtxt{as_function_t}@<F>, @\oldtxt{ValueType}\newtxt{value_type_t}@<Is>...>();
  }
  ...
  concept bool IndirectCallableRelation() {
    return Readable<I1>() &&
      Readable<I2>() &&
      Relation<@\oldtxt{FunctionType}\newtxt{as_function_t}@<F>, @\oldtxt{ValueType}\newtxt{value_type_t}@<I1>, @\oldtxt{ValueType}\newtxt{value_type_t}@<I2>>();
  }
  ...
  concept bool IndirectCallableStrictWeakOrder() {
    return Readable<I1>() &&
      Readable<I2>() &&
      StrictWeakOrder<@\oldtxt{FunctionType}\newtxt{as_function_t}@<F>, @\oldtxt{ValueType}\newtxt{value_type_t}@<I1>, @\oldtxt{ValueType}\newtxt{value_type_t}@<I2>>();
  }
  ...
  @\newtxt{struct indirect_result_of<F(Is...)> :}@
    @\newtxt{result_of<as_function_t<F>(value_type_t<Is>...)> \{ \};}@
...
\begin{codeblock}
  template <Readable I, IndirectRegularCallable<I> Proj>
    requires RegularFunction<@\oldtxt{FunctionType}\newtxt{as_function_t}@<Proj@\newtxt{\&}@>, @\oldtxt{ValueType}\newtxt{reference_t}@<I>>
  struct @\oldtxt{P}\newtxt{p}@rojected {
    ...
    @\newtxt{using value_type = decay_t<result_of_t<as_function_t<Proj\&>(value_type_t<I>)>{}>;}@
    @\newtxt{result_of_t<as_function_t<Proj\&>(reference_t<I>)> operator*() const;}@
  };
...
for rearrangements: \tcode{Permutable}, \tcode{Mergeable}, \oldtxt{\tcode{MergeMovable}}\newtxt{\tcode{MoveMergeable}}, and \tcode{Sortable}.
...
\rSec2[commonalgoreq.mergemovable]{Concept MoveMergeable}

\pnum
The \oldtxt{\tcode{MergeMovable}}\newtxt{\tcode{MoveMergeable}} concept specifies the requirements of
algorithms that merge sorted sequences into an output sequence by moving elements.

\indexlibrary{\idxcode{MoveMergeable}}%
\begin{codeblock}
  template <class I1, class I2, class Out,
      class R = less<>, class P1 = identity, class P2 = identity>
  concept bool @\oldtxt{MergeMovable}\newtxt{MoveMergeable}@() {
    return InputIterator<I1>() &&
      InputIterator<I2>() &&
      WeaklyIncrementable<Out>() &&
...
\pnum
The \tcode{SizedIteratorRange} concept specifies the
  ...
  concept bool SizedIteratorRange() {
    return Sentinel<S, I>() &&
      requires (const I i, const S j) {
        { i - i } -> @\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I>;
        { j - j } -> @\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I>;
        { i - j } -> @\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I>;
        { j - i } -> @\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I>;
      };
  }
\end{codeblock}
...
\tcode{S}. Let \tcode{n} be the smallest value of type
\tcode{\oldtxt{DifferenceType<I>}}\tcode{\newtxt{difference_type_t<I>}} such that after
\tcode{n} applications of \tcode{++a},\newtxt{ then} \tcode{bool(a == b)}. Then
\tcode{SizedIteratorRange<I, S>()} is satisfied if and only if:
...
\enternote The \tcode{SizedIteratorRange} concept is satisfied by pairs of
...
\begin{codeblock}
  ...
  template <class WeaklyIncrementable> using @\oldtxt{DifferenceType}\newtxt{difference_type_t}@
    = typename difference_type<WeaklyIncrementable>::type;
  template <class Readable> using @\oldtxt{ValueType}\newtxt{value_type_t}@
    = typename value_type<Readable>::type;
  template <class WeakInputIterator> using @\oldtxt{ValueType}\newtxt{value_type_t}@
...
\begin{codeblock}
  ...
  concept bool @\xname{WeaklyEqualityComparable}@ = @\seebelow@;  // \expos
  template<class S, class I>
  concept bool @\xname{WeakSentinel}@ = @\seebelow@;              // \expos

  template <@\oldtxt{Input}@Iterator I, @\xname{WeakSentinel}@<I> S> class common_iterator;

  @\newtxt{template <InputIterator I, class S>}@
  @\newtxt{struct iterator_category<common_iterator<I, S>{}>;}@

  template <@\oldtxt{InputIterator}\newtxt{class}@ I1, @\oldtxt{\xname{WeakSentinel}<I1>}\newtxt{class}@ S1,
            @\oldtxt{InputIterator}\newtxt{class}@ I2, @\oldtxt{\xname{WeakSentinel}<I2>}\newtxt{class}@ S2>
  ...
  template <@\oldtxt{InputIterator}\newtxt{class}@ I1, @\oldtxt{\xname{WeakSentinel}<I1>}\newtxt{class}@ S1,
    ...
    requires SizedIteratorRange<I1, I1>() && SizedIteratorRange<I2, I2>() &&
      requires (const I1 i1, const S1 s1, const I2 i2, const S2 s2) {
        {i1-i2}->@\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I2>; {i2-i1}->@\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I2>;
        {i1-s2}->@\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I2>; {s2-i1}->@\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I2>;
        {i2-s1}->@\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I2>; {s1-i2}->@\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I2>;
      }
  @\oldtxt{DifferenceType}\newtxt{difference_type_t}@<I2> operator-(
    const common_iterator<I1, S1>& x, const common_iterator<I2, S2>& y);
...
\begin{codeblock}
  ...
  @\added{using std::begin;}@
  @\added{using std::end;}@
  @\added{template <class>}@
    @\added{concept bool _Auto = true; // \expos}@
  template <@\changed{class}{_Auto}@ C> constexpr auto cbegin(const C& c) noexcept(noexcept(@\removed{std::}@begin(c)))
    -> decltype(@\removed{std::}@begin(c));
  template <@\changed{class}{_Auto}@ C> constexpr auto cend(const C& c) noexcept(noexcept(@\removed{std::}@end(c)))
    -> decltype(@\removed{std::}@end(c));
  template <@\changed{class}{_Auto}@ C> auto rbegin(C& c) -> decltype(c.rbegin());
  template <@\changed{class}{_Auto}@ C> auto rbegin(const C& c) -> decltype(c.rbegin());
  template <@\changed{class}{_Auto}@ C> auto rend(C& c) -> decltype(c.rend());
  template <@\changed{class}{_Auto}@ C> auto rend(const C& c) -> decltype(c.rend());
  template <@\changed{class}{_Auto}@ T, size_t N> reverse_iterator<T*> rbegin(T (&array)[N]);
  template <@\changed{class}{_Auto}@ T, size_t N> reverse_iterator<T*> rend(T (&array)[N]);
  template <@\changed{class}{_Auto}@ E> reverse_iterator<const E*> rbegin(initializer_list<E> il);
  template <@\changed{class}{_Auto}@ E> reverse_iterator<const E*> rend(initializer_list<E> il);
  template <@\changed{class}{_Auto}@ C> auto crbegin(const C& c) -> decltype(@\removed{std}\added{ranges\oldtxt{_v1}}@::rbegin(c));
  template <@\changed{class}{_Auto}@ C> auto crend(const C& c) -> decltype(@\removed{std}\added{ranges\oldtxt{_v1}}@::rend(c));
\end{codeblock}
\begin{addedblock}
\begin{codeblock}
  template <@\oldtxt{class}\newtxt{_Auto}@ C> auto size(const C& c) -> decltype(c.size());
  template <@\oldtxt{class}\newtxt{_Auto}@ T, size_t N> constexpr size_t size(T (&array)[N]) noexcept;
  template <@\oldtxt{class}\newtxt{_Auto}@ E> @\newtxt{constexpr}@ size_t size(initializer_list<E> il) noexcept;
...
namespace std {
  ...
  template <experimental::ranges@\oldtxt{_v1}@::WeakIterator I>
    struct iterator_traits;
  template <experimental::ranges@\oldtxt{_v1}@::WeakInputIterator I>
    struct iterator_traits;
  ...
  template<experimental::ranges@\oldtxt{_v1}@::WeakIterator I>
    struct common_type<experimental::ranges@\oldtxt{_v1}@::counted_iterator<I>,
                       experimental::ranges@\oldtxt{_v1}@::default_sentinel>;
  template<experimental::ranges@\oldtxt{_v1}@::WeakIterator I>
    struct common_type<experimental::ranges@\oldtxt{_v1}@::default_sentinel,
                       experimental::ranges@\oldtxt{_v1}@::counted_iterator<I>>;

  // \ref{unreachable.traits.specializations} \tcode{common_type} specializations
  template<experimental::ranges@\oldtxt{_v1}@::Iterator I>
    struct common_type<I, experimental::ranges@\oldtxt{_v1}@::unreachable>;
  template<experimental::ranges@\oldtxt{_v1}@::Iterator I>
    struct common_type<experimental::ranges@\oldtxt{_v1}@::unreachable, I>;
}
\end{codeblock}
...
\pnum
\enternote The use of the exposition only \tcode{_Auto} concept in the above signatures
is to break a tie in overload resolution between similarly named functions in the \tcode{std}
and \tcode{std::experimental::v1::ranges} namespaces. Since the functions that use
\tcode{_Auto} are constrained, they are preferred to the unconstrained versions in namespace
\tcode{std}.\exitnote
...
\oldtxt{\tcode{WeakInputIterator}}\newtxt{\tcode{WII}} is the name of a type that satisfies the
\tcode{WeakInputIterator} concept~(\ref{iterators.weakinput}) concept}, the types
...
@\oldtxt{ValueType<WeakInputIterator>}\newtxt{value_type_t<WII>}@
\end{codeblock}
\end{addedblock}
...
\begin{codeblock}
namespace std {
  template <experimental::ranges@\oldtxt{_v1}@::WeakIterator Out>
...
\begin{codeblock}
  template <experimental::ranges@\oldtxt{_v1}@::WeakInputIterator WeakIn>
...
\item If \oldtxt{type \tcode{In} has a nested type \tcode{reference}}
      \newtxt{\tcode{In::value_type} is valid and denotes a type}, then
...
other part of this \oldtxt{standard}\newtxt{document}.\exitnote
}
...
namespace std { @\added{namespace experimental \{ namespace ranges\oldtxt{_v1} \newtxt{\{ inline namespace v1} \{}@
  struct output_iterator_tag { };
  @\added{struct weak_input_iterator_tag \{ \};}@
  struct input_iterator_tag@\added{ : \oldtxt{public} weak_input_iterator_tag}@ { };
...
types that satisfy \tcode{SizedIteratorRange} provide the} \tcode{-}
...
time for them); for \newnewtxt{weak output, weak input}, input, forward and bidirectional iterators they use
\tcode{++}
...
\pnum
\requires
\tcode{bound} shall be reachable from \tcode{i}\newtxt{ or \tcode{i} shall
be reachable from \tcode{bound} and \tcode{is_assignable<I \&, S \&\&>::value}
is \tcode{true}}.
...
\item If \oldtxt{\tcode{I} and \tcode{S} are the same type, this function
      is constant time}\newtxt{\tcode{is_assignable<I \&, S \&\&>::value} is \tcode{true}
      this function is equivalent to \tcode{i = std::move(bound)}}.

\item \oldtxt{If}\newtxt{Otherwise, if} \tcode{SizedIteratorRange<I, S>()} is satisfied,
      this function \oldtxt{shall dispatch to}\newtxt{is equivalent to}
      \tcode{advance(i, bound - i)}.

\item \newtxt{Otherwise, this function i}\oldtxt{I}ncrements iterator\oldoldtxt{ reference}
      \tcode{i} until \tcode{i == bound}.
\end{itemize}
\end{itemdescr}
...
\effects
\begin{itemize}
\item If \tcode{SizedIteratorRange<I, S>()} is satisfied:
      \begin{itemize}
      \item If \tcode{(0 <= n ? n >= $D$ : n <= $D$)} is \tcode{true}, where $D$ is \tcode{bound - i},
            this function \oldtxt{dispatches}\newtxt{is equivalent} to \tcode{advance(i, bound)},
      \item Otherwise, this function \oldtxt{dispatches}\newtxt{is equivalent} to \tcode{advance(i, n)}.
      \end{itemize}
...
\pnum
\effects
If \changed{\tcode{InputIterator} meets the requirements of random access iterator}{
\tcode{SizedIteratorRange<I, S>()} is satisfied}, returns \tcode{(last - first)}; otherwise,
...
\requires
If \changed{\tcode{InputIterator} meets the requirements of random access iterator}{
\tcode{SizedIteratorRange<I, S>()} is satisfied} \tcode{last} shall be reachable from
...
\end{codeblock}\end{addedblock}\begin{codeblock}
    ...
    template <@\changed{class}{BidirectionalIterator}@ U>
      @\added{requires ConvertibleTo<U, I>()}@
...
\pnum
\effects
Assigns \tcode{u.\oldoldtxt{base()}\newnewtxt{current}} to \tcode{current}.
...
\begin{itemdescr}
\pnum
\returns \changed{\tcode{std::addressof(operator*())}}{\tcode{prev(current)}}.
\end{itemdescr}
...
namespace std { @\added{namespace experimental \{ namespace ranges\oldtxt{_v1} \newtxt{\{ inline namespace v1} \{}@
  ...
  public:
    ...
    @\newnewtxt{const}@ back_insert_iterator@\oldoldtxt{<Container>}@&
      operator=(const typename Container::value_type& value) @\newnewtxt{const}@;
    @\newnewtxt{const}@ back_insert_iterator@\oldoldtxt{<Container>}@&
      operator=(typename Container::value_type&& value) @\newnewtxt{const}@;

    @\newnewtxt{const}@ back_insert_iterator@\oldoldtxt{<Container>}@& operator*() @\newnewtxt{const}@;
    @\newnewtxt{const}@ back_insert_iterator@\oldoldtxt{<Container>}@& operator++() @\newnewtxt{const}@;
    @\newnewtxt{const}@ back_insert_iterator@\oldoldtxt{<Container>}\newnewtxt{\&}@ operator++(int) @\newnewtxt{const}@;
  };
...
namespace std { @\added{namespace experimental \{ namespace ranges\oldtxt{_v1} \newtxt{\{ inline namespace v1} \{}@
  ...
  public:
    ...
    @\newnewtxt{const}@ front_insert_iterator@\oldoldtxt{<Container>}@&
      operator=(const typename Container::value_type& value) @\newnewtxt{const}@;
    @\newnewtxt{const}@ front_insert_iterator@\oldoldtxt{<Container>}@&
      operator=(typename Container::value_type&& value) @\newnewtxt{const}@;

    @\newnewtxt{const}@ front_insert_iterator@\oldoldtxt{<Container>}@& operator*() @\newnewtxt{const}@;
    @\newnewtxt{const}@ front_insert_iterator@\oldoldtxt{<Container>}@& operator++() @\newnewtxt{const}@;
    @\newnewtxt{const}@ front_insert_iterator@\oldoldtxt{<Container>}\newnewtxt{\&}@ operator++(int) @\newnewtxt{const}@;
  };
...
{\color{newclr}
\pnum
If \tcode{is_base_of<input_iterator_tag, iterator_category_t<I>{}>::value}
is \tcode{true}, the template specialization \tcode{move_iterator<I>} shall define the nested
type named \tcode{iterator_category} as a synonym for \tcode{input_iterator_tag}, otherwise
as a synonym for \tcode{weak_input_iterator_tag}.
...
namespace std { namespace experimental { namespace ranges@\oldtxt{_v1} \newtxt{\{ inline namespace v1}@ {
  ...
  template <@\oldtxt{Input}@Iterator I, @\xname{WeakSentinel}@<I> S>
    requires !Same<I, S>()
  ...
  public:
    ...
    common_iterator(I i);
    common_iterator(S s);
    template <@\oldtxt{InputIterator}\newtxt{class}@ U, @\oldtxt{\xname{WeakSentinel}<U>}\newtxt{class}@ V>
...
{\color{newclr}
\pnum
For a type \tcode{I} that satisfies \tcode{InputIterator} and a type \tcode{S}, the partial
specialization \tcode{iterator_category<common_iterator<I, S>{}>} shall define a nested
type named \tcode{type} as follows:
\begin{itemize}
\item If \tcode{I} satisfies \tcode{ForwardIterator}, then \tcode{iterator_category<common_iterator<I, S>{}>::type}
      shall be a synonym for \tcode{forward_iterator_tag}.
\item Otherwise, \tcode{iterator_category<common_iterator<I, S>{}>::type} shall be a synonym for
      \tcode{input_iterator_tag}.
\end{itemize}
}
...
\pnum
\effects Constructs a \tcode{counted_iterator}, initializing
\tcode{current} with \tcode{u.\oldtxt{base()}\newtxt{current}} and \tcode{cnt} with \tcode{u.\oldtxt{count()}\newtxt{cnt}}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Assigns \tcode{u.\oldtxt{base()}\newtxt{current}} to
\tcode{current} and \tcode{u.\oldtxt{count()}\newtxt{cnt}} to \tcode{cnt}.
...
namespace std {
  ...
  struct common_type<experimental::ranges@\oldtxt{_v1}@::counted_iterator<I>,
                     experimental::ranges@\oldtxt{_v1}@::default_sentinel> {
    using type = experimental::ranges@\oldtxt{_v1}@::common_iterator<
      experimental::ranges@\oldtxt{_v1}@::counted_iterator<I>,
      experimental::ranges@\oldtxt{_v1}@::default_sentinel>;
  };
  template<experimental::ranges@\oldtxt{_v1}@::WeakIterator I>
...
namespace std {
  template<experimental::ranges@\oldtxt{_v1}@::Iterator I>
  struct common_type<I, experimental::ranges@\oldtxt{_v1}@::unreachable> {
    using type = experimental::ranges@\oldtxt{_v1}@::common_iterator<I, experimental::ranges@\oldtxt{_v1}@::unreachable>;
  };
  template<experimental::ranges@\oldtxt{_v1}@::Iterator I>
  struct common_type<experimental::ranges@\oldtxt{_v1}@::unreachable, I> {
    using type = experimental::ranges@\oldtxt{_v1}@::common_iterator<I, experimental::ranges@\oldtxt{_v1}@::unreachable>;
...
namespace std { @\added{namespace experimental \{ namespace ranges\oldtxt{_v1} \newtxt{\{ inline namespace v1} \{}@
  ...
  public:
   ...
   ~ostream_iterator();
    @\newnewtxt{const}@ ostream_iterator@\oldoldtxt{<T, charT, traits>}@& operator=(const T& value) @\newnewtxt{const}@;

    @\newnewtxt{const}@ ostream_iterator@\oldoldtxt{<T, charT, traits>}@& operator*() @\newnewtxt{const}@;
    @\newnewtxt{const}@ ostream_iterator@\oldoldtxt{<T, charT, traits>}@& operator++() @\newnewtxt{const}@;
    @\newnewtxt{const}@ ostream_iterator@\oldoldtxt{<T, charT, traits>}@& operator++(int) @\newnewtxt{const}@;
  private:
...
\indexlibrary{\idxcode{ostream_iterator}!\idxcode{operator*}}%
\begin{itemdecl}
@\newtxt{const}@ ostream_iterator& operator*() @\newtxt{const}@;
\end{itemdecl}
...
\begin{itemdecl}
@\newtxt{const}@ ostream_iterator& operator++() @\newtxt{const}@;
@\newtxt{const}@ ostream_iterator& operator++(int) @\newtxt{const}@;
\end{itemdecl}
...
namespace std { @\newtxt{namespace experimental \{ namespace ranges\oldtxt{_v1} \{ inline namespace v1 \{}@
  ...
  class ostreambuf_iterator @\removed{:}@
    @\removed{public iterator<output_iterator_tag, void, void, void, void>}@ {
  public:
    @\added{typedef output_iterator_tag}@            @\added{iterator_category;}@
  ...
  public:
    ...
    @\newnewtxt{const}@ ostreambuf_iterator& operator=(charT c) @\newnewtxt{const}@;

    @\newnewtxt{const}@ ostreambuf_iterator& operator*() @\newnewtxt{const}@;
    @\newnewtxt{const}@ ostreambuf_iterator& operator++() @\newnewtxt{const}@;
    @\newnewtxt{const}@ ostreambuf_iterator& operator++(int) @\newnewtxt{const}@;
...
\begin{itemdecl}
@\newnewtxt{const}@ ostreambuf_iterator@\oldoldtxt{<charT, traits>}@&
  operator=(charT c) @\newnewtxt{const}@;
\end{itemdecl}
...
\indexlibrary{\idxcode{operator*}!\idxcode{ostreambuf_iterator}}%
\begin{itemdecl}
@\newnewtxt{const}@ ostreambuf_iterator@\oldoldtxt{<charT, traits>}@& operator*() @\newnewtxt{const}@;
\end{itemdecl}
...
\begin{itemdecl}
@\newnewtxt{const}@ ostreambuf_iterator@\oldoldtxt{<charT, traits>}@& operator++() @\newnewtxt{const}@;
@\newnewtxt{const}@ ostreambuf_iterator@\oldoldtxt{<charT, traits>}@& operator++(int) @\newnewtxt{const}@;
\end{itemdecl}
...
template <class T>
  ...
  @\newtxt{\}}@
using @\oldtxt{IteratorType}\newtxt{iterator_t}@ = decltype(begin(declval<T@\newtxt{\&}@>()));

template <class T>
  @\newtxt{requires requires (T\& t) \{}@
    @\newtxt{typename \oldtxt{IteratorType}\newtxt{iterator_t}<T>;}@
    @\newtxt{\{ end(t) \} -> Sentinel<\oldtxt{IteratorType}\newtxt{iterator_t}<T>{}>;}@
  @\newtxt{\}}@
using @\oldtxt{SentinelType}\newtxt{sentinel_t}@ = decltype(end(declval<T@\newtxt{\&}@>()));

template <class T>
concept bool Range() {
  return requires@\oldtxt{(T t)}@ {
    typename @\oldtxt{IteratorType}\newtxt{iterator_t}@<T>;
...
concept bool SizedRange() {
  ...
  @\newtxt{return !disable_sized_range<T>::value \&\&}@
    ...
    @\newtxt{requires (const remove_reference_t<T>\& t) \{}@
      ...
      @\newtxt{\{ size(t) \} -> ConvertibleTo<\oldtxt{DifferenceType}difference_type_t<\oldtxt{IteratorType}iterator_t<T>{}>{}>;}@
    @\newtxt{\};}@
@\newtxt{\}}@
\end{itemdecl}
...
concept bool @\xname{ContainerLike}@ =
  Range<T>() && @\newtxt{Range<const T>() \&\&}@
  !Same<@\oldtxt{ReferenceType}\newtxt{reference_t}@<@\oldtxt{IteratorType}\newtxt{iterator_t}@<T@\oldtxt{\&}@>>,
        @\oldtxt{ReferenceType}\newtxt{reference_t}@<@\oldtxt{IteratorType}\newtxt{iterator_t}@<const T@\oldtxt{\&}@>>>();
...
\begin{itemdescr}
\pnum \returns \tcode{ranges\oldtxt{_v1}::distance(begin(r), end(r))}
\end{itemdescr}
