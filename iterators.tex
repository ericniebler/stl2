...
\pnum
Since iterators are an abstraction of pointers, their semantics \removed{is}\added{are}
...
void reverse(@\changed{BidirectionalIterator}{I}@ first, @\changed{BidirectionalIterator}{I}@ last) {
  @\removed{typename iterator_traits<BidirectionalIterator>::difference_type}\added{difference_type_t<I>}@ n =
    distance(first, last);
  --n;
  while(n > 0) {
    @\removed{typename iterator_traits<BidirectionalIterator>::value_type}\added{value_type_t<I>}@
      tmp = *first;
    *first++ = *--last;
    *last = tmp;
    n -= 2;
...
namespace std { @\added{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  @\removed{protected}\added{private}@:
    Container* container;              // \expos
    typename Container::iterator iter; // \expos
