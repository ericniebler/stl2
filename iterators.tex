...
return an iterator and a sentinel or an iterator. The \tcode{SizedIterable}
concept refines \tcode{Iterable} but adds the requirement that the number of
elements in the iterable can be determined in constant time with the
\tcode{size} function. The \tcode{Range} concept describes requirements on an
iterable type with reference semantics to its elements.

\pnum
In addition to the three fundamental iterable categories, this standard defines
a number of convenience refinements of iterable that group together requirements
that turn up often in the concepts and range views. \techterm{Bounded iterables}
...
\tcode{reference_type}, \tcode{is_range<T>::value}
