...
time for them); for \added{output, }\oldtxt{weak input, }input, forward and bidirectional iterators they use
\tcode{++}
...
\tcode{SizedIteratorRange<I, S>()} is satisfied}\removed{ \tcode{last} shall be reachable from
...
\end{codeblock}\end{addedblock}\begin{codeblock}
  ...
  template <@\changed{class Iterator}{RandomAccessIterator I}@>
    reverse_iterator<@\changed{Iterator}{I}@>
      operator+(
    @\removed{typename reverse_iterator<Iterator>::difference_type}\oldtxt{DifferenceType<I>}\added{difference_type_t<I>}@ n,
    const reverse_iterator<@\changed{Iterator}{I}@>& x);
