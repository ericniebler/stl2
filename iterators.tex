...
\pnum
Since iterators are an abstraction of pointers, their semantics \removed{is}\newtxt{are}
...
\pnum
\removed{An iterator }\added{A }\added{sentinel}
\tcode{\removed{j}\added{s}}
is called
\term{reachable}
from an iterator
\tcode{i}
...
\tcode{++i}
that makes
\tcode{i == \removed{j}\added{s}}.
If
\tcode{\removed{j}\added{s}}
is reachable from
\tcode{i},
they \removed{refer to elements of the same sequence}\added{denote a range}.

\pnum
Most of the library's algorithmic templates that operate on data structures have interfaces that use ranges.
A
\term{range}
is \removed{a pair of iterators }\added{an iterator and a sentinel} that designate the beginning and end of the computation.
...
\pnum
The \tcode{Writable} concept describes the requirements for \newtxt{writing} a value into an iterator's
referenced object.
...
\begin{itemdecl}
  ...
  concept bool Sentinel() {
    return @\newtxt{Semi}@regular<@\newtxt{S}@>() &&
      Iterator<I>() &&
      @\newtxt{Weakly}@EqualityComparable<@\newtxt{S}@, I>();
  }
\end{itemdecl}
...
\pnum
The \tcode{Sized\newtxt{Sentinel}} concept specifies
requirements on a\newtxt{n} \tcode{Iterator} and a \tcode{Sentinel}
...
\pnum
The \tcode{as_function_t} is an alias used to turn a
...
\tcode{IndirectlyMovable}, \tcode{IndirectlyCopyable}, and \tcode{IndirectlySwappable}.} There are \newtxt{three} relational concepts
...
\pnum
The \tcode{IndirectlyMovable} concept \newtxt{specifies} the relationship between a \tcode{Readable}
...
\pnum
The \tcode{IndirectlyCopyable} concept \newtxt{specifies} the relationship between a \tcode{Readable}
...
\begin{codeblock}
  ...
  concept bool IndirectlyComparable() {
    return IndirectCallableRelation<R, @\newtxt{p}@rojected<I1, P1>, @\newtxt{p}@rojected<I2, P2>>();
  }
\end{codeblock}
...
\begin{codeblock}
  ...
  concept bool Mergeable() {
    return InputIterator<I1>() &&
      ...
      IndirectCallableStrictWeakOrder<R, @\newtxt{p}@rojected<I1, P1>, @\newtxt{p}@rojected<I2, P2>>();
  }
\end{codeblock}
...
sequences into ordered sequence\newtxt{s} (e.g., \tcode{sort}).
...
\begin{codeblock}
namespace std { @\added{namespace experimental \{ namespace ranges \newtxt{\{ inline namespace v1} \{}@
...
\begin{codeblock}
  ...
  template <Iterator I, Sentinel<I> S>
    @\newtxt{difference_type_t}@<I> advance(I& i, \newtxt{difference_type_t}@<I> n, S bound);
  template <Iterator I, Sentinel<I> S>
...
\begin{codeblock}
  ...
  template <class @\removed{Iterator1}\added{I1}@, class @\removed{Iterator2}\added{I2}@>
  ...
  template <class @\removed{Iterator1}\added{I1}@, class @\removed{Iterator2}\added{I2}@>
    ...
    @\removed{auto}\added{difference_type_t<I2>}@ operator-(
      const reverse_iterator<@\changed{Iterator1}{I1}@>& x,
  ...
  template <@\changed{class Iterator}{RandomAccessIterator I}@>
    reverse_iterator<@\changed{Iterator}{I}@>
      operator+(
    @\removed{typename reverse_iterator<Iterator>::difference_type}\added{difference_type_t<I>}@ n,
  ...
  template <@\removed{class Iterator}\added{InputIterator I}@> class move_iterator;
  template <class @\removed{Iterator1}\added{I1}@, class @\removed{Iterator2}\added{I2}@>
      @\added{requires EqualityComparable<I1, I2>()}@
    bool operator==(
  ...
  template <@\changed{class Iterator}{RandomAccessIterator I}@>
    move_iterator<@\changed{Iterator}{I}@>
      operator+(
    @\removed{typename move_iterator<Iterator>::difference_type}\added{difference_type_t<I>}@ n,
    const move_iterator<@\changed{Iterator}{I}@>& x);
  template <@\removed{class Iterator}\added{InputIterator I}@>
...
\begin{codeblock}
  ...
  template <class I1, class @\newtxt{I2}@, \newtxt{Sentinel<I2> S1}@, \newtxt{Sentinel<I1>}@ S2>
  ...
  template <@\newtxt{class}@ I1, @\newtxt{class}@ I2>
    ...
    bool operator!=(
      default_sentinel x, const counted_iterator<@\newtxt{auto}@>& y);
  template <@\newtxt{class}@ I1, @\newtxt{class}@ I2>
      requires Common<I1, I2>()
    bool operator<(
      const counted_iterator<I1>& x, const counted_iterator<I2>& y);
  ...
  template <Iterator I>
    counted_iterator<I> make_counted_iterator(I i, @\newtxt{difference_type_t}@<I> n);

  template <Iterator I>
    void advance(counted_iterator<I>& i, @\newtxt{difference_type_t}@<I> n);

  // \ref{unreachable.sentinels} Unreachable sentinels
  @\newtxt{class}@ unreachable;
  template <Iterator I>
...
shall be an alias for \tcode{decltype(*declval<\newtxt{R\&}>())}.
\end{addedblock}
...
\begin{codeblock}
  template <InputIterator I> struct @\xname{pointer_type}@ {        @\newtxt{// \expos}@
    using type = add_pointer_t<@\newtxt{reference_t}@<I>>;
  };
  ...
  template <InputIterator I> struct @\xname{iterator_traits}@<I> {  @\newtxt{// \expos}@
    using difference_type = @\newtxt{difference_type_t}@<I>;
    using value_type = @\newtxt{value_type_t}@<I>;
    using reference = @\newtxt{reference_t}@<I>;
...
void reverse(@\changed{BidirectionalIterator}{I}@ first, @\changed{BidirectionalIterator}{I}@ last) {
  @\removed{typename iterator_traits<BidirectionalIterator>::difference_type}\newtxt{difference_type_t<I>}@ n =
    distance(first, last);
  --n;
  while(n > 0) {
    @\removed{typename iterator_traits<BidirectionalIterator>::value_type}\newtxt{value_type_t<I>}@
      tmp = *first;
    *first++ = *--last;
    *last = tmp;
    n -= 2;
...
namespace std {
  ...
  struct iterator_traits<Out> {
    using difference_type   = experimental::ranges::@\newtxt{difference_type_t}@<Out>;
...
\begin{codeblock}
  ...
  struct iterator_traits<In> {
    ...
    using value_type        = experimental::ranges::@\newtxt{value_type_t}@<In>;
    using reference         = @\seebelow@;
    using pointer           = @\seebelow@;
...
\pnum
Let type \tcode{C} be \tcode{experimental::ranges::}\tcode{\newtxt{iterator_category_t}}\tcode{<In>}.
...
\item If \tcode{C} is the same as or inherits from \tcode{std::input_iterator_tag} or
      ...
      is \tcode{C}.
\item \newtxt{Otherwise, if} \tcode{experimental::ranges::\newtxt{reference_t}<In>} is not a reference type,
      \tcode{std::\brk{}iterator_traits<In>::\brk{}iterator_category} is \tcode{std::input_iterator_tag}.
\item \newtxt{Otherwise, if} \tcode{C} is the same as or inherits from \tcode{experimental::\brk{}ranges::\brk{}random_access_iterator_tag},
      \tcode{std::\brk{}iterator_traits<In>::\brk{}iterator_category} is \tcode{std::random_access_iterator_tag}.
\item \newtxt{Otherwise, if} \tcode{C} is the same as or inherits from \tcode{experimental::\brk{}ranges::\brk{}bidirectional_iterator_tag},
      \tcode{std::\brk{}iterator_traits<In>::\brk{}iterator_category} is \tcode{std::bidirectional_iterator_tag}.
\item \newtxt{Otherwise, if} \tcode{C} is the same as or inherits from \tcode{experimental::\brk{}ranges::\brk{}forward_iterator_tag},
...
types that satisfy \tcode{Sized\newtxt{Sentinel}} provide the} \tcode{-}
operator\removed{s}, the library provides \removed{two}\added{four} function templates
\tcode{advance}\added{,}
...
\item If \tcode{\newtxt{SizedSentinel<S, I>}()} is satisfied:
      \begin{itemize}
      \item If \brk{}\newtxt{$|\tcode{n}| >= |\tcode{bound - i}|$, equivalent} to \tcode{advance(i, bound)}\newtxt{.}

      \item Otherwise, \newtxt{equivalent} to \tcode{advance(i, n)}.
      \end{itemize}

\item \newtxt{Otherwise, i}ncrements (or decrements for negative \tcode{n})
...
\removed{\requires
If }\removed{\tcode{InputIterator} meets the requirements of random access iterator}
\removed{ \tcode{last} shall be reachable from
...
\end{codeblock}\end{addedblock}\begin{codeblock}
    ...
    reverse_iterator(const reverse_iterator<@\removed{U}\added{ConvertibleTo<I>}@>& @\removed{u}\added{i}@);
    @\removed{template <class U>}@
    reverse_iterator& operator=(const reverse_iterator<@\removed{U}\added{ConvertibleTo<I>}@>& @\removed{u}\added{i}@);
    ...
    reverse_iterator& operator-=(difference_type n)@\removed{;}@
      @\added{requires RandomAccessIterator<I>();}@
    @\removed{\unspec}\added{reference}@ operator[](difference_type n) const@\removed{;}@
      @\added{requires RandomAccessIterator<I>();}@
  @\removed{protected}\added{private}@:
...
\tcode{current}
with
\tcode{\removed{u}\added{i}.current}.
\end{itemdescr}
...
@\removed{template <class U>}@
reverse_iterator&
  operator=(const reverse_iterator<@\removed{U}\added{ConvertibleTo<I>}@>& @\removed{u}\added{i}@);
\end{itemdecl}

\begin{itemdescr}
\pnum
\effects
Assigns \tcode{\removed{u.base()}\added{i.current}} to \tcode{current}.
...
\indexlibrary{\idxcode{operator[]}!\idxcode{reverse_iterator}}%
\begin{itemdecl}
@\removed{\unspec}\added{reference}@ operator[](
...
template <@\changed{class Iterator}{RandomAccessIterator I}@>
  reverse_iterator<@\changed{Iterator}{I}@>
    operator+(
  @\removed{typename reverse_iterator<Iterator>::difference_type}\newtxt{difference_type_t<I>}@ n,
  const reverse_iterator<@\changed{Iterator}{I}@>& x);
\end{itemdecl}
...
\end{codeblock}\end{addedblock}\begin{codeblock}
    ...
    @\removed{template <class U>}@
    move_iterator(const move_iterator<@\removed{U}\added{ConvertibleTo<I>}@>& @\removed{u}\added{i}@);
    @\removed{template <class U>}@
    move_iterator& operator=(const move_iterator<@\removed{U}\added{ConvertibleTo<I>}@>& @\removed{u}\added{i}@);

    @\removed{iterator_type}\added{I}@ base() const;
    reference operator*() const;
...
\removed{\returns}
\added{\effects Equivalent to }
\tcode{x.\removed{base()}\added{current} == y.\removed{base()}\added{current}}.
\end{itemdescr}
...
namespace std { namespace experimental { namespace ranges@\newtxt{\{ inline namespace v1}@ {
  ...
  public:
    ...
    common_iterator(I i);
    common_iterator(S s);
    common_iterator(const common_iterator<@\newtxt{ConvertibleTo<I>}@, @\newtxt{ConvertibleTo<S>}@>& u);
    common_iterator& operator=(const common_iterator<@\newtxt{ConvertibleTo<I>}@, @\newtxt{ConvertibleTo<S>}@>& u);
...
\pnum
\effects Constructs a \tcode{counted_iterator}, initializing
\tcode{current} with \tcode{\newtxt{i.current}} and \tcode{cnt} with \tcode{\newtxt{i.cnt}}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Assigns \tcode{\newtxt{i.current}} to
\tcode{current} and \tcode{\newtxt{i.cnt}} to \tcode{cnt}.
...
\indexlibrary{\idxcode{counted_iterator}!\idxcode{operator[]}}%
\begin{itemdecl}
  @\removed{\unspec}\added{decltype(auto)}@ operator[](difference_type n) const
    requires RandomAccessIterator<I>();
\end{itemdecl}
...
\effects
\begin{codeblock}
i = make_counted_iterator(next(i.@\newtxt{current}@, n), i.@\newtxt{cnt}@ - n);
\end{codeblock}
\end{itemdescr}
...
\pnum
\effects
Constructs the end-of-stream iterator. If \tcode{T} is a literal type, then \removed{this}\added{these}
constructor\added{s} shall be\removed{ a} \tcode{constexpr} constructor\added{s}.

\pnum
\postcondition \tcode{in_stream == \removed{0}\added{nullptr}}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\requires \tcode{in_stream != \removed{0}\added{nullptr}}.
...
namespace std { @\added{namespace experimental \{ namespace ranges \newtxt{\{ inline namespace v1} \{}@
  ...
  public:
    ...
    ostream_iterator(const ostream_iterator@\removed{<T, charT, traits>}@& x) @\newtxt{noexcept}@;
   ~ostream_iterator();
...
\pnum
\effects
Initializes \tcode{out_stream} and \tcode{delim} with \newtxt{\tcode{nullptr}}.
\end{itemdescr}
\end{addedblock}
...
\pnum
\effects
Initializes \tcode{out_stream} with \tcode{\&s} and \tcode{delim} with \newtxt{\tcode{nullptr}}.
\end{itemdescr}
...
\begin{codeblock}
*out_stream << value;
if(delim != @\removed{0}\added{nullptr}@)
  *out_stream << delim;
return *this;
\end{codeblock}
...
\tcode{istreambuf_iterator()}
and the constructor
\tcode{istreambuf_iterator(\removed{0}\added{nullptr})}
...
\begin{codeblock}
namespace std { @\newtxt{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  template<class charT, class traits = char_traits<charT>>
  class istreambuf_iterator
...
\begin{addedblock}
\pnum
\requires \tcode{sbuf_ != \removed{0}\added{nullptr}}.
\end{addedblock}
...
\pnum
In addition to the three fundamental range categories, this \newtxt{document} defines
...
concept bool SizedRange() {
  @\newtxt{return Range<T>() \&\&}@
    ...
    @\newtxt{requires (const remove_reference_t<T>\& t) \{}@
      ...
      @\newtxt{\{ ranges::size(t) \} -> ConvertibleTo<difference_type_t<iterator_t<T>{>}{>};}@
    @\newtxt{\};}@
@\newtxt{\}}@
\end{itemdecl}
...
two are differentiated with the help of the \tcode{\newtxt{enable}_view}
trait. Users may specialize \tcode{\newtxt{enable}_view}
...
\item \newtxt{Otherwise, if} both \tcode{T} and \tcode{const T} satisfy \tcode{Range} and \tcode{reference_t}
...
\pnum
The \tcode{ForwardRange} concept \newtxt{specifies} requirements of an
\tcode{InputRange} type for which \tcode{begin} returns a \newtxt{type that satisfies}
\tcode{ForwardIterator}~(\ref{iterators.forward}).

\begin{codeblock}
template <class T>
concept bool ForwardRange() {
  return InputRange<T>() && ForwardIterator<@\newtxt{iterator_t}@<T>>();
}
\end{codeblock}
...
\pnum
The \tcode{BidirectionalRange} concept \newtxt{specifies} requirements of a
\tcode{ForwardRange} type for which \tcode{begin} returns a \newtxt{type that satisfies}
\tcode{BidirectionalIterator}~(\ref{iterators.bidirectional}).

\begin{codeblock}
template <class T>
concept bool BidirectionalRange() {
  return ForwardRange<T>() && BidirectionalIterator<@\newtxt{iterator_t}@<T>>();
}
\end{codeblock}
...
\pnum
The \tcode{RandomAccessRange} concept \newtxt{specifies} requirements of a
\tcode{BidirectionalRange} type for which \tcode{begin} returns a \newtxt{type that satisfies}
\tcode{RandomAccessIterator}~(\ref{iterators.random.access}).

\begin{codeblock}
template <class T>
concept bool RandomAccessRange() {
  return BidirectionalRange<T>() && RandomAccessIterator<@\newtxt{iterator_t}@<T>>();
}
\end{codeblock}
...
\begin{itemdescr}
\pnum \returns \tcode{ranges::distance(\newtxt{ranges::}begin(r), \newtxt{ranges::}end(r))}
\end{itemdescr}
