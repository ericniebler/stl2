...
\pnum
\removed{Most of the library's algorithmic templates that operate on data structures
have interfaces that use ranges. A
\term{range}
is a pair of iterators that designate the beginning and end of the computation.}
A range \removed{\range{i}{i}}\added{\range{i}{s}}
is \removed{an} empty \removed{range}\added{ if \tcode{i == s}};
...
\pnum
\changed{R}{A r}ange \changed{\range{i}{j}}{\range{i}{s}}
...
\begin{codeblock}
  ...
  concept bool Readable() {
    @\oldtxt{return Semiregular<I>() \&\&}@
      @\oldtxt{requires(const I i) \{}@
        @\oldtxt{typename value_type_t<I>;}@
        @\oldtxt{\{ *i \} -> const value_type_t<I>\&; // pre: i is dereferenceable}@
      @\oldtxt{\};}@
    @\newtxt{return Movable<I>() \&\& DefaultConstructible<I>() \&\&}@
      @\newtxt{requires(const I\& i) \{}@
        @\newtxt{typename value_type_t<I>;}@
        @\newtxt{typename reference_t<I>;}@
        @\newtxt{typename rvalue_reference_t<I>;}@
        @\newtxt{\{ *i \} -> Same<reference_t<I>{}>;}@
        @\newtxt{\{ ranges::iter_move(i) \} -> Same<rvalue_reference_t<I>{}>;}@
      @\newtxt{\} \&\&}@
      @\newtxt{// Relationships between associated types}@
      @\newtxt{CommonReference<reference_t<I>, value_type_t<I>\&>() \&\&}@
      @\newtxt{CommonReference<reference_t<I>, rvalue_reference_t<I>{}>() \&\&}@
      @\newtxt{CommonReference<rvalue_reference_t<I>, const value_type_t<I>\&>() \&\&}@
      @\newtxt{Same<}@
        @\newtxt{common_reference_t<reference_t<I>, value_type_t<I>{}>,}@
        @\newtxt{value_type_t<I>{}>() \&\&}@
      @\newtxt{Same<}@
        @\newtxt{common_reference_t<rvalue_reference_t<I>, value_type_t<I>{}>,}@
        @\newtxt{value_type_t<I>{}>();}@
  }
\end{codeblock}
...
\ednote{With the addition of proxy iterator support, ``If \tcode{E} is an xvalue'' no longer
captures the proper semantics, since a proxy for an rvalue will in all likelihood be an xvalue. This
must be revisited.}
...
\begin{codeblock}
  template <Readable I, IndirectRegularCallable<I> Proj>
    @\oldtxt{requires RegularCallable<Proj, reference_t<I>{}>()}@
  struct projected {
    using value_type = decay_t<indirect_result_of_t<Proj&(I)>>;
    @\newtxt{indirect_}@result_of_t<Proj&(@\oldtxt{reference_t<}@I@\oldtxt{>}@)> operator*() const;
  };
...
\begin{codeblock}
  @\oldtxt{template <class In>}@
  @\oldtxt{using rvalue_reference_t =}@
    @\oldtxt{decltype(std::move(declval<reference_t<In>{}>()));}@
...
\oldtxt{reference types of}\newtxt{values referenced by} two \tcode{Readable} types.
...
\begin{codeblock}
  ...
  concept bool IndirectlySwappable() {
    return Readable<I1>() && Readable<I2>() &&
      @\oldtxt{Swappable<reference_t<I1>, reference_t<I2>{}>();}@
      @\newtxt{requires(const I1 i1, const I2 i2) \{}@
        @\newtxt{ranges::iter_swap(i1, i2);}@
        @\newtxt{ranges::iter_swap(i2, i1);}@
        @\newtxt{ranges::iter_swap(i1, i1);}@
        @\newtxt{ranges::iter_swap(i2, i2);}@
      @\newtxt{\};}@
  }
\end{codeblock}
...
\begin{addedblock}
\begin{codeblock}
  @\newnewtxt{// \ref{iterator.traits}, traits:}@
  ...
  @\newtxt{template <_Dereferenceable T> using reference_t}@
    @\newtxt{= decltype(*declval<T\&>());}@

  @\newtxt{template <_Dereferenceable T>}@
      @\newtxt{requires \seebelow{ }using rvalue_reference_t}@
    @\newtxt{= decltype(ranges::iter_move(declval<T\&>()));}@

  @\newtxt{template <Readable T> using iter_common_reference_t}@
    @\newtxt{= common_reference_t<reference_t<T>, value_type_t<T>\&>;}@

  @\newtxt{template <class I1, class I2> struct is_indirectly_movable;}@
  @\newtxt{template <class I1, class I2 = I1> struct is_indirectly_swappable;}@
  @\newtxt{template <class I1, class I2> struct is_nothrow_indirectly_movable;}@
  @\newtxt{template <class I1, class I2 = I1> struct is_nothrow_indirectly_swappable;}@

  @\newtxt{template <class I1, class I2> constexpr bool is_indirectly_movable_v}@
    @\newtxt{= is_indirectly_movable<I1, I2>::value;}@
  @\newtxt{template <class I1, class I2> constexpr bool is_indirectly_swappable_v}@
    @\newtxt{= is_indirectly_swappable<I1, I2>::value;}@
  @\newtxt{template <class I1, class I2> constexpr bool is_nothrow_indirectly_movable_v}@
    @\newtxt{= is_nothrow_indirectly_movable<I1, I2>::value;}@
  @\newtxt{template <class I1, class I2> constexpr bool is_nothrow_indirectly_swappable_v}@
    @\newtxt{= is_nothrow_indirectly_swappable<I1, I2>::value;}@
\end{codeblock}
\end{addedblock}
\begin{codeblock}
  ...
  @\newnewtxt{// \ref{std.iterator.tags}, iterator tags}@
...
\pnum
Effects: Equivalent to \tcode{static_cast<Ret>(*r)}
\end{itemdescr}
\end{itemize}
...
\oldoldtxt{In addition, the types}
\begin{codeblock}
@\oldtxt{reference_t<R>}@
\end{codeblock}
\oldtxt{shall be an alias for \tcode{decltype(*declval<R\&>())}.}
\begin{removedblock}
\begin{codeblock}
...
\note The argument order in the \textit{Effects} element is reversed because \tcode{cnt}
counts down, not up.
...
namespace std { @\added{namespace experimental \{ namespace ranges \{ inline namespace v1 \{}@
  ...
  public:
    ...
    ostream_iterator(ostream_type& s) @\added{noexcept}@;
    ostream_iterator(ostream_type& s, const charT* delimiter) @\added{noexcept}@;
    ostream_iterator(const ostream_iterator@\removed{<T, charT, traits>}@& x) @\added{noexcept}@;
   ~ostream_iterator();
...
\indexlibrary{\idxcode{ostream_iterator}!constructor}%
\begin{itemdecl}
ostream_iterator(const ostream_iterator& x) @\added{noexcept}@;
\end{itemdecl}
...
\item
  Otherwise, \tcode{DECAY_COPY((E).begin())} if its type \tcode{I} meets the
  syntactic requirements of \tcode{Iterator<I>()}. If
  \tcode{Iterator} is not satisfied, the program is ill-formed
  with no diagnostic required.

\item
  Otherwise, \tcode{DECAY_COPY(begin(E))} if its type \tcode{I} meets the
...
\item
  Otherwise, \tcode{DECAY_COPY((E).end())} if its type \tcode{S} meets the
...
\item
  Otherwise, \tcode{DECAY_COPY(end(E))} if its type \tcode{S} meets the
  syntactic requirements of
...
\item
  Otherwise, \tcode{DECAY_COPY((E).rbegin())} if its type \tcode{I} meets the
...
\item
  Otherwise, \tcode{DECAY_COPY((E).rend())} if its type \tcode{S} meets the
  syntactic requirements of
...
\item
  Otherwise, \tcode{DECAY_COPY(((const T\&)(E)).size())} if its type \tcode{I}
  satisfies \tcode{Integral<I>()} and
  \tcode{disable_\-sized_\-range<T>}~(\ref{ranges.sized}) is
  \tcode{false}.

\item
  Otherwise, \tcode{DECAY_COPY(size((const T\&)(E)))} if its type \tcode{I}
...
\item
  ...
  except that \tcode{E} is only evaluated once, if the type of
...
\item
  Otherwise, \tcode{DECAY_COPY((E).data())} if it has pointer to object type.
