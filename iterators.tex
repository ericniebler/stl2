...
\ednote{TODO this gets defined later on in terms of \tcode{Projected}. Unify.}

\pnum
The \tcode{IndirectlyComparable} concept describes an relation between
two \tcode{Readable} types and (optionally) a \tcode{Relation} and two projections.
...
\tcode{Writable}~(\ref{writable.iterators})) and which be both pre- and post-incremented. However,
...
\ednote{TODO Say something about the post-conditions mentioned by Table 108, or is that part of the
Writable concept? }
...
\ednote{Specifying the algorithms in terms of these indirect invokable concepts would ease
...
\begin{codeblock}
  ...
  template <class F, Readable...Is>
  concept bool IndirectRegularInvokable =
    RegularInvokable<F, ValueType<Is>...>;
...
WeaklyIncrementable concept(~\ref{TODO Reference}), \tcode{Readable} is the name of a type that
models the Readable concept(~\ref{TODO Reference}), and \tcode{WeakInputIterator} is the name of a
...
\ednote{Think through the implications here.}
...
\begin{codeblock}
  ...
  template <WeaklyIncrementable I> struct @\xname{iterator_traits2}@ {
  ...
  @\ednote{Is this syntax right?}@
  template <WeakInputIterator I> struct @\xname{iterator_traits2}@ {
    using difference_type = DifferenceType<I>;
  ...
  template <class, class = void> struct @\xname{iterator_traits}@ { };
  template <class I, enable_if_t<WeaklyIncrementable<I>>>
  struct @\xname{iterator_traits}@ : @\xname{iterator_traits2}@<I> { };
  template <class I>
    using iterator_traits = @\xname{iterator_traits2}@<I>;
\end{codeblock}
...
@\added{requires RandomAccessIterator<I>}@
reverse_iterator
operator+(typename reverse_iterator<@\changed{Iterator}{I}@>::difference_type n) const;
\end{itemdecl}
...
@\added{requires RandomAccessIterator<I>}@
reverse_iterator&
operator+=(typename reverse_iterator<@\changed{Iterator}{I}@>::difference_type n);
\end{itemdecl}
...
@\added{requires RandomAccessIterator<I>}@
@\unspec@ operator[](
    typename reverse_iterator<@\changed{Iterator}{I}@>::difference_type n) const;
\end{itemdecl}
