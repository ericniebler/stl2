...
\pnum
As presented in this document, the vast majority of algorithms do not accept rvalue \oldtxt{Iterables}\newtxt{Ranges}.
...
\oldtxt{Iterable}\newtxt{Range} they point into gets destroyed -- but not for \textit{all}
\oldtxt{Iterables}\newtxt{Ranges}. For a\oldtxt{n} \oldtxt{Iterable}\newtxt{Range} that
is little more than a pair of iterators, it is perfectly safe to use an iterator into the
\oldtxt{Iterable}\newtxt{Range}
after the \oldtxt{Iterable}\newtxt{Range} has been destroyed. The iterator's validity is not tied to the
\oldtxt{Iterable}\newtxt{Range}'s lifetime at all.
...
obtain an iterator into an expired \oldtxt{Iterable}\newtxt{Range}, the user must explicitly call a method called
...
specialized on user-defined \oldtxt{Iterable}\newtxt{Range} types. That way, \oldtxt{Iterables}\newtxt{Ranges} that guarantee their iterators can
outlive the \oldtxt{Iterable}\newtxt{Range} will never have their iterators wrapped in \tcode{dangling}.
