...
associated \tcode{\oldtxt{DifferenceType}}\tcode{\newtxt{difference_type_t}}. The
\tcode{\oldtxt{DifferenceType}}\tcode{\newtxt{difference_type_t}} must be a built-in integral type, and
...
\tcode{\oldtxt{DifferenceType}}\tcode{\newtxt{difference_type_t}} in the concept requirements places a hard upper limit on how big ranges can
...
integral \tcode{\oldtxt{DifferenceType}}\tcode{\newtxt{difference_type_t}}, and some are not.
...
the access/traversal boundary like suggested in ``New Iterator \oldtxt{Categories}\newtxt{Concepts}''
(Abrahams et. al.~\cite{N1550}) would be a more complete fix, at the probable expense of greater
